...
function DownloadProgress({ acquisition, modelName, onCancel, pending = false }: { acquisition: CatalogLocalModel["acquisitionState"]; modelName: string; onCancel?: () => void; pending?: boolean }) {
  ...
  return <div className="w-full min-w-0" aria-label="Model download">
    ...
    <Progress data-testid={automation.modelDownloadProgress} data-download-stage={progress.stage} data-download-completed-bytes={progress.completedBytes} data-download-total-bytes={progress.totalBytes} aria-label="Download progress" aria-valuetext={bytes} value={percent} trackClassName="h-2" indicatorClassName={`bg-blue-700 dark:bg-blue-500 ${percent === null ? "w-full motion-safe:animate-pulse" : ""}`} />
...
function ModelCard({ model, showMemory = false, replacing }: { model: CatalogLocalModel; showMemory?: boolean; replacing?: string }) {
  ...
  return <article data-testid={automation.model(model.modelId)} data-acquisition-state={acquisition._tag} data-model-ready={residency?._tag === "Ready"} data-model-installed={localModelIsInstalled(model)} className={pageLayout.modelCard}>
