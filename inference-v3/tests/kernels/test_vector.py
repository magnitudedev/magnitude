...
from magnitude_engine.kernels.norm.portable import rms_norm
...
def execute(factory, inputs, shape):
    ...
    kernel = context.compile(factory(context.capability))
...
def test_pointwise(kind):
    ...
    actual = execute(
        lambda capability: pointwise(a.size, kind, capability=capability, precision=REFERENCE_F32),
        (a, b),
        a.shape,
    )
...
def test_norm(width):
    ...
    actual = execute(
        lambda capability: rms_norm(
            3, width, 1e-6, capability=capability, precision=REFERENCE_F32
        ),
        (a, w),
        a.shape,
    )
...
def test_residual_add_keeps_fp32_accumulation_with_bf16_update(output_dtype):
    ...
    with ExitStack() as cleanup:
        ...
        operation = Elementwise(context, Pointwise.ADD, REFERENCE_F32)
