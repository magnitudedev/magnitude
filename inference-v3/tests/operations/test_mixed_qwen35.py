...
from magnitude_engine.kernels import precision as precision_module
...
from tests.operations.test_qwen35_model import fixture
...
@pytest.mark.parametrize(
    "precision", [precision_module.MIXED_BF16, precision_module.MIXED_BF16_F32_RESIDUAL]
)
def test_mixed_storage_prefill_decode_and_checkpoint(tmp_path, precision):
    path = tmp_path / "mixed.gguf"
    weights = fixture(path)
    tokens = (1, 3, 2, 7, 8, 12, 14, 3, 6, 11, 9)
    expected, _ = reference(
        weights, tokens, 8, residual_f32=precision.residual == DType.F32
    )
    with ExitStack() as cleanup:
        ...
        binding = bind(artifact, context, precision)
        cleanup.callback(binding.close)
        provider = binding.operations
        model = DenseRuntime(
            inspect_dense(artifact.directory, artifact.identity), provider, precision
        )
