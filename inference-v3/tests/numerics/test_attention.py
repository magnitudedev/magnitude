...
from magnitude_engine.numerics.attention import merge_runs, run_attention, run_attention_cpu
...
def test_independent_physical_runs_merge_to_contiguous_causal_attention(
    ...
    merger = context.compile(
        merge_runs(rows, heads, width, 3 * partitions, cpu=backend == Backend.LLVM)
    )
