...
def test_allocations_coalesce_only_without_a_physical_visibility_gap(first_length):
    ...
    with ExitStack() as cleanup:
        ...
        attention = CausalAttention(context, heads, kv_heads, width)
...
def test_grouped_reads_mask_nan_gaps_and_nonmonotonic_physical_placement(rows, dtype):
    ...
    with ExitStack() as cleanup:
        ...
        append, attention = KVAppend(context, kh, width), CausalAttention(context, heads, kh, width)
        cleanup.callback(append.close)
        cleanup.callback(attention.close)
        ...
        assert pool.slab_count == 0
        output.close()
        # Reusable numerical scratch belongs to the operation, not the KV runs.
        attention.close()
        assert context.allocated_bytes == 0
...
def test_attention_scratch_reuses_backing_and_pins_growth_until_completion():
    ...
    with ExitStack() as cleanup:
        ...
        cleanup.callback(context.close)
        attention = CausalAttention(context, 4, 2, 40)
        cleanup.callback(attention.close)
        cases = tuple(
            AttentionCase(
                attention,
        ...
        # closing the operation must preserve every submitted consumer's backing.
        attention.close()
