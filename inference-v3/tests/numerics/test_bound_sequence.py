...
def test_nested_binding_replays_and_invocation_outlives_every_owner():
    ...
    add = context.compile(pointwise(67, Pointwise.ADD, cpu=backend == Backend.LLVM))
    first = BoundSequence(
        context,
        (
...
def test_independent_region_joins_producer_and_consumer_with_rebinding():
    ...
    with ExitStack() as cleanup:
        ...
        add = context.compile(pointwise(count, Pointwise.ADD, cpu=backend == Backend.LLVM))
        multiply = context.compile(
            pointwise(count, Pointwise.MULTIPLY, cpu=backend == Backend.LLVM)
        )
