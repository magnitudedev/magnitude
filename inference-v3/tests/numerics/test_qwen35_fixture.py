...
from magnitude_engine.blueprints import artifacts, execution, models, operations
...
def test_pinned_prefill_and_teacher_forced_decode():
    ...
    with ExitStack() as cleanup:
        artifact = artifacts.GGUF(path=source)
        root = models.Qwen35Dense(
            description=models.Qwen35DenseDescription(artifact=artifact),
            operations=operations.ResidentOperations(
                artifact=artifact,
                context=execution.Device(backend=backend, budget_bytes=8 * 1024**3),
            ),
        )
