...
def test_bfloat_fusion_keeps_intermediate_publications(kind):
    ...
    try:
        ...
        weight_spec = mt.TensorSpec(codes.shape, mt.DType.BF16).with_representation(
        ...
        if kind.startswith("attention"):
            ...
            def attention_function(q, history, visible, gate, w):
                attended = mt.causal_attention(q, history, visible, sequence_count=1)
                return mt.linear(mt.reshape(attended * mt.sigmoid(gate), (rows, channels)), w)

            function = attention_function
            ...
            def recurrent_function(mixed, norm, gate, w):
                normalized = mt.rms_norm(mixed, norm, epsilon=1e-6)
                return mt.linear(mt.reshape(normalized, (rows, channels)) * mt.silu(gate), w)

            function = recurrent_function
            ...
            def dense_function(value, w):
                return mt.linear(mt.silu(mt.linear(value, w)) * mt.linear(value, w), w)
        ...
        inputs = []
        for array in arrays:
            dtype = mt.DType.I32 if array.dtype == torch.int32 else mt.DType.BF16
            payload = (
                array.numpy().tobytes()
                if dtype == mt.DType.I32
                else array.view(torch.uint16).numpy().tobytes()
            )
            resource = device.upload(mt.TensorSpec(tuple(array.shape), dtype), payload)
        ...
        signature = mt.Signature(
            tuple(
                mt.Argument(
                    value.spec,
                    f"x{i}",
                    mt.ValueKind.RESOURCE
                    if kind.startswith("attention") and i == 1
                    else mt.ValueKind.INPUT,
                )
        ...
        compiled = mt.compile(
            ...
            device=device,
            constants={"w": weight},
            options=mt.CompileOptions(mode="decode" if kind.endswith("decode") else "prefill"),
        )
        assert any(
            c.selected and c.name.startswith(family) for c in compiled.diagnostics.candidates
        )
        execution = (
...
def test_float_residual_and_bfloat_normalized_publications_share_one_region():
    ...
    try:
        for array in (left, update, gain):
            dtype = mt.DType.F32 if array.dtype == torch.float32 else mt.DType.BF16
            payload = (
                array.numpy().tobytes()
                if dtype == mt.DType.F32
                else array.view(torch.uint16).numpy().tobytes()
            )
            resources.append(device.upload(mt.TensorSpec(tuple(array.shape), dtype), payload))

        def function(left, update, gain):
            residual = left + mt.cast(update, mt.DType.F32)
            return residual, mt.rms_norm(residual, gain, output_dtype=mt.DType.BF16)

        compiled = mt.compile(
            function,
            signature=mt.Signature(
                tuple(mt.Argument(value.spec, f"v{i}") for i, value in enumerate(resources))
            ),
            device=device,
            constants={},
        ...
        assert any(
            c.selected and c.name.startswith("residual_rms.fused")
