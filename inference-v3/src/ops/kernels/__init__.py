...
def build_primitive(graph, root, context, *, remaining):
    ...
    if node.operation == "reshape":
        ...
        return (BoundOperation(f"reshape.view@{root}", frozenset({root}), node.inputs, node.outputs,
                               lambda operands: None, aliases=((output, source),), kernel_count=0),)
    if node.operation in {"linear", "matmul"}:
        ...
        body = (PackedMatrixRule() if packet_format(graph.value(node.inputs[1]).spec) is not None
        ...
        if any(binding is not None and binding.residency == Residency.STREAMED for binding in bindings):
...
def _authored(result, body):
    dispatch = tuple(item for item in code_dependencies(build_primitive)
