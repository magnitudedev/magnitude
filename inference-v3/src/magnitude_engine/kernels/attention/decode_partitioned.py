...
NOTE: this is the one schedule that still reaches a target directly. Its
cooperative matrix operations and the ``metal.simdgroup`` scope have no portable
spelling in the fork yet, so the pass-through rule carries a single recorded
exception for this file; see design/inference/engine/kernels.md. Everything
about its selection is already portable: the candidate names a strategy and is
gated only by a capability.
