"""Whole-step Qwen specialization over Magnitensor resources."""
...
class InvocationSpecs:
    batch: int
    tokens: mt.TensorSpec
    coordinates: mt.TensorSpec
    recurrent_offsets: mt.TensorSpec | None
    output_rows: mt.TensorSpec | None
    draws: mt.TensorSpec | None
    destinations: tuple[mt.TensorSpec, ...]
    visible: tuple[mt.TensorSpec, ...]
    attention_state: tuple[mt.TensorSpec, ...]
    convolution_state: tuple[mt.TensorSpec, ...]
    delta_state: tuple[mt.TensorSpec, ...]
    features: tuple[mt.TensorSpec, ...] = ()
    feature_rows: tuple[mt.TensorSpec, ...] = ()
    ...
    def __post_init__(self) -> None:
        ...
        if self.coordinates != mt.TensorSpec((self.tokens.shape[0], 3), mt.DType.I32):
            raise ValueError("Qwen coordinates differ from token rows")
        if self.recurrent_offsets is not None and self.recurrent_offsets != mt.TensorSpec(
            (self.batch + 1,), mt.DType.I32
        ):
        ...
        if (output_rows is None) != (self.draws is None) or (
            ...
            and self.draws != mt.TensorSpec((output_rows.shape[0], 6), mt.DType.U32)
        ):
            raise ValueError("Qwen draw rows must exactly match selected output rows")
        if len(self.features) != len(self.feature_rows) or any(
            value.rank != 2
            or value.dtype != mt.DType.F32
            or rows != mt.TensorSpec((value.shape[0],), mt.DType.I32)
...
class WeightResidency(Protocol):
    def resident(self, descriptor, dtype: mt.DType) -> mt.Resource: ...
...
class ProgramDefinition:
    ...
    options: mt.CompileOptions
    constant_specs: Mapping[str, mt.TensorSpec]


class TensorProgram:
    """Description-bound factory for maximal Magnitensor specializations."""

    def __init__(
        self,
        description: DenseDescription,
        device: mt.Device,
        weights: Mapping[str, mt.Resource] | WeightResidency,
    ):
        ...
        self.weights = (
            ...
            if isinstance(weights, Mapping)
                ...
                descriptor.name: weights.resident(descriptor, dtype)
...
def define(
    ...
    *,
    ...
    kwargs: dict[str, mt.Argument] = {}
    for name, spec in _state_arguments(specs):
        kwargs[name] = mt.Argument(spec, name, mt.ValueKind.RESOURCE)
    for descriptor, _ in weight_roles(description):
        kwargs[descriptor.name] = mt.Argument(
            weight_specs[descriptor.name], descriptor.name, mt.ValueKind.CONSTANT
        )
    arguments = [
        mt.Argument(specs.tokens, "tokens"),
        mt.Argument(specs.coordinates, "coordinates"),
    ]
    if specs.recurrent_offsets is not None:
        arguments.append(mt.Argument(specs.recurrent_offsets, "recurrent_offsets"))
    ...
    def function(*args, **bound):
        ...
        result = decoder(
            ...
            feature_values=(mt.concatenate(feature_values, axis=0) if feature_values else None),
            feature_rows=(mt.concatenate(feature_rows, axis=0) if feature_rows else None),
        )
        if draws is None:
            return result
        outputs = cast(tuple[mt.Tensor, ...], result)
        return outputs[0], mt.sample(outputs[0], draws), *outputs[1:]
...
def _concatenate_batch(values: tuple[mt.Tensor, ...]) -> mt.Tensor:
    return values[0] if len(values) == 1 else mt.concatenate(values, axis=0)
...
def _bind_weights(description: DenseDescription, values: Mapping[str, mt.Tensor]) -> DecoderTensors:
    blocks = []
