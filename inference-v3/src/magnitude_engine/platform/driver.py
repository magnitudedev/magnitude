...
Kernels reach the device through TileLang's public compilation entry: this
driver is one implementation for every backend and passes no per-target
compiler configuration. Target pipeline defaults and the execution adapter for
a target belong to the fork.

This is also the only place a ``Backend`` value decides anything numerical: it
resolves one ``Capability``, which is what every other package reads.
...
from magnitude_engine.kernels.capabilities import HOST, Capability, capability_probe
...
class TorchDriver:
    ...
    def _probe(self) -> Capability:
        """Resolve the endpoint facts kernels are allowed to consult.

        TVM's ``Target("metal")`` reports placeholder limits (a 16-lane warp and
        256 threads per block) that are wrong for Apple silicon, so the compiled
        pipeline is the truthful source there. CUDA and HIP populate their target
        attributes per architecture and are read directly. The host has no
        threadgroups, no lanes, and no threadgroup memory.
        """
        if self.backend == Backend.LLVM:
            return HOST
        if self.backend == Backend.METAL:
            adapter = self._adapter(self.compile(capability_probe()))
            return Capability(
                subgroup_width=int(adapter.thread_execution_width),
                threads_per_group=int(adapter.max_total_threads_per_threadgroup),
                shared_memory_bytes=_target_attribute(self.backend, "max_shared_memory_per_block"),
                matrix_instructions=True,
            )
        return Capability(
            subgroup_width=_target_attribute(self.backend, "thread_warp_size"),
            threads_per_group=_target_attribute(self.backend, "max_num_threads"),
            shared_memory_bytes=_target_attribute(self.backend, "max_shared_memory_per_block"),
            matrix_instructions=True,
        )

    @staticmethod
    def _adapter(executable: Executable):
        kernel = executable.native
        assert isinstance(kernel, _Kernel)
        return kernel.kernel.adapter
    ...
    def compile(self, program: PrimFunc) -> Executable:
        ...
        with self.launch_context():
            kernel = tilelang.compile(program, target=self.backend.value)
        executable = Executable(signature, _Kernel(kernel, self.views, self.launch_context), self)
        # The probe itself establishes the width; everything compiled afterwards
        # must agree with the plan that was selected from it.
        resolved = getattr(self, "capability", None)
        if resolved is not None and self.backend == Backend.METAL:
            if int(self._adapter(executable).thread_execution_width) != resolved.subgroup_width:
                raise ValueError("compiled Metal execution width differs from the selected plan")
        return executable


def _target_attribute(backend: Backend, name: str) -> int:
    from tilelang import tvm

    value = tvm.target.Target(backend.value).attrs.get(name)
    if value is None:
        raise RuntimeError(f"target {backend.value} does not report {name}")
    return int(value)
