...
_ACTIVE_TRACE: ContextVar[Trace | None] = ContextVar("magnitensor_trace", default=None)
...
class Signature:
    args: tuple[Argument, ...]
    kwargs: Mapping[str, Argument] = MappingProxyType({})
...
def trace(function, signature: Signature, *, name: str | None = None) -> Graph:
    ...
    if active is not None:
        raise RuntimeError("nested Magnitensor tracing is not supported")
    state = Trace(name or getattr(function, "__name__", "tensor_function"))
    token = _ACTIVE_TRACE.set(state)
    try:
        args = tuple(
            state.argument(argument, index) for index, argument in enumerate(signature.args)
        )
        kwargs = {key: state.argument(argument, key) for key, argument in signature.kwargs.items()}
...
def active_trace() -> Trace:
    ...
    if trace is None:
        raise RuntimeError("Magnitensor operations may only run while tracing")
    return trace
...
def _flatten_outputs(value: Any) -> tuple[Tensor, ...]:
    ...
    if isinstance(value, (tuple, list)):
        ...
        return tuple(result)
    raise TypeError("tensor function outputs must be tensors or nested tuples/lists of tensors")
...
def _source_location() -> SourceLocation | None:
    ...
    try:
        while frame is not None:
            ...
            if "/magnitensor/" not in filename:
