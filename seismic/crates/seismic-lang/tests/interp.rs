...
fn projection_matches_direct_dot_products() {
    ...
    let (n, k) = (6, 128);
    let x: Vec<f64> = (0..k).map(|_| seismic_lang::interp::bf16_round((rng.unit() - 0.5) as f32) as f64).collect();
...
fn rms_norm_matches_definition() {
    ...
    let mut rng = Rng(7);
    let (r, w) = (3, 64);
    let x: Vec<f64> = (0..r * w).map(|_| seismic_lang::interp::bf16_round((rng.unit() * 2.0 - 1.0) as f32) as f64).collect();
    let weight: Vec<f64> = (0..w).map(|_| seismic_lang::interp::bf16_round((rng.unit() + 0.5) as f32) as f64).collect();
    let mut interp = Interpreter::new(&program);
