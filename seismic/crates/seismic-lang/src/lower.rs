...
/// Piece capacity for streaming over a dynamic extent, until the model chooses it.
pub const DEFAULT_PIECE: i64 = 64;
...
pub struct Options {
    /// Piece capacity for streaming over a static extent larger than it. `None` streams the
    /// whole axis as one piece.
...
pub fn lower_with(program: &Program, name: &str, backend: &str, shapes: &HashMap<String, i64>, opts: &Options) -> Result<Lowered, String> {
    ...
    let mut ctx = Inliner { program, backend, selections: Vec::new(), counter: 0, opts: opts.clone(), piece_values: HashMap::new() };
    ...
    let mut vars: Vec<Var> = f.vars.iter().map(|v| Var { ty: subst_ty(&v.ty, &env), ..v.clone() }).collect();
    let body = ctx.inline_block(&f.body, &env, &HashMap::new(), &mut vars, &mut HashMap::new(), 0)?;
    let params = f.params.iter().map(|(n, t)| (n.clone(), subst_ty(t, &env))).collect();
    Ok(Lowered { name: name.to_string(), backend: backend.to_string(), params, vars, body, shapes: shapes.clone(), selections: ctx.selections })
}
...
impl<'a> Inliner<'a> {
    ...
    fn inline_call(&mut self, callee: &str, shape_args: &[Sym], elem_args: &[Elem], args: &[Expr], env: &HashMap<String, Sym>, vmap: &VarMap, vars: &mut Vec<Var>, atom_map: &mut HashMap<String, Atom>, depth: usize) -> Result<Vec<Stmt>, String> {
        ...
        self.inline_block(&body, &inner_env, &inner_map, vars, &mut inner_atoms, depth + 1)
    }
    ...
    fn inline_expr(&mut self, e: &Expr, env: &HashMap<String, Sym>, vmap: &VarMap, vars: &mut Vec<Var>, atom_map: &mut HashMap<String, Atom>) -> Result<Expr, String> {
        ...
        let kind = match &e.kind {
            ...
            ExprKind::TileAlloc { shape, dtype } => ExprKind::TileAlloc { shape: shape.iter().map(|s| subst_sym(s, env, atom_map)).collect(), dtype: *dtype },
            ...
            ExprKind::Builtin { name, args } => ExprKind::Builtin { name: *name, args: args.iter().map(|a| self.inline_expr(a, env, vmap, vars, atom_map)).collect::<Result<_, _>>()? },
...
fn subst_elem_ty(t: &Ty, elems: &HashMap<String, Elem>) -> Ty {
    let f = |s: &Shaped| {
        let elem = match &s.elem {
            Elem::Param(p) => elems.get(p).cloned().unwrap_or_else(|| s.elem.clone()),
            other => other.clone(),
        };
...
fn count_non_element_uses(s: &Stmt, producers: &HashMap<VarId, (Vec<VarId>, Expr)>, out: &mut HashMap<VarId, usize>) {
    let mut expr_uses = |e: &Expr, out: &mut HashMap<VarId, usize>| {
