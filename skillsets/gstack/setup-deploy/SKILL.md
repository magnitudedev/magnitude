...
description: |
  ...
  the configuration to $M/reports/deploy-config.md so all future deploys are automatic.
...
checks, and deploy status commands — then persist the configuration to `$M/reports/deploy-config.md`.

After this runs once, `/land-and-deploy` reads the config file and skips detection entirely.
...
```bash
grep -A 20 "## Deploy Configuration" "$M/reports/deploy-config.md" 2>/dev/null || echo "NO_CONFIG"
```
...
2. **What's the production URL?** (The URL where the app runs)
...
Write the configuration to `$M/reports/deploy-config.md` (create or overwrite):
...
Saved to $M/reports/deploy-config.md. /land-and-deploy will use these settings automatically.

Next steps:
- Run /land-and-deploy to merge and deploy your current PR
- Edit $M/reports/deploy-config.md to change settings
...
- **Idempotent.** Running /setup-deploy multiple times overwrites `$M/reports/deploy-config.md` cleanly.
