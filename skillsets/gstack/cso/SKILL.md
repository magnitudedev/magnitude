...
The bash blocks throughout this skill show WHAT patterns to search for, not HOW to run them. Use your Grep tool (which handles permissions and access correctly) rather than raw bash grep. The bash blocks are illustrative examples — do NOT copy-paste them into a terminal. Do NOT use `| head` to truncate results.
...
List the contents of the project's local AI agent skills directory to check for installed skills.
...
**Tier 2 — global skills (requires permission):** Before scanning globally installed AI coding agent skills or user settings, ask the user:
...
If yes, run the same Grep patterns on globally installed skill files and check hooks in user settings.
If no, proceed with repo-local only.
...
**FP rules:** Skills from well-known trusted sources are trusted (check if skill path resolves to a known repo). Skills that use `curl` for legitimate purposes (downloading tools, health checks) need context — only flag when the target URL is suspicious or when the command includes credential variables.
...
22. Skill files from well-known trusted sources (verify the source before trusting)
...
For each candidate finding, perform an independent verification pass: re-read the relevant code with a skeptic's eye, as if seeing it for the first time without the initial scan's context. Assess independently whether the vulnerability is real. Score 1-10. Below 8 (daily mode) or below 2 (comprehensive mode) = explain why it's not real and discard the finding.

If the agentic environment supports spawning sub-tasks, use that capability for parallel verification with fresh context per finding.
...
**Trend Tracking:** If prior reports exist in `$M/reports/security/`:
```
...
**Remediation Roadmap:** For the top 5 findings, present each one with:
1. Context: The vulnerability, its severity, exploitation scenario
2. Recommendation: Preferred fix with one-line reason
3. Options:
   ...
   - D) Defer — add to project backlog with security label

### Phase 14: Save Report

Write the security report to `$M/reports/security/cso-{date}-{HHMMSS}.json` using this schema:
...
Security reports are written to `$M/reports/security/` (the session workspace) and are not stored in the project directory.
