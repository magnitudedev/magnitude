...
description: |
  ...
  "quality score".
  Voice triggers: "health check", "run all checks", "quality score"
---
...
Check for `$M/reports/health-stack.md`. If found, parse the tools listed there and
skip auto-detection.

If no saved health stack exists, auto-detect available tools:
...
After auto-detection, present the detected tools to the user and ask them to confirm
...
Let me know if any of these look wrong, or if you'd prefer to skip saving this
configuration. Otherwise I'll save it and run the checks."

If confirmed, write the detected configuration to `$M/reports/health-stack.md`:
...
On subsequent runs, check for `$M/reports/health-stack.md` first before auto-detecting.
...
Append one JSONL line to `$M/reports/health-history.jsonl`:
...
Read the last 10 entries from `$M/reports/health-history.jsonl` (if the file exists
and has prior entries).
...
**If this is the first entry in health-history.jsonl (no prior entries):**
Say: "First health check -- no trend data yet. Run /health again after making changes
to track progress."
...
Prioritize suggestions by impact (weight x score deficit):
...
3. **Respect saved config.** If `$M/reports/health-stack.md` is configured, use those exact commands. Do not second-guess.
