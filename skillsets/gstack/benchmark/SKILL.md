...
Before collecting metrics, ensure the target URL is accessible and the dev server (or staging environment) is running.
...
Navigate to `<page-url>` using your browser tool. Then collect performance timing data using your browser tool.

Then gather detailed metrics via JavaScript — evaluate the following in the browser console using your browser tool:
...
Resource analysis — evaluate in browser console using your browser tool:
```javascript
...
Bundle size check — evaluate in browser console using your browser tool:
```javascript
...
Network summary — evaluate in browser console using your browser tool:
```javascript
...
Write to `$M/reports/benchmark/baseline.json`.
...
Read historical baseline files from `$M/reports/benchmark/` and show trends:
...
Write to `$M/reports/benchmark/{date}-benchmark.md` and `$M/reports/benchmark/{date}-benchmark.json`.
