...
description: |
  ...
  Voice triggers: "office hours", "brainstorm this", "is this worth building", "I have an idea"
---
...
1. Read the project's context files (`README.md`, project docs) if they exist.
...
Ask these questions **ONE AT A TIME** one at a time. Push on each one until the answer is specific, evidence-based, and uncomfortable. Comfort means the founder hasn't gone deep enough.
...
Ask these **ONE AT A TIME** one at a time. The goal is to brainstorm and sharpen the idea, not interrogate.
...
Ask the user to confirm each premise. If the user disagrees with a premise, revise understanding and loop back.
...
Present the approaches and ask the user to choose. Do NOT proceed without user approval of the approach.

---

## Phase 5: Design Doc

Write the design document to `$M/plans/office-hours-{title}-{datetime}.md`.
...
{Optional: if an independent subagent review was run, include its key findings here — steelman, key insight, challenged premise, prototype suggestion. Omit this section if no independent review was run.}
...
## Cross-Model Perspective
{Optional: if an independent subagent review was run, include its key findings here — coolest version, key insight, existing tools, prototype suggestion. Omit this section if no independent review was run.}
...
Present the reviewed design doc to the user and ask:
...
Once the design doc is APPROVED, suggest next steps:

- **`/plan-ceo-review`** — for ambitious features, rethink scope and strategy
- **`/plan-eng-review`** — for architecture and implementation planning
- **`/plan-design-review`** — for visual/UX design review

The approved design doc at `$M/plans/` is available for downstream skills to read.
