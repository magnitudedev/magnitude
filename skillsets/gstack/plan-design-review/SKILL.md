...
description: |
  ...
  then fixes the plan to get there. For live site visual audits, use
  /design-review. Use when asked to "review the design plan" or "design critique".
...
If the plan has UI scope, proceed with visual review. Use your browser tool for any live site inspection needed. If you have tools available to generate visual mockups or wireframes, use them — real visuals are better than text descriptions of what something "could look like."
...
### 0D. Focus Areas
Ask the user: "I've rated this plan {N}/10 on design completeness. The biggest gaps are {X, Y, Z}. I'll review all 7 dimensions. Want me to focus on specific areas instead?"

**STOP.** Do NOT proceed until user responds.

## Design Outside Voices (parallel)

Ask the user: "Want an outside design voice before the detailed review? A subagent does an independent completeness review with no prior context."

If the user declines, skip this step and continue.

**Independent design subagent** (via Agent tool):
...
If the subagent fails: "Outside voice unavailable — continuing with primary review."

Present subagent output under a `OUTSIDE DESIGN VOICE:` header.

**Pass integration:**
- Hard rejections → raised as the FIRST items in Pass 1, tagged `[HARD REJECTION]`
- Pre-identified issues → pre-loaded as known issues in the relevant pass
...
If a dimension rates below 7/10, describe specifically what the improved version would look like — what changes, what it accomplishes. If you have tools available to generate a visual, use them to make the gap concrete.
...
```
  ...
  ...
```
Each decision — ask the user individually with recommendation + WHY + alternatives. Edit the plan with each decision as it's made.
...
After all review passes are complete, present a plain summary list of any design items that were deferred: what each item is and why it was deferred. Do not write to any file — just list them in conversation so the user has a clear picture of what remains.
...
```
  ...
  | Deferred items       | ___ items listed                            |
...
After the Completion Summary, recommend the next review(s) based on what this design review discovered.

**Recommend /plan-eng-review** — it's the standard next step after design review. If this review added significant interaction specifications, new user flows, or changed the information architecture, emphasize that eng review needs to validate the architectural implications.

**Consider recommending /plan-ceo-review** — only if this design review revealed fundamental product direction gaps: overall design score started below 4/10, major structural information architecture problems, or questions about whether the right problem is being solved.

**If both are needed, recommend eng review first.**

**Recommend /design-shotgun** if this design review found visual issues that would benefit from exploring new directions.

Ask the user which next step they want. Include only applicable options:
