...
Generates a comprehensive engineering retrospective analyzing commit history, work patterns, and code quality metrics. Team-aware: identifies the user running the command, then analyzes every contributor with per-person praise and growth opportunities. Designed for a senior IC/CTO-level builder using an AI coding agent as a force multiplier.
...
ls -t $M/reports/retros/*.json 2>/dev/null
```
...
today=$(date +%Y-%m-%d)
existing=$(ls $M/reports/retros/${today}-*.json 2>/dev/null | wc -l | tr -d ' ')
next=$((existing + 1))
# Save as $M/reports/retros/${today}-${next}.json
```
...
**Note:** Only include the `test_health` field if test files were found (command 8 returns > 0). If no data, omit the field entirely.
...
### Test Health
- Total test files: N (from command 8)
- Tests added this period: M (from command 10 — test files changed)
- Regression test commits: list `test(qa):` and `test(design):` and `test: coverage` commits from command 9
...
**AI collaboration note:** If many commits have `Co-Authored-By` AI trailers (e.g., various AI coding tools), note the AI-assisted commit percentage as a team metric. Frame it neutrally — "N% of commits were AI-assisted" — without judgment.
...
Ask the user to provide the list of repo paths to include in the global retro, or search for git repos under common directories:

```bash
find ~ -maxdepth 4 -name ".git" -type d 2>/dev/null | grep -v node_modules | sed 's|/.git$||' | head -20
```

Present the list and ask the user which repos to include.
...
Analyze tool usage patterns based on git `Co-Authored-By` trailers and commit patterns across all repos:
- Which AI tool appears most in Co-Authored-By trailers per repo
- Commit patterns by repo (e.g., repos with heavy AI co-authorship vs. solo commits)
- Behavioral patterns (e.g., "AI tool A used exclusively for myapp, tool B for everything else")
...
- Top Work: 3 bullet points summarizing the user's major themes, inferred from
  ...
  not "feat: add-global-discover" + "feat: retro global template".
...
- Tool A: N sessions across M repos — patterns observed
- Tool B: N sessions across M repos — patterns observed
- Tool C: N sessions across M repos — patterns observed
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t $M/reports/retros/global-*.json 2>/dev/null | head -5
```
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
today=$(date +%Y-%m-%d)
existing=$(ls $M/reports/retros/global-${today}-*.json 2>/dev/null | wc -l | tr -d ' ')
next=$((existing + 1))
```

Use the Write tool to save JSON to `$M/reports/retros/global-${today}-${next}.json`:
  ...
  "projects": [
    {
      ...
      "insertions": 3200,
      "deletions": 800,
      "sessions": { "tool_a": 15, "tool_b": 3, "tool_c": 0 }
    }
  ],
  "totals": {
    "commits": 182,
    ...
    "projects": 5,
    "active_days": 6,
    "sessions": { "tool_a": 48, "tool_b": 8, "tool_c": 3 },
    "global_streak_days": 52,
    "avg_context_switches_per_day": 2.1
  },
  "tweetable": "Week of Mar 14: 5 projects, 182 commits, 15.3k LOC | AI sessions: 59 | Focus: my-project (58%) | Streak: 52d"
}
```
...
5. Save only the current-window snapshot to `$M/reports/retros/` (same as a normal retro run); do **not** persist the prior-window metrics.
...
- Output directly to the conversation — do NOT write to filesystem (except the `$M/reports/retros/` JSON snapshot)

## Important Rules

- ALL narrative output goes directly to the user in the conversation. The ONLY file written is the `$M/reports/retros/` JSON snapshot.
- Use `origin/<default>` for all git queries (not local main which may be stale)
- Display all timestamps in the user's local timezone (do not override `TZ`)
- If the window has zero commits, say so and suggest a different window
- Round LOC/hour to nearest 50
- Treat merge commits as PR boundaries
- Do not read project config docs — this skill is self-contained
- On first run (no prior retros), skip comparison sections gracefully
- **Global mode:** Does NOT require being inside a git repo. Saves snapshots to `$M/reports/retros/`. Gracefully skip AI tools that aren't installed. Only compare against prior global retros with the same window value. If streak hits 365d cap, display as "365+ days".
