...
use icn_contracts::models::{
    ...
    ModelFailure, ModelPackage, ModelPackageId, ServableModelBundle, StartModelDownloadRequest,
        ...
        while let Some(event) = stream.next().await {
            let attempt = match event {
                ...
                ModelDownloadEvent::Failed {
                    ...
                    total_bytes,
                    ..
                } if error.code == "cancelled" => DownloadAttempt::Cancelled {
                    id: id.clone(),
                    package_id: package.id.clone(),
                    completed_bytes,
                    total_bytes,
...
fn model_download(
    ...
    let state = if record.cancelled {
        ...
        ModelDownloadState::Failed {
            ...
            total_bytes,
            failure: ModelFailure {
                code: "download_state_missing".to_owned(),
...
mod tests {
    ...
    fn missing_package_attempt_state_fails_instead_of_implying_completion() {
        ...
        assert!(matches!(
            ...
            ModelDownloadState::Failed { failure, .. }
                if failure.code == "download_state_missing"
        ));
    }
