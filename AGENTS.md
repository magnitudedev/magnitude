...
- **daemon-management** — private desktop discovery/control, native child supervision, login registration, and one-time legacy migration. Only privileged host composition roots (CLI bootstrap, desktop main, development server) import it. The desktop bundles and owns ACN; clients never acquire or elect a standalone daemon.
