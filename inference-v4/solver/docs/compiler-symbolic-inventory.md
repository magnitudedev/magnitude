...
`seismic-lang/src/lowered_ir.rs::FoldWindows::new` represents small admissible widths plus an aligned progression. Snapshot-only preparation admits `1..=segment`; packet forms require alignment and tail conditions derived from packet groups. This justifies union-of-progression domains and exact remainder relations, rather than a dense table over every width. The existing compiler remains the authority for constructing the exact legal domain: the solver must not replace packet-specific legality with a guessed divisibility rule.
