...
func check(_ c:[String:Any])->[String:Any] {
    ...
    if family=="compute" || family=="pressure" {
        let chains=num(c,"chains"),a=(c["a"] as! NSNumber).floatValue,b=(c["b"] as! NSNumber).floatValue
        for id in [0,threads/2,threads-1] { var sum:Float=0;for k in 0..<chains { var v=xp[id+k*n];for _ in 0..<(it*8){v=fmaf(v,a,b)};sum+=v }; if family=="pressure" {let tg=num(c,"tg");sum+=xp[id/tg*tg+(id%tg+1)%tg]};compare(yp[id],sum) }
        ...
        let expected=Float(threads/n*it)
        for id in Set([0,n/2,n-1]) { compare(c["op"] as! String=="cas" ? yp[id] : Float(yu[id]),expected) }
    } else if family=="sync" {
        for id in [0,threads/2,threads-1] {
            let name=c["kernel"] as! String
            if name=="barrier" {let tg=num(c,"tg");compare(yp[id],xp[id/tg*tg+(id%tg+it)%tg])}
...
for c in rows {
    ...
    do {
        ...
        repetitions[id]=reps
        log(["kind":"pilot","id":id,"gpu_s":pilot.0,"host_s":pilot.1,"reps":reps,"check":check(c)])
