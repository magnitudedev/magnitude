...
# Reuse each canonical packed word and its coefficients across its codes.
lower matmul[M, N, K](A: tile[M, K] T, B: tile[N, K] q4g32, acc: tile[M, N] f32):
  partials = tile[N] f32
  for n in owned(partials): partials[n] = 0.0
  for w in lanes(K / 8, 1):
    for n in range(N):
      ...
      for c in range(8):
        code = (word >> (4 * c)) & 15
        decoded = fma(f32(code), s, b)
        partials[n] = fma(A[0, w * 8 + c], decoded, partials[n])
  for n in range(N):
    acc[0, n] += simd_sum(partials[n])

# Reuse each canonical packed word and its coefficients across its codes.
lower matmul[M, N, K](A: tile[M, K] T, B: tile[N, K] q8g32a, acc: tile[M, N] f32):
  partials = tile[N] f32
  for n in owned(partials): partials[n] = 0.0
  for w in lanes(K / 4, 1):
    for n in range(N):
      word = B.words[n, w]
      s = B.scale[n, w / 8]
      b = B.bias[n, w / 8]
      for c in range(4):
        code = (word >> (8 * c)) & 255
        decoded = fma(f32(code), s, b)
        partials[n] = fma(A[0, w * 4 + c], decoded, partials[n])
  for n in range(N):
    acc[0, n] += simd_sum(partials[n])

# Reuse each canonical packed word and its coefficients across its codes.
lower matmul[M, N, K](A: tile[M, K] T, B: tile[N, K] q8g16z32, acc: tile[M, N] f32):
  partials = tile[N] f32
  for n in owned(partials): partials[n] = 0.0
  for w in lanes(K / 4, 1):
    for n in range(N):
      ...
      for c in range(4):
        code = (word >> (8 * c)) & 255
        decoded = f32(i32(code) - 32) * s
        partials[n] = fma(A[0, w * 4 + c], decoded, partials[n])
  for n in range(N):
    acc[0, n] += simd_sum(partials[n])

# Source-independent decoded element reads retain this representation's exact
# code interpretation. Output coverage restricts this lane strategy to M == 1.
lower matmul[M, N, K](A: tile[M, K] T, B: tile[N, K] q4g32, acc: tile[M, N] f32):
  partials = tile[N] f32
  for n in owned(partials): partials[n] = 0.0
  for k in lanes(K, 1):
    for n in range(N):
      partials[n] = fma(A[0, k], B[n, k], partials[n])
  for n in range(N):
    acc[0, n] += simd_sum(partials[n])
...
# Source-independent decoded element reads retain this representation's exact
# code interpretation. Output coverage restricts this lane strategy to M == 1.
lower matmul[M, N, K](A: tile[M, K] T, B: tile[N, K] q8g32s, acc: tile[M, N] f32):
  partials = tile[N] f32
  for n in owned(partials): partials[n] = 0.0
  for k in lanes(K, 1):
    for n in range(N):
...
# Source-independent decoded element reads retain this representation's exact
# code interpretation. Output coverage restricts this lane strategy to M == 1.
lower matmul[M, N, K](A: tile[M, K] T, B: tile[N, K] iq4g32, acc: tile[M, N] f32):
  partials = tile[N] f32
  for n in owned(partials): partials[n] = 0.0
  for k in lanes(K, 1):
    for n in range(N):
