...
common_peg_parse_result common_peg_arena::parse(common_peg_parse_context & ctx, size_t start) const {
    ...
    ctx.ast.seal(ctx.retained_nodes, ctx.input);
    return result;
}
    ...
    std::function<std::string(common_peg_parser_id)> to_gbnf = [&](common_peg_parser_id id) -> std::string {
        ...
        return std::visit([&](const auto & p) -> std::string {
            ...
            if constexpr (std::is_same_v<T, common_peg_epsilon_parser> ||
                ...
                // Name nested repetitions so converters cannot flatten their
                // grouping into an ambiguous consecutive quantifier (e.g. +?).
                if (std::holds_alternative<common_peg_repetition_parser>(child_parser)) {
                    child_gbnf = builder.add_rule("repeat-" + std::to_string(id), child_gbnf);
                }
                if (std::holds_alternative<common_peg_choice_parser>(child_parser) ||
                    std::holds_alternative<common_peg_sequence_parser>(child_parser)) {
...
void common_peg_ast_arena::visit_tags(const common_peg_parse_result & result,
                                      const common_peg_ast_visitor & visitor) const {
    for (auto id : result.nodes) { visit_tags(id, visitor); }
}
