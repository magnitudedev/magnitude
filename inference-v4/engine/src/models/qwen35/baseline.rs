...
impl Baseline {
    ...
    fn sample(&mut self, prompt: &[u32], continuation: &[u32]) -> Result<Sample, Error> {
    ...
    pub fn measure(&mut self, prompt: &[u32], continuation: &[u32]) -> Result<Report, Error> {
        if prompt.is_empty()
        ...
        let components = |steps: Vec<super::decoder::DecoderStepObservation>| {
            steps
                ...
                .map(|s| Component {
                    ...
                    block: s.block,
                    entry: s.entry,
                    host_seconds: s.execution.host_seconds,
                })
                .collect::<Vec<_>>()
        };
        let mut state = self.decoder.state_store().create()?;
