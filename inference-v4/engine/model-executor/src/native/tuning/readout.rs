...
//! (`qwen_head_rows`, `qwen_selected_rows`, the MTP head's
...
use magnitude_model_kernels::{
    head_logits_rows, qwen_head_rows, qwen_selected_rows, sample_rows, shape_rows,
};
use seismic::{Element, Tensor};

/// Candidate tokens of a `qwen_selected_rows` tuning point.
...
/// `qwen_head_rows`: final RMS norm and vocabulary projection of the rows
/// `out_rows` selects.
...
impl HeadRowsTuning {
    fn elements(&self) -> qwen_head_rows::Elements {
...
impl EntryTuning for HeadRowsTuning {
    type Entry = qwen_head_rows::Entry;
    ...
    fn args<'a>(case: &'a mut Self::Case) -> qwen_head_rows::Args<'a> {
        qwen_head_rows::Args {
            hidden: &case.hidden,
            norm: &case.norm,
            weight: &case.weight,
    ...
    generated_entry!(qwen_head_rows, this => this.elements());
}

/// `qwen_selected_rows`: final RMS norm and the logits of a candidate token
/// set.
...
impl SelectedRowsTuning {
    fn elements(&self) -> qwen_selected_rows::Elements {
...
impl EntryTuning for SelectedRowsTuning {
    type Entry = qwen_selected_rows::Entry;
    ...
    fn args<'a>(case: &'a mut Self::Case) -> qwen_selected_rows::Args<'a> {
        qwen_selected_rows::Args {
    ...
    generated_entry!(qwen_selected_rows, this => this.elements());
}
...
/// Pseudo-random logits of `rows` rows over the vocabulary.
fn logits(inputs: &TuningInputs<'_, '_>, rows: u64, seed: u64) -> Result<Tensor, String> {
    inputs.activation(
        Element::f32(),
        &[rows, inputs.definition.geometry.vocabulary],
        seed,
    )
}

/// `shape_rows`: penalties, temperature, top-k, min-p and top-p of each
/// selected row. Its parameters partition the vocabulary without changing
/// any result.
...
impl EntryTuning for ShapeRowsTuning {
    ...
    fn statics(&self, inputs: &TuningInputs<'_, '_>) -> Result<Vec<(&'static str, u64)>, String> {
        Ok(vec![
            ("V", inputs.definition.geometry.vocabulary),
            ("Hn", HISTORY_WIDTH as u64),
        ])
    }
    ...
    fn rotation(
        ...
        let rows = point.rows;
        let vocabulary = inputs.definition.geometry.vocabulary;
...
/// `sample_rows`: one draw per selected row over its shaped logits. Its
/// parameters partition the vocabulary without changing any draw.
...
impl EntryTuning for SampleRowsTuning {
    ...
    fn statics(&self, inputs: &TuningInputs<'_, '_>) -> Result<Vec<(&'static str, u64)>, String> {
        Ok(vec![("V", inputs.definition.geometry.vocabulary)])
    }
