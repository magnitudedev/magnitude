...
// visible rows occupy, at Qwen3.5-4B geometry on local Metal. The same number
...
use magnitude_model_kernels::qwen_attention_decode;
...
fn decode_time_by_segment_count() {
    ...
    // The tuned 4B decode configuration (SPAN, PARTS, SIMDS).
    let (span, parts, simds) = (32, 16, 4);
    let kernel = qwen_attention_decode::native_for_device_with(
        &device,
