...
// work item converts ROWS of the B * N rows of the [B, N, K] view with the
...
fn repack_weight<L: Isa, E: Elements>(_l: L, cx: &Context<'_, E>, group: [u64; 3], _shared: &mut [u8]) {
    ...
    let first = group[1] as usize * n + (group[0] * cx.param_rows()) as usize;
    let end = (group[1] as usize + 1) * n;
    let rows = first.min(end)..(first + cx.param_rows() as usize).min(end);
