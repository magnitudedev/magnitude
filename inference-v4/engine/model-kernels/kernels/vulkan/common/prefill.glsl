// The body of the prefill attention entries (`qwen_attention_prefill` over
// dense history, `qwen_attention_prefill_k8v4` over affine K8/V4 history).
// The Vulkan form of `metal/qwen_attention_prefill.metal`: the same three
// launches and partition contract; an entry passes its history buffers as an
// `attention_history` (common/history.glsl), which is all that differs.
//
