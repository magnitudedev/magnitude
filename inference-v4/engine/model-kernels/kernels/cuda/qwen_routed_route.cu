...
// four launches: `qwen_routed_route_stage` (M > 8, one block per row) publishes
// the normalized rows once; `qwen_routed_route_gemv` (M <= 8, one warp per
// logit column, normalizing on the fly and publishing the normalized rows) or
// `qwen_routed_route_gemm` (32 rows x 32 columns x one hidden share of SPLIT
// per block over shared tiles) forms the [M, E + 1] logits, column E being
// the shared-expert gate, as SPLIT partial planes; `qwen_routed_route_select`
...
extern "C" __global__ void qwen_routed_route_stage(SEISMIC_KERNEL_PARAMS) {
...
// normalized rows.
extern "C" __global__ void qwen_routed_route_gemv(SEISMIC_KERNEL_PARAMS) {
    const float *residual = reinterpret_cast<const float *>(SEISMIC_PTR(SEISMIC_BUFFER_RESIDUAL));
    const unsigned char *norm = SEISMIC_PTR(SEISMIC_BUFFER_NORM);
...
extern "C" __global__ void qwen_routed_route_gemm(SEISMIC_KERNEL_PARAMS) {
    const unsigned char *router = SEISMIC_PTR(SEISMIC_BUFFER_ROUTER);
    const float *shared_router = reinterpret_cast<const float *>(SEISMIC_PTR(SEISMIC_BUFFER_SHARED_ROUTER));
...
extern "C" __global__ void qwen_routed_route_select(SEISMIC_KERNEL_PARAMS) {
