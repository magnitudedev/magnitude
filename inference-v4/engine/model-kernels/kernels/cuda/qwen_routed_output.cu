...
#define SJ_W0 SEISMIC_EXPERT_DOWN
#define SJ_W1 SEISMIC_SHARED_DOWN
...
__device__ __forceinline__ unsigned routed_slot(sj::u64 n, sj::u64 first) {
    const sj::u64 local = n - first;
...
struct SelectEpi {
    ...
    sj::u64 first;
    float score;
    __device__ __forceinline__ void operator()(unsigned, sj::u64 n, float value, float) const {
        float &carried = selected[routed_slot(n, first)];
        carried = __fmaf_rn(score, sj::Act::round(value), carried);
    }
};
...
struct FinalEpi {
    ...
    float coefficient;
    __device__ __forceinline__ void operator()(unsigned, sj::u64 n, float value, float) const {
        out[n] = residual[n] + selected[routed_slot(n, first)] + sj::Act::round(value) * coefficient;
    }
};
...
extern "C" __global__ void qwen_routed_output(SEISMIC_KERNEL_PARAMS) {
    ...
    if (group >= sj::gemv_groups<Shape>(SEISMIC_DIM_H))
        return;
    const sj::u64 m = blockIdx.y;
    const sj::u64 first = group * Shape::TPW * 16;
...
#pragma unroll
    ...
    for (sj::u64 k = 0; k < SEISMIC_DIM_K; ++k) {
        ...
        if (Shape::KSPLIT > 1)
            sj::named_barrier(1 + Shape::group(), Shape::KSPLIT * 32);
        const sj::u64 expert = (sj::u64)routes[m * SEISMIC_ROUTES_STRIDE_0 + k * SEISMIC_ROUTES_STRIDE_1];
        const sj::u64 row = routed_expert_row(expert, SEISMIC_DIM_H);
        ...
        sj::gemv_segment<Shape>(shared, pro, 1u, SEISMIC_DIM_F / 64, group, SEISMIC_DIM_H,
                                SJ_W0_AT(SEISMIC_PTR(SEISMIC_BUFFER_EXPERT_DOWN) + row * SJ_CAT(SJ_W0, _ROW_STRIDE_BYTES)),
                                sj::NoWeight{}, epi);
    }
    ...
    sj::gemv_segment<Shape>(shared, pro, 1u, SEISMIC_DIM_S / 64, group, SEISMIC_DIM_H,
                            SJ_W1_AT(SEISMIC_PTR(SEISMIC_BUFFER_SHARED_DOWN)), sj::NoWeight{}, epi);
}
