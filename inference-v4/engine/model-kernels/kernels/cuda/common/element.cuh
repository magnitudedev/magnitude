// Dense element types of the CUDA entries: activations, norm vectors, dense
// weights and state, over the Seismic CUDA prelude's conversions. Every type
// converts one stored element to F32 (`load`) and rounds F32 to the element
// (`store`, `round`, round-to-nearest-even). Tensors are addressed in
// elements of their own dtype from a byte base: `at`, `put`, `put2`, and
// `span` (N contiguous elements into registers).
//
