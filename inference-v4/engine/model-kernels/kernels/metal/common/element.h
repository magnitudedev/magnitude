// Dense element types of the Metal entries: activations, norm vectors, dense
// weights and state. Every type converts one stored element to F32 (`load`),
// rounds F32 to the element (`store`, `round`), and names its storage and
// MSL scalar (`native`, the matrix-operand and vector-load type).
//
// `store` is round-to-nearest-even. For bf16 it is written out on the bits
// (the manual rounding the projection library and the recurrent kernels
// use); a conversion to `native` (`bfloat(x)`) is the compiler's rounding,
// which the attention kernels use. They agree on every finite value; kernels
// keep whichever they use.
//
// The element of a tensor or element parameter is selected from its ABI
// prefix: `ELEMENT_OF(SEISMIC_NORM)`; `element::Act` is the activation
// element `A` when the entry binds one.
