...
#define ROUTED_W2 SEISMIC_SHARED_GATE
#define ROUTED_W3 SEISMIC_SHARED_UP
...
kernel void qwen_routed_expand(
    ...
    if (group.y < choices) {
        if (ulong(tile) * SG * R >= SEISMIC_DIM_F) return;
        ...
        const auto gate = routed::rows_from<routed::W0>(expert_gate, ROUTED_W0_LAYOUT(SEISMIC_DIM_H),
            expert * SEISMIC_DIM_F, SEISMIC_DIM_H);
        const auto up = routed::rows_from<routed::W1>(expert_up, ROUTED_W1_LAYOUT(SEISMIC_DIM_H),
            expert * SEISMIC_DIM_F, SEISMIC_DIM_H);
        const projection::output_paired<A> out{expert_product
            + (m * SEISMIC_RESULT_0_STRIDE_0 + k * SEISMIC_RESULT_0_STRIDE_1) * A::bytes,
            0, SEISMIC_RESULT_0_STRIDE_2};
        projection::gemv_paired<routed::W0, routed::W1, SG, R, 1>(in, out, gate, up, 1,
            SEISMIC_DIM_F, SEISMIC_DIM_H, tile, shared, sg, lane);
        return;
    }

    if (ulong(tile) * SG * R >= SEISMIC_DIM_S) return;
    const auto in = routed::activation(normalized, SEISMIC_NORMALIZED_STRIDE_0, SEISMIC_NORMALIZED_STRIDE_1,
        uint(SEISMIC_DIM_H));
    const auto gate = routed::rows_from<routed::W2>(shared_gate, ROUTED_W2_LAYOUT(SEISMIC_DIM_H), 0,
        SEISMIC_DIM_H);
    const auto up = routed::rows_from<routed::W3>(shared_up, ROUTED_W3_LAYOUT(SEISMIC_DIM_H), 0,
        SEISMIC_DIM_H);
    const projection::output_paired<A> out{shared_product, SEISMIC_RESULT_1_STRIDE_0,
        SEISMIC_RESULT_1_STRIDE_1};
    const uint rows = uint(SEISMIC_DIM_M);
    PROJECTION_FOR_ROWS(rows,
        (projection::gemv_paired<routed::W2, routed::W3, SG, R, MAXM>(in, out, gate, up, rows,
