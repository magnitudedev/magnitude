...
// `shared_product` scratch. L2 (`qwen_routed_combine_down`): the K1 GEMM of
// the shared down projection into the `shared_output` scratch [M, H], in A.
// L3 (`qwen_routed_combine`): the grouped expert outputs of each row
...
kernel void qwen_routed_combine_down(
