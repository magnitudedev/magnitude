...
//   qwen_routed_route_stage (one threadgroup per row): the row's RMS and its
//     normalized values (rounded to the activation dtype), published once.
//   qwen_routed_route_gemv (M <= 8): one simdgroup per logit column reads
...
//   qwen_routed_route_gemm (M > 8): 32 rows x 32 columns per threadgroup on
//     8x8 F32 simdgroup matrices over staged tiles.
//   Logits are [M, E + 1]: column E is the shared-expert gate.
//   qwen_routed_route_select (one simdgroup per row): the softmax over every expert
...
kernel void qwen_routed_route_stage(
...
kernel void qwen_routed_route_gemv(
    device const float *residual [[buffer(SEISMIC_BUFFER_RESIDUAL)]],
    device const uchar *norm [[buffer(SEISMIC_BUFFER_NORM)]],
...
kernel void qwen_routed_route_gemm(
    device const uchar *router [[buffer(SEISMIC_BUFFER_ROUTER)]],
    device const float *shared_router [[buffer(SEISMIC_BUFFER_SHARED_ROUTER)]],
...
kernel void qwen_routed_route_select(
