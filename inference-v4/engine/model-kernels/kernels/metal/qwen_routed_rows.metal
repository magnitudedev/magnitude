...
inline float normalized_at(constant ulong *seismic_words, device const float *residual, device const uchar *norm,
    ...
    return round_activation(residual[offset2(row, source, SEISMIC_RESIDUAL_STRIDE_0, SEISMIC_RESIDUAL_STRIDE_1)]
...
kernel void qwen_routed_rows(
    ...
    float squares = 0.0f;
    for (ulong source = 0; source < SEISMIC_DIM_H; ++source) {
        float value = residual[offset2(row, source, SEISMIC_RESIDUAL_STRIDE_0,
    ...
    for (ulong expert = 0; expert < SEISMIC_DIM_E; ++expert)
        maximum = metal::max(maximum, router_logit(seismic_words, residual, norm, router, row, expert, inverse));
    float total = 0.0f;
    for (ulong expert = 0; expert < SEISMIC_DIM_E; ++expert)
        total += metal::exp(router_logit(seismic_words, residual, norm, router, row, expert, inverse) - maximum);
    ...
    for (ulong rank = 0; rank < SEISMIC_DIM_K; ++rank) {
        ...
        int winner = -1;
        for (ulong expert = 0; expert < SEISMIC_DIM_E; ++expert) {
            float probability = metal::exp(router_logit(seismic_words, residual, norm, router, row, expert, inverse) - maximum) / total;
    ...
    for (ulong choice = 0; choice < SEISMIC_DIM_K; ++choice) {
        ...
        for (ulong feature = 0; feature < SEISMIC_DIM_F; ++feature) {
            ...
            float up = 0.0f;
            for (ulong source = 0; source < SEISMIC_DIM_H; ++source) {
                float input = normalized_at(seismic_words, residual, norm, row, source, inverse);
        ...
        if (choice + 1 < SEISMIC_DIM_K) {
            ...
            for (ulong candidate = 0; candidate < SEISMIC_DIM_E; ++candidate) {
                float probability = metal::exp(router_logit(seismic_words, residual, norm, router, row, candidate, inverse) - maximum) / total;
    ...
    for (ulong source = 0; source < SEISMIC_DIM_H; ++source)
        shared_dot = metal::fma(normalized_at(seismic_words, residual, norm, row, source, inverse),
