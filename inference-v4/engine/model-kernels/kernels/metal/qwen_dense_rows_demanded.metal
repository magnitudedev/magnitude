...
#else
#error "qwen_dense_rows_demanded requires a dense norm representation"
#endif
}
inline float load_gate(device const uchar *base, ulong logical) {
...
#else
#error "qwen_dense_rows_demanded requires dense gate weights"
#endif
}
inline float load_up(device const uchar *base, ulong logical) {
...
#else
#error "qwen_dense_rows_demanded requires dense up weights"
#endif
}
inline float load_down(device const uchar *base, ulong logical) {
...
#else
#error "qwen_dense_rows_demanded requires dense down weights"
#endif
}
...
#else
#error "qwen_dense_rows_demanded requires a dense activation representation"
#endif
}
inline float normalized_at(device const float *residual, device const uchar *norm,
    constant ulong *seismic_words, ulong source_row, ulong column, float inverse) {
    return round_activation(
...
inline float product_at(device const float *residual, device const uchar *norm,
    device const uchar *gate_weight, device const uchar *up_weight,
    constant ulong *seismic_words, ulong source_row, ulong feature, float inverse) {
    float gate = 0.0f;
    float up = 0.0f;
    for (ulong source = 0; source < SEISMIC_DIM_H; ++source) {
        ...
        gate += input * load_gate(gate_weight, feature * SEISMIC_DIM_H + source);
        up += input * load_up(up_weight, feature * SEISMIC_DIM_H + source);
    }
...
kernel void qwen_dense_rows_demanded(
    ...
    float squares = 0.0f;
    for (ulong source = 0; source < SEISMIC_DIM_H; ++source) {
        float value = residual[offset2(source_row, source, SEISMIC_RESIDUAL_STRIDE_0, SEISMIC_RESIDUAL_STRIDE_1)];
        squares += value * value;
    }
    ...
    for (ulong feature = 0; feature < SEISMIC_DIM_F; ++feature) {
        projected += product_at(residual, norm, gate_weight, up_weight, seismic_words, source_row, feature, inverse)
            * load_down(down_weight, column * SEISMIC_DIM_F + feature);
    }
    projected = round_activation(projected);
    result[offset2(row, column, SEISMIC_RESULT_0_STRIDE_0, SEISMIC_RESULT_0_STRIDE_1)] =
        residual[offset2(source_row, column, SEISMIC_RESIDUAL_STRIDE_0, SEISMIC_RESIDUAL_STRIDE_1)] + projected;
}
