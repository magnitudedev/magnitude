...
// The three launches over dense history (bodies in common/attention.h).

kernel void qwen_attention_prefill_prepare(
...
kernel void qwen_attention_prefill_attend(
    device const attention::Scalar *query_gate [[buffer(SEISMIC_BUFFER_QUERY_GATE)]],
...
kernel void qwen_attention_prefill_merge(
    device const attention::Scalar *query_gate [[buffer(SEISMIC_BUFFER_QUERY_GATE)]],
    device attention::Scalar *gated [[buffer(SEISMIC_RESULT_0_BUFFER)]],
