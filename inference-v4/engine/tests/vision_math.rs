...
fn check_vision_reference(entry: &str, fixture: &str) {
    ...
    let dimensions: HashMap<String, i64> = reference["dimensions"]
        ...
        .collect();
    let program = seismic_engine::models::qwen35::program::program().unwrap();
    let function = program.functions.iter().find(|f| f.name == entry).unwrap();
    let mut vm = Interpreter::new(&program);
        ...
        match ty {
            Ty::Tensor(t) => {
                let shape: Vec<usize> = t
                    .shape
                    .iter()
                    .map(|d| d.eval(&|n| dimensions.get(n).copied()).unwrap() as usize)
                    .collect();
    ...
    vm.run(entry, &args, &dimensions).unwrap();
...
fn erf_gelu_matches_python_oracle_at_boundaries_small_values_and_tails() {
    ...
    sources.push(SourceFile {
        ...
        text: "fn erf_test[N](x: tensor[1, N] f32, out: tensor[1, N] f32):\n  for row in parallel:\n    source = load(x[row])\n    result = tile[N] f32\n    erf_values(source, result)\n    store(result, out[row])\n".into(),
    ...
    vm.run("erf_test", &[Arg::Tensor(x), Arg::Tensor(erf)], &shape)
        .unwrap();
    vm.run("gelu", &[Arg::Tensor(x), Arg::Tensor(gelu)], &shape)
        .unwrap();
