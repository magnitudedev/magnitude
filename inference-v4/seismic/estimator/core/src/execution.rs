...
use seismic_ir::target::{KernelDialect, KernelEmissionLayout};
...
/// Trusted vocabulary witness. Production implementations are generated by
/// `analytical_services!`, which emits the enum and `ALL` from one variant
/// list. This trait is an implementation detail of that macro.
#[doc(hidden)]
pub trait MacroGeneratedAnalyticalService: AnalyticalService {}
...
macro_rules! analytical_services {
    ($vis:vis enum $name:ident { $($variant:ident => $stable_name:literal),+ $(,)? }) => {
        ...
        impl $crate::MacroGeneratedAnalyticalService for $name {}
    };
}

pub trait AnalyticalModelDefinition<T: KernelDialect>: Send + Sync + 'static {
    type Service: MacroGeneratedAnalyticalService + PartialEq;
...
pub trait ExecutionModel<B: KernelDialect>: ServiceModel {
    ...
    fn operation_cost(
        ...
        locals: &LaunchLocalLayout,
        op: ClosedOpView<'_, B>,
    ) -> OperationCost<ServiceClassId>;
}

pub fn estimate<B: KernelDialect, M: ExecutionModel<B> + ?Sized>(
    profile: &M,
    arena: &mut ExprArena,
...
mod tests {
    ...
    use seismic_ir::schedule::LaunchMode;
    ...
    impl ExecutionModel<Dialect> for Model {
        ...
        fn operation_cost(
            ...
            OperationCost::Elided(ProvenElision::CompileTimeOnly)
        }
    }
    ...
    fn real_builders_and_prediction_require_no_native_backend() {
        ...
        for trips in 0..8 {
            ...
            let executable = ir.close_execution(&mut arena);
    ...
    fn guarded_repeat_is_total_when_an_operation_is_proven_elided() {
        ...
        let ir = planned.finish();
        let executable = ir.close_execution(&mut arena);
        let assessment = estimate(&model, &mut arena, executable.view());

        let mut complete_assignment = Assignment::new();
        complete_assignment.bind(branch_symbol, seismic_lang::expr::SymbolValue::Nat(0));
        ...
        incomplete_assignment.bind(branch_symbol, seismic_lang::expr::SymbolValue::Nat(1));
        let total = arena
    ...
    fn explicit_operation_elision_still_produces_a_total_model() {
        ...
        let ir = planned.finish();
        let executable = ir.close_execution(&mut arena);
        let assessment = estimate(&Model(definition), &mut arena, executable.view());
        let duration = arena
