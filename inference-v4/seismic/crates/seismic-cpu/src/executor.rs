...
use seismic_compiler::schedule::FillValue;
...
pub struct Executor {
    ...
    contract: Arc<DeviceContract<Cpu>>,
    execution: Arc<ExecutionProfile<Cpu>>,
}

impl Executor {
    /// Opens the production worker pool first, then acquires the complete
    /// profile from the immutable facts of that exact pool.
    pub fn host() -> Result<Self, TargetError> {
        let mut workers =
            Workers::host().map_err(|error| TargetError::DeviceUnavailable(error.to_string()))?;
        let (contract, execution) = crate::profile::profile_for_workers(&mut workers)?;
        Ok(Self {
            device: Device,
    ...
    pub fn new(mut workers: Workers) -> Result<Self, TargetError> {
        let (contract, execution) = crate::profile::profile_for_workers(&mut workers)?;
        Ok(Self {
            device: Device,
            workers: Arc::new(Mutex::new(workers)),
    ...
    pub fn contract(&self) -> &Arc<DeviceContract<Cpu>> {
        &self.contract
    }

    pub fn execution_profile(&self) -> &Arc<ExecutionProfile<Cpu>> {
        &self.execution
    }
}
...
impl NativeSubmission<Cpu> for Submission {
    ...
    fn execute(
        ...
        env: &mut ExecutionEnvironment<'_, Cpu, Device>,
...
fn view_range(
    ...
    bytes: u64,
    env: &ExecutionEnvironment<'_, Cpu, Device>,
) -> *mut u8 {
