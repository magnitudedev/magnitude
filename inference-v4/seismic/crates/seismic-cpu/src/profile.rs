...
use seismic_compiler::target::{
    CompatibilityIdentity, DataTypeSupport, DeviceContract, DeviceContractParts, ExecutionProfile,
    ExecutionProfileParts, KernelAbiFootprint, KernelAbiLayout, KernelAbiModel, LocalRealization,
    LocalRealizationPolicy, NumericalEnvironment, ProfileAcquisitionMetrics, TargetLimits,
    VectorOperationClass, VectorSupport, VectorSupportEntry,
};
...
) -> Result<(DeviceContract<Cpu>, ExecutionProfile<Cpu>), TargetError> {
    let worker_count = u32::try_from(workers.count())
        .map_err(|_| TargetError::UnsupportedDevice("CPU worker count exceeds u32::MAX".into()))?;
    let (device_parts, execution_parts) = discover_host(worker_count, workers)?;
    let device = DeviceContract::assemble(device_parts, registry())?;
    let execution = ExecutionProfile::assemble(&device, execution_parts)?;
    Ok((device, execution))
}

fn discover_host(
    workers: u32,
    pool: &mut crate::workers::Workers,
) -> Result<(DeviceContractParts<Cpu>, ExecutionProfileParts), TargetError> {
    ...
    // CPU probes are fixed native Rust routines; there is no generated probe
    // artifact or compilation phase between opening the pool and execution.
    let probe_build_ns = 0;
    let probe_started = Instant::now();
    let services = services::probe_services(pool)?;
    let composition_qualification = services::qualify_compositions(pool)?;
    ...
    Ok((
        ...
        ExecutionProfileParts {
            probe_suite_revision: services::probe_suite_revision(),
            services,
...
fn vector_support(facts: &HostFacts) -> VectorSupport {
    use seismic_compiler::kernel::ops::{BinaryOp, BitOp, UnaryOp};
