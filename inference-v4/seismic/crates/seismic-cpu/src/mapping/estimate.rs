...
impl EstimateModel {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if positive && self.workers > 0 {
            Ok(())
        } else {
            Err("CPU estimate coefficients must be positive and finite".into())
        }
    }
    ...
    pub fn scope_ns(&self, phases: u64, totals: &Totals, pieces: u64) -> Result<u64, String> {
        ...
        let claimed = if phases > 0 {
            pieces as f64 * self.piece_ns / usable
        } else {
            0.0
        };
        ...
        if span.is_finite() && span < u64::MAX as f64 {
            Ok(span.ceil() as u64)
        } else {
            Err("CPU estimate exceeds the representable range".into())
        }
    }
    ...
    pub fn materialization_ns(&self, bits: u64) -> Result<u64, String> {
        self.scope_ns(
            ...
            &Totals {
                ...
                memory_bits: bits
                    .checked_mul(2)
                    .ok_or("CPU estimate traffic overflows u64")?,
            },
            1,
        )
    }
