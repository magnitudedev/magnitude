...
const FLAGS: &[&str] = &["--fn", "--shape", "--element", "--numerics", "--target", "--strategy"];
...
fn metal() -> Result<(Metal, String), String> {
    ...
    if let Ok(device) = seismic_metal::runtime::Device::open() {
        ...
        return Ok((Metal::from_device(&info).map_err(|e| e.to_string())?, origin));
    }
    let limits = Limits {
...
fn cpu() -> Result<(Cpu, String), String> {
    let workers = std::thread::available_parallelism().map_err(|e| format!("host parallelism: {e}"))?.get() as u64;
    let backend = Cpu::host(workers).map_err(|e| e.to_string())?;
    let origin = format!("host ({} workers, {} scratch bytes per worker, unqualified estimate coefficients)", backend.limits().workers, backend.limits().max_scratch_bytes);
    Ok((backend, origin))
}
...
fn cuda() -> Result<(Cuda, String), String> {
    if let Ok(device) = seismic_cuda::Device::open(0) {
        let origin = format!("device `{}` (max_threads_per_block={}, max_grid_x={}, warp_size={})", device.info.name, device.info.max_threads_per_block, device.info.max_grid_x, device.info.warp_size);
        return Ok((Cuda::from_device(&device.info).map_err(|e| e.to_string())?, origin));
    }
    let backend = Cuda::new(seismic_cuda::mapping::Limits::gb10(), seismic_cuda::mapping::EstimateModel::default()).map_err(|e| e.to_string())?;
    Ok((backend, "documented GB10 limits, no CUDA device (unqualified estimate coefficients)".into()))
}

fn run<B: Backend>(o: &Options, program: &Program, backend: &B) -> Result<Selected<B::Execution>, String> {
    selection::select(program, o.entry()?, &o.workload, backend, Budget { strategy: o.strategy, ..Budget::default() }).map_err(|e| e.to_string())
}

/// The selection report of `backend`: the same finite site domains selection searched over.
fn selected_report<B: Backend>(o: &Options, program: &Program, backend: &B, capacities: &str) -> Result<String, String> {
    let selected = run(o, program, backend)?;
    let domains = backend.bind_structure(program, &selected.family).map_err(|e| e.to_string())?;
...
pub fn emit(args: &[String]) -> Result<(), String> {
    ...
    let text = match o.target.as_str() {
        ...
        "cuda" => run(&o, &program, &cuda()?.0)?.execution.ptx().iter().enumerate().map(|(launch, text)| format!("; launch {launch}\n{text}\n")).collect(),
...
pub fn analyze_search(args: &[String]) -> Result<(), String> {
    ...
    println!("search structure of `{}` on {target} for {} (counts, not a time prediction)", family.entry, workload(&family));
...
fn workload(family: &Family) -> String {
    let shapes = family.workload.shapes.iter().map(|(k, v)| format!("{k}={v}"));
    let elems = family.workload.elems.iter().map(|(k, v)| format!("{k}={v}"));
...
fn definition(program: &Program, id: DefId) -> String {
    ...
    let kind = match &d.kind {
        DefKind::Body => "fn".to_string(),
        DefKind::Contract => "contract".to_string(),
        DefKind::Lower { target } => format!("lower for {target}"),
        DefKind::Adopt { target } => format!("lower for {target} = portable"),
    };
...
fn domain(values: &[i64]) -> String {
    ...
    match values {
        ...
        [head @ .., last] => format!("{{{},…,{last}}} ({} values)", list(&head[..8]), values.len()),
        [] => "{}".into(),
    }
}

fn choice(witness: &Witness, occurrence: OccurrenceId) -> String {
    witness.choices.get(&occurrence).map_or("inactive".into(), |c| format!("candidate {c}"))
}

fn cover(cover: Option<&Vec<(u32, u32)>>) -> String {
    cover.map_or("inactive".into(), |c| c.iter().map(|(s, e)| format!("[{s},{e})")).collect::<Vec<_>>().join(" "))
}
...
fn report<E>(program: &Program, selected: &Selected<E>, domains: &BTreeMap<family::SiteId, Vec<i64>>, capacities: &str) -> String {
    macro_rules! say {
        ($out:expr, $($format:tt)*) => { $out.push(format!($($format)*)) };
    }
    let (family, witness, seed) = (&*selected.family, &selected.witness, &selected.seed);
    let mut out: Vec<String> = Vec::new();
    say!(out,"entry `{}` on {} for {}", family.entry, family.target, workload(family));
    say!(out,"backend capacities: {capacities}");
    say!(out,"\noccurrences:");
    for occurrence in &family.occurrences {
        let origin = match occurrence.parent {
            None => "entry".to_string(),
            Some(parent) => format!("call {} in {}", occurrence.call.map_or("?".into(), |c| c.0.to_string()), candidate_ref(parent)),
        };
        say!(
            out,
            "  occurrence {} `{}` ({origin}): selected {}, seed {}",
            occurrence.id.0, program.families[occurrence.family].name, choice(witness, occurrence.id), choice(seed, occurrence.id)
        );
        for (ordinal, candidate) in occurrence.candidates.iter().enumerate() {
            let mark = if witness.choices.get(&occurrence.id) == Some(&(ordinal as u32)) { '*' } else { ' ' };
            let template = family.template(candidate.template);
            let bound = template.shapes.iter().map(|(k, v)| format!("{k}={v}"))
                .chain(template.elems.iter().map(|(k, v)| format!("{k}={v}")))
                .chain(candidate.structural.iter().map(|(k, s)| format!("{k}=site {}", s.0 .0)))
                .collect::<Vec<_>>().join(",");
            say!(
                out,
                "  {mark} candidate {ordinal}: {} via {}, template {} [{bound}]",
                definition(program, template.definition), definition(program, candidate.via), candidate.template.0
            );
            if !candidate.requirements.is_empty() {
                say!(out,"      requires {}", candidate.requirements.iter().map(requirement).collect::<Vec<_>>().join(", "));
            }
        }
        if occurrence.candidates.is_empty() {
            say!(out,"    no applicable implementation: missing coverage on this path");
        }
        for (rejected, reason) in &occurrence.rejected {
            say!(out,"    rejected {}: {reason}", definition(program, *rejected));
        }
    }
    say!(out,"\nnumerical sites:");
    for site in &family.sites {
        let kind = match &site.kind {
            SiteKind::Width { region, slice } => format!("width of slice {} in region {}", slice.0, region.0),
            SiteKind::Parts { region, slice } => format!("parts of slice {} in region {}", slice.0, region.0),
        };
        let value = |w: &Witness| w.sites.get(&site.id).map_or("inactive".into(), i64::to_string);
        say!(
            out,
            "  site {} ({kind}, extent {}, owner {}): domain {}, selected {}, seed {}",
            site.id.0, site.extent, candidate_ref(site.owner),
            domains.get(&site.id).map_or("unbound".into(), |d| domain(d)), value(witness), value(seed)
        );
    }
    say!(out,"\nsequences:");
    for sequence in &family.sequences {
        let units = sequence.units.iter().map(|u| {
        ...
        say!(out,"  sequence {} (owner {}, scope {:?}): units [{units}]", sequence.id.0, candidate_ref(sequence.owner), sequence.scope);
        say!(out,"    cover selected {}, seed {}", cover(witness.covers.get(&sequence.id)), cover(seed.covers.get(&sequence.id)));
    }
    say!(out,"\nestimate model: {} (estimates, not measurements)", selected.estimate_model);
    ...
    say!(out,"strategy: {:?} over {} variables, {} factors", s.strategy, s.variables, s.factors);
    match s.strategy {
        Strategy::Greedy => say!(out,"  greedy: {} sweeps, {} trial witnesses", s.greedy_sweeps, s.greedy_trials),
        Strategy::Exact => {
            say!(out,"  exact phase: {:.3} ms, work {}, nodes {}", ms(s.exact.time), s.exact.work, s.exact.nodes);
            match s.neighborhood {
                Some(n) => say!(out,"  neighborhood phase: {:.3} ms, work {}, nodes {}", ms(n.time), n.work, n.nodes),
    ...
    if family.obligations.is_empty() && selected.unresolved.is_empty() {
        ...
        for o in &family.obligations {
            say!(out,"  occurrence {} {}: {}", o.occurrence.0, definition(program, o.definition), o.reason);
        }
