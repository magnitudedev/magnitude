...
/// A violated estimate-coefficient or range invariant, typed. Ranking
/// only: an estimate never decides legality, and the boundary that
/// consumes it (catalog construction, cost ranking) maps each variant
/// into its own failure class.
#[derive(Clone, Debug, PartialEq)]
pub enum EstimateError {
    /// A coefficient is not finite and positive (with at least one
    /// concurrent subgroup and a nonzero threadgroup pool).
    Coefficient,
    /// An evaluated estimate lies outside the representable nanosecond
    /// range.
    Range(f64),
}
...
impl std::fmt::Display for EstimateError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            EstimateError::Coefficient => f.write_str(
                "estimate coefficients must be finite and positive, with at least one \
                 concurrent subgroup",
            ),
            EstimateError::Range(value) => {
                write!(f, "estimate {value} ns is outside the representable range")
            }
        }
    }
}
...
impl EstimateModel {
    /// The probe-calibrated constant model, valid by construction (every
    /// coefficient finite and positive, at least one concurrent subgroup,
    /// a nonzero threadgroup pool). The catalog consumes exactly this
    /// constructor, so no validation failure path exists in production.
...
impl EstimateModel {
    ...
    pub fn validate(&self) -> Result<(), EstimateError> {
        ...
        {
            return Err(EstimateError::Coefficient);
        }
        Ok(())
    }

    fn nanoseconds(value: f64) -> Result<u64, EstimateError> {
        if !value.is_finite() || value < 0.0 || value >= u64::MAX as f64 {
            return Err(EstimateError::Range(value));
        }
    ...
    pub fn materialization_ns(&self, bits: u64) -> Result<u64, EstimateError> {
