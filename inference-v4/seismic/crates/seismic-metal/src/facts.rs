...
//!
//! Nothing here is discovered later. The facts the old native assembler
//! reflected after pipeline creation (`threadExecutionWidth`,
//! `maxTotalThreadsPerThreadgroup`) are established here: the SIMD width is
//! probed from a pipeline while opening the production service, and every rendered kernel carries
//! `[[max_total_threads_per_threadgroup(N)]]` with `N` equal to the device
//! limit, so a pipeline can never expose a smaller resident-thread limit
//! than the profile advertised.
...
pub struct MetalFacts {
    /// The device itself: it is the native toolchain (`newLibraryWithSource`)
    /// and the allocator, so the native compiler reaches it through the
    /// profile. Compared by registry identity.
