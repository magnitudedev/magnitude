...
pub(crate) const INTEGER: ServiceClassId = ServiceClassId::new("metal.integer");
pub(crate) const F32_ADD_SUB: ServiceClassId = ServiceClassId::new("metal.f32.add-sub");
pub(crate) const F32_MULTIPLY: ServiceClassId = ServiceClassId::new("metal.f32.multiply");
pub(crate) const F32_DIVIDE: ServiceClassId = ServiceClassId::new("metal.f32.divide");
pub(crate) const F32_REMAINDER: ServiceClassId = ServiceClassId::new("metal.f32.remainder");
pub(crate) const F32_MIN_MAX: ServiceClassId = ServiceClassId::new("metal.f32.min-max");
pub(crate) const F32_FMA: ServiceClassId = ServiceClassId::new("metal.f32.fma");
pub(crate) const F32_COMPARE: ServiceClassId = ServiceClassId::new("metal.f32.compare");
pub(crate) const F32_TO_INTEGER: ServiceClassId = ServiceClassId::new("metal.f32.to-integer");
pub(crate) const INTEGER_TO_F32: ServiceClassId = ServiceClassId::new("metal.integer.to-f32");
pub(crate) const F32_TO_F16: ServiceClassId = ServiceClassId::new("metal.f32.to-f16");
pub(crate) const F16_TO_F32: ServiceClassId = ServiceClassId::new("metal.f16.to-f32");
pub(crate) const F32_TO_BF16: ServiceClassId = ServiceClassId::new("metal.f32.to-bf16");
pub(crate) const F16_STRICT: ServiceClassId = ServiceClassId::new("metal.f16.strict");
pub(crate) const BF16_STRICT: ServiceClassId = ServiceClassId::new("metal.bf16.strict");
pub(crate) const APPROXIMATE_MATH: ServiceClassId = ServiceClassId::new("metal.approximate-math");
pub(crate) const GLOBAL_MEMORY: ServiceClassId = ServiceClassId::new("metal.global-memory");
pub(crate) const WORKGROUP_MEMORY: ServiceClassId = ServiceClassId::new("metal.workgroup-memory");
pub(crate) const REPRESENTATION: ServiceClassId = ServiceClassId::new("metal.representation");
pub(crate) const ATOMIC: ServiceClassId = ServiceClassId::new("metal.atomic");
pub(crate) const BARRIER: ServiceClassId = ServiceClassId::new("metal.barrier");
pub(crate) const SUBGROUP: ServiceClassId = ServiceClassId::new("metal.subgroup");
pub(crate) const MATRIX: ServiceClassId = ServiceClassId::new("metal.simdgroup-matrix");

pub(crate) fn required_service_classes(
    facts: &crate::facts::MetalFacts,
    supported_intrinsics: &BTreeSet<seismic_lang::ids::IntrinsicId>,
) -> BTreeSet<ServiceClassId> {
    let mut classes: BTreeSet<_> = [
        CONTROL,
        INTEGER,
        F32_ADD_SUB,
        F32_MULTIPLY,
    ...
    if facts.bfloat_arithmetic {
        classes.insert(BF16_STRICT);
        classes.insert(F32_TO_BF16);
    }
    ...
    if supported("subgroup") {
        classes.insert(SUBGROUP);
    }
    if supported("matrix") {
        classes.insert(MATRIX);
    }
    classes
}
...
fn one(arena: &mut ExprArena, class: ServiceClassId, mode: DemandMode) -> ExecutionDemand {
...
fn emission_class(family: ScalarEmissionFamily) -> ServiceClassId {
    match family {
...
fn memory_class(kind: ClosedPlaceKind) -> ServiceClassId {
    match kind {
...
pub(crate) fn execution_demand(
    _target: &DeviceContract<Metal>,
    arena: &mut ExprArena,
    _kernel: &seismic_compiler::kernel::Kernel<Metal>,
    ...
    match op {
        ClosedOpView::Constant { .. } => vec![one(arena, CONTROL, Latency)],
        ClosedOpView::Unary { out, .. } | ClosedOpView::Bitcast { out, .. } => {
            ...
            vec![demand(class, lanes(arena, out.ty), Latency)]
        }
        ...
        ClosedOpView::VectorSplat { out, .. } | ClosedOpView::VectorLane { out, .. } => {
            vec![demand(CONTROL, lanes(arena, out.ty), Latency)]
        }
        ...
        ClosedOpView::Cmp { a, .. } => {
            ...
            vec![demand(class, lanes(arena, a.ty), Latency)]
        }
        ClosedOpView::Select { .. } => vec![one(arena, CONTROL, Latency)],
        ClosedOpView::Logic { .. } | ClosedOpView::Not { .. } => {
            vec![one(arena, INTEGER, Latency)]
        }
        ...
        | ClosedOpView::Branch { .. }
        | ClosedOpView::Repeat { .. }
        | ClosedOpView::Yield { .. } => vec![one(arena, CONTROL, Latency)],
        ClosedOpView::Read { place, .. } => {
            let mut result = vec![one(arena, memory_class(place.kind), Capacity)];
            if matches!(
                place.geometry,
        ...
        ClosedOpView::ReadPlane { place, .. } => vec![
        ...
        ClosedOpView::VectorRead { out, place, .. } => {
            ...
            let mut result = vec![demand(memory_class(place.kind), units, Capacity)];
            if matches!(
                place.geometry,
                seismic_compiler::target::ReadableRepresentationGeometry::Packed(_)
            ) {
        ...
        ClosedOpView::Write { place, .. } => {
            vec![one(arena, memory_class(place.kind), Capacity)]
        }
        ...
        ClosedOpView::Atomic { .. } => vec![one(arena, ATOMIC, Capacity)],
        ClosedOpView::Barrier(_) => vec![one(arena, BARRIER, Latency)],
        ClosedOpView::Intrinsic { op, .. } => match op {
            MetalIntrinsic::LaneIndex
            | MetalIntrinsic::Shuffle { .. }
            | MetalIntrinsic::SubgroupReduce { .. } => vec![one(arena, SUBGROUP, Latency)],
...
pub(crate) fn acquire(
    ...
    let composition_pipeline = facts
        .device
        .raw()
...
fn qualify_composition(
    ...
    // must exercise the same dependency-latency demand that `execution_demand`
