...
//! into `DeviceContract<Metal>`, while fixed probe observations on the
...
use seismic_compiler::target::{
    CapabilityRegistry, CompatibilityIdentity, DataTypeSupport, DeviceContract,
    DeviceContractParts, ExecutionProfile, ExecutionProfileParts, KernelAbiAllocation,
    KernelAbiAllocationRole, KernelAbiFootprint, KernelAbiLayout, KernelAbiModel, LocalRealization,
...
pub fn registry() -> &'static CapabilityRegistry<Metal> {
    static REGISTRY: OnceLock<CapabilityRegistry<Metal>> = OnceLock::new();
    REGISTRY.get_or_init(|| {
        CapabilityRegistry::assemble(
            crate::intrinsic::registrations(),
            crate::factories::structural_factories(),
        )
    })
}

/// Acquires the complete target profile from the already-open production
/// device service. The service's queue exists before any profile query or
/// probe and is the queue later used by the executor.
pub fn open(
    device: &MetalDevice,
) -> Result<(DeviceContract<Metal>, ExecutionProfile<Metal>), TargetError> {
    let (device_parts, execution_parts) = discover(device)?;
    let contract = DeviceContract::assemble(device_parts, registry())?;
    let execution = ExecutionProfile::assemble(&contract, execution_parts)?;
    Ok((contract, execution))
}

/// Gathers every Metal target fact of one device.
fn discover(
    device: &MetalDevice,
) -> Result<(DeviceContractParts<Metal>, ExecutionProfileParts), TargetError> {
    ...
    let supported_intrinsics = crate::intrinsic::subgroup_supported(&facts, &limits, &dtypes)
        .into_iter()
        .chain(crate::intrinsic::matrix_supported(&facts, &limits, &dtypes))
        .collect();
    let required_services =
        crate::services::required_service_classes(&facts, &supported_intrinsics);
    let identity_and_limits_ns = total_started.elapsed().as_nanos() as u64;
    let service_acquisition = crate::services::acquire(device, &facts, &required_services)?;
    ...
    Ok((
        ...
        ExecutionProfileParts {
            probe_suite_revision: PROBE_SUITE_REVISION,
            services: service_acquisition.services,
            acquisition,
            composition_qualification: service_acquisition.composition,
        },
    ))
}
...
impl KernelAbiModel<Metal> for MetalKernelAbi {
    fn layout(&self, kernel: &seismic_compiler::kernel::Kernel<Metal>) -> KernelAbiLayout {
        let interface = kernel.interface();
        let entries = interface.bindings.len() as u64 + u64::from(RESERVED_ARGUMENT_ENTRIES);
        let word_bytes =
            u64::from(seismic_compiler::target::KernelWordLayout::for_kernel(kernel).total) * 8;
