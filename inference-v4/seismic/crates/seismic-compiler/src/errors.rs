...
impl fmt::Display for NativeCompilationError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::ToolchainFailure(s) => write!(f, "toolchain failure: {s}"),
            Self::MalformedToolchainOutput(s) => write!(f, "malformed toolchain output: {s}"),
            Self::DeviceLost(s) => write!(f, "device lost: {s}"),
            Self::CacheFailure(s) => write!(f, "cache failure: {s}"),
            Self::ToolchainResourceExhausted(s) => write!(f, "toolchain resources exhausted: {s}"),
        }
    }
}
