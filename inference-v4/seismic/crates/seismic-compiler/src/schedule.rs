...
impl<T: ScalarType> ScalarSlotId<T> {
    ...
    pub(crate) fn from_any(slot: AnyScalarSlot) -> Self {
...
pub struct AnyScalarSlot {
    ...
    pub dtype: DType,
    pub symbol: SymbolId,
    pub sort: seismic_lang::expr::SymbolSort,
}
impl AnyScalarSlot {
...
impl ParametricSchedule {
    ...
    pub(crate) fn direct_view_uses(&self) -> &[(AnyBufferView, ScheduleUse)] {
        &self.direct_view_uses
    }
    pub(crate) fn launch_uses(&self) -> &[(LaunchId, ScheduleUse)] {
        &self.launch_uses
    }
...
pub(crate) struct ScheduleConstruction<B: Backend> {
    owner: OwnerToken,
    launches: Vec<Launch>,
    slots: Vec<AnyScalarSlot>,
    regions: Vec<Region>,
...
impl<B: Backend> ScheduleConstruction<B> {
    ...
    pub(crate) fn begin_branch(&mut self, parent: u32, condition: BoolExpr) -> (u32, u32) {
        assert!(!self.closed, "a closed schedule cannot be extended");
...
pub struct ScheduleBuilder<'a, B: Backend> {
    inner: internals::Builder<'a, B>,
}
impl<'a, B: Backend> ScheduleBuilder<'a, B> {
    ...
    pub(crate) fn temporary_bool(&mut self) -> AnyScalarSlot {
    ...
    pub(crate) fn slot_symbol_any(&mut self, slot: AnyScalarSlot) -> SymbolId {
        self.inner.slot_symbol(slot, slot.sort)
    }
    pub(crate) fn copy_any(
        &mut self,
    ...
    pub(crate) fn fill_constant_any(
        &mut self,
        destination: AnyBufferView,
    ...
    pub(crate) fn launch_sequential(&mut self, kernel: KernelId) -> ScheduleUse {
...
pub struct ChoiceBuilder<'a, B: Backend> {
    inner: internals::Choice<'a, B>,
}
impl<'a, B: Backend> ChoiceBuilder<'a, B> {
...
mod internals {
    ...
    pub(super) struct Choice<'a, B: Backend> {
        arena: &'a mut ExprArena,
        views: &'a [crate::storage::BufferViewLayout],
