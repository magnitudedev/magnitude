...
pub fn domain(site: &Site, mentioned: &[Requirement], owner: &[Requirement]) -> Vec<i64> {
    ...
    match site.kind {
        SiteKind::Width { .. } => {
            let units: BTreeSet<i64> = mentioned
                .iter()
                .filter_map(|r| match r {
                    Requirement::Multiple { unit, .. } if *unit > 0 => Some(*unit),
                    _ => None,
                })
                .chain([1])
                .collect();
            let pinned: BTreeSet<i64> = mentioned
                .iter()
                .filter_map(|r| match r {
                    Requirement::Equal { value, .. } => Some(*value),
                    _ => None,
                })
                .collect();
            let mut keep = |d: i64| {
                let structured = units
                    .iter()
                    .any(|u| d % u == 0 && u64::try_from(d / u).is_ok_and(u64::is_power_of_two));
    ...
    let values: Vec<i64> = values
        .into_iter()
        .filter(|v| owner.iter().all(|r| requirement_holds(r, *v)))
        .collect();
    if values.len() <= DOMAIN_VALUES {
        return values;
    }
    let last = values.len() - 1;
    let ordinals: BTreeSet<usize> = (0..DOMAIN_VALUES)
        .map(|k| k * last / (DOMAIN_VALUES - 1))
        .collect();
...
pub fn domains(
    ...
    let all: Vec<&Requirement> = family
        .occurrences
        .iter()
    ...
    for site in &family.sites {
        let mentioned: Vec<Requirement> = all
            .iter()
            .filter(|r| requirement_site(r) == site.id)
            .map(|r| (*r).clone())
            .collect();
        let owner: Vec<Requirement> = family
        ...
        if let SiteKind::Width { slice, .. } = site.kind {
            ...
            if let Some(SliceParent::Domain { lo, hi }) = parent {
                let fixed = |name: &String| {
                    template.shapes.contains_key(name)
                        || candidate
                            .structural
                            .iter()
                            .any(|(structural, _)| structural == name)
                };
        ...
        if values.is_empty() {
            let definition = &program
                .definition(
                    family
                        .template(family.candidate(site.owner).template)
                        .definition,
                )
                .name;
            return Err(SelectionError::UnsupportedMapping(format!(
                "`{definition}` site {}: no divisor of extent {} satisfies its requirements",
                site.id.0, site.extent
            )));
        }
...
pub fn unit_launch<'b, A: Accounting>(
    analysis: &'b Analysis<'_, A>,
    owner: CandidateRef,
    kind: &UnitKind,
) -> Option<&'b Launch<A>> {
    let UnitKind::Region(region) = kind else {
        return None;
    };
    analysis
        .accounts
...
pub fn intervals<A: Accounting>(
    ...
    for sequence in &family.sequences {
        ...
        let invocation = analysis
            ...
            .map_err(SelectionError::Reconstruction)?
            .1
            .invocation;
        let fusible = |left: usize| -> bool {
            ...
            let (Some(x), Some(y)) = (
                unit_launch(analysis, sequence.owner, &a.kind),
                unit_launch(analysis, sequence.owner, &b.kind),
            ) else {
                return false;
            };
            let plain = |l: &Launch<A>| {
                l.mode == RegionMode::Parallel && !l.merge && l.binders.len() == l.binder_count
            };
            invocation
                && plain(x)
                && plain(y)
                && x.binder_count == y.binder_count
                && x.binders.iter().zip(&y.binders).all(|(p, q)| p.1 == q.1)
        };
            ...
            for end in start + 2..=sequence.units.len() {
                ...
                if region_run {
                    ...
                    for unit in &sequence.units[start + 1..end] {
                        ...
                        equal_sites.extend(pairs.into_iter().flat_map(|(a, b)| {
                            a.binders.iter().zip(&b.binders).map(|(p, q)| (p.0, q.0))
                        }));
                    }
                }
...
pub fn constraints(
    ...
    for legality in legalities
        .iter()
        .filter(|l| l.scope().is_empty() && !(l.holds)(&|_| None))
    {
        if legality
            .guard
            .iter()
            .all(|g| family.occurrence(g.occurrence).candidates.len() == 1)
        {
            return Err(SelectionError::IncompatibleComposition(format!(
                "{} (needs {})",
                legality.reason,
                (legality.needed)(&|_| None).unwrap_or_else(|e| e)
            )));
        }
    }
    Ok(legalities
        ...
        .map(|legality| {
            ...
            Constraint {
                guard: legality.guard,
                scope: sites,
                holds: Box::new(move |values| holds(&lookup(&table, values))),
...
pub fn factors<A: Accounting>(
    ...
    for sequence in &family.sequences {
        sequenced.extend(
            sequence
                .units
                .iter()
                .filter_map(|u| unit_launch(analysis, sequence.owner, &u.kind))
                .map(|l| (sequence.owner, l.region.0)),
        );
    }
    for (&candidate, account) in &analysis.accounts {
        ...
        let mut scopes: Vec<(String, u64, &A::Work, &Quantity, Launched)> = account
            ...
            .map(|(ordinal, l)| {
                (
                    format!("region#{}", l.region.0),
                    u64::from(!sequenced.contains(&(candidate, l.region.0))),
                    &l.work,
                    &l.pieces,
        ...
        scopes.push((
            "body".into(),
            account.serial_launches,
            &account.rest,
            &account.context.scope.pieces,
            account.context.scope.launch,
        ));
        for (part, launches, work, pieces, launch) in scopes {
            if launches == 0 && A::quantities(work).is_empty() {
                continue;
            }
            let ScopeCost { reads, cost } = costs.scope(CostScope {
                analysis,
                account,
                launch,
                launches,
    ...
    for (ordinal, interval) in intervals.iter().enumerate() {
        ...
        if units
            .iter()
            .all(|u| unit_launch(analysis, sequence.owner, &u.kind).is_some())
        {
            ...
            out.push(Factor {
                ...
                label: format!(
                    "`{name}` sequence {} [{}, {}): one launch, unqualified estimate",
        ...
        let surviving: Result<Quantity, String> =
            analysis
                .sequence_block(sequence)
                .and_then(|(block, executed)| {
                    let after = block
                        .get(units[units.len() - 1].statements.end..)
                        .unwrap_or(&[]);
                    let mut terms = Vec::new();
                    for (position, unit) in units
                        .iter()
                        .enumerate()
                        .filter(|(_, u)| u.kind == UnitKind::Elementwise)
                    {
                        let (variable, bits) = analysis.unit_output(sequence.owner, block, unit)?;
                        if position + 1 == units.len()
                            || variable.is_none_or(|v| referenced(after, v))
                        {
                            terms.push(Quantity::product(
                                executed.multiplicity.iter().cloned().chain([bits]),
                            ));
                        }
                    }
        ...
        let sites = surviving
            .as_ref()
            .map_or_else(|_| Vec::new(), |q| scope([q]));
        let (table, materialization) = (sites.clone(), costs.materialization());
        out.push(Factor {
            guard,
            intervals: reference,
            scope: sites,
            cost: Box::new(move |values| {
                materialization(
                    surviving
                        .as_ref()
                        .map_err(Clone::clone)?
                        .eval(&lookup(&table, values))?,
                )
            }),
            label: format!(
                "`{name}` sequence {} [{}, {}): surviving tile write + read, unqualified estimate",
                sequence.id.0, interval.start, interval.end
            ),
        });
    }
...
pub fn seed<A: Accounting>(
    ...
    impl Construction<'_> {
        fn admissible(&self, chosen: &[CandidateRef], candidate: CandidateRef) -> bool {
            let requirements: Vec<&Requirement> = chosen
                .iter()
                .chain([&candidate])
                .flat_map(|c| &self.family.candidate(*c).requirements)
                .collect();
            self.family
                .candidate(candidate)
                .requirements
                .iter()
                .map(requirement_site)
                .all(|site| {
                    self.domains.get(&site).is_some_and(|values| {
                        values.iter().any(|v| {
                            requirements
                                .iter()
                                .filter(|r| requirement_site(r) == site)
                                .all(|r| requirement_holds(r, *v))
                        })
                    })
                })
        }
        ...
        fn complete(
            &mut self,
            pending: &mut Vec<OccurrenceId>,
            chosen: &mut Vec<CandidateRef>,
        ) -> bool {
            ...
            let preferred: Vec<u32> = ordinals
                .clone()
                .filter(lowered)
                .chain(ordinals.filter(|c| !lowered(c)))
                .collect();
            for candidate in preferred.into_iter().map(|candidate| CandidateRef {
                occurrence: id,
                candidate,
            }) {
    ...
    if !construction.complete(&mut vec![OccurrenceId(0)], &mut active) {
        ...
        let name = family
            .occurrences
            .get(id.0 as usize)
            .and_then(|o| program.families.get(o.family))
    ...
    witness
        .choices
        .extend(active.iter().map(|c| (c.occurrence, c.candidate)));
    // Admissible values: the domain under every active single-site requirement.
    let requirements: Vec<&Requirement> = active
        .iter()
        .flat_map(|c| &family.candidate(*c).requirements)
        .collect();
    ...
    for site in active.iter().flat_map(|c| &family.candidate(*c).sites) {
        ...
        let (Some(least), Some(most)) = (values.first().copied(), values.last().copied()) else {
            return Err(SelectionError::UnsupportedMapping(format!(
                "seed: site {} has no value satisfying the selected requirements",
                site.0
            )));
        };
        let parts = matches!(
            family.sites.get(site.0 as usize).map(|s| &s.kind),
            Some(SiteKind::Parts { .. })
        );
        witness
            .sites
            .insert(*site, if parts { least } else { most });
        admissible.insert(*site, values);
    }
    let step = |witness: &mut Witness, site: SiteId, up: bool| -> bool {
        let (Some(values), Some(current)) =
            (admissible.get(&site), witness.sites.get(&site).copied())
        else {
            return false;
        };
        let next = if up {
            ...
            values.iter().rev().copied().find(|v| *v < current)
        };
        next.map(|v| witness.sites.insert(site, v)).is_some()
    };
    for launch in active
        .iter()
        .flat_map(|c| &analysis.accounts[c].launches)
        .filter(|l| l.mode == RegionMode::Parallel)
    {
        ...
        while launch
            .pieces
            .eval(&|id| witness.sites.get(&id).copied())
            .is_ok_and(|p| p > piece_target)
        {
            if !launch
                .binders
                .iter()
                .rev()
                .any(|(site, _)| step(&mut witness, *site, true))
            {
                break;
            }
        }
    }
    let legalities: Vec<Legality> = legalities
        .into_iter()
        .chain(refinements(family))
        .filter(|l| l.guard.iter().all(|g| active.contains(g)))
        .collect();
    let mut budget = 2 * admissible.values().map(Vec::len).sum::<usize>() + 1;
    while let Some(violated) = legalities
        .iter()
        .find(|l| !(l.holds)(&|id| witness.sites.get(&id).copied()))
    {
        budget -= 1;
        let repaired = budget > 0
            && violated.repairs.iter().any(|(quantities, up)| {
                scope(quantities)
                    .iter()
                    .rev()
                    .any(|site| step(&mut witness, *site, *up))
            });
        if !repaired {
            return Err(SelectionError::UnsupportedMapping(format!(
                "seed: no admissible widths satisfy: {}",
                violated.reason
            )));
        }
    }
    for sequence in active.iter().flat_map(|c| &family.candidate(*c).sequences) {
        let units = family
            ...
            .map_or(0, |s| s.units.len()) as u32;
        if let Some(missing) = (0..units).find(|i| {
            !intervals
                .iter()
                .any(|v| v.sequence == *sequence && v.start == *i && v.end == i + 1)
        }) {
            return Err(SelectionError::UnsupportedMapping(format!(
                "seed: sequence {} unit {missing} has no separate execution",
                sequence.0
            )));
        }
        witness
            .covers
            .insert(*sequence, (0..units).map(|i| (i, i + 1)).collect());
    }
    Ok(witness)
}
