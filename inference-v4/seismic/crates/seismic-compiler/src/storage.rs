...
pub(crate) fn derive_launch_local_layout(
    arena: &mut ExprArena,
...
impl GlobalAllocationTopology {
    ...
    pub(crate) fn allocation_ids(&self) -> impl Iterator<Item = GlobalAllocationId> {
...
impl LocalAllocationTopology {
    ...
    pub(crate) fn into_locals(self) -> Vec<Vec<LocalAllocation>> {
        self.locals
    }
}
...
pub(crate) fn representation_alignment(representation: RepresentationId) -> u64 {
    match &registry::representation_info(representation).kind {
...
impl TopologyBuilder {
    ...
    pub(crate) fn views(&self) -> &[BufferViewLayout] {
        &self.views
    }
    pub(crate) fn view_layout(&self, view: AnyBufferView) -> &BufferViewLayout {
        self.assert_owner(view.owner());
        let layout = self
            .views
    ...
    pub(crate) fn allocation_kind(&self, id: GlobalAllocationId) -> &GlobalBufferKind {
        self.assert_owner(id.owner());
        &self.allocations[id.index() as usize].kind
    }
    pub(crate) fn allocation_bytes(&self, id: GlobalAllocationId) -> NatExpr {
        self.assert_owner(id.owner());
        self.allocations[id.index() as usize].bytes
    }
    pub(crate) fn allocation_alignment(&self, id: GlobalAllocationId) -> u64 {
        self.assert_owner(id.owner());
        self.allocations[id.index() as usize].alignment
    }
    pub(crate) fn result_views(&self) -> &[ResultViewPublication] {
        &self.result_views
    }
