...
pub fn reshape_strides(shape: &[i64], strides: &[i64], target: &[i64]) -> Result<Vec<i64>, String> {
    if shape.len() != strides.len() || shape.iter().chain(target).any(|n| *n < 0) || target.is_empty() {
        return Err("reshape requires nonnegative dimensions and a nonempty target rank".into());
    }
    let product = |dims: &[i64]| dims.iter().try_fold(1i64, |n,d| n.checked_mul(*d).ok_or_else(|| "reshape size overflow".to_string()));
    let count = product(shape)?;
    if count != product(target)? { return Err("reshape must preserve element count".into()); }
    let mut expected=1i64;
    if count != 0 {
        for (&dim,&stride) in shape.iter().zip(strides).rev() {
            if dim > 1 && stride != expected { return Err("reshape requires contiguous row-major storage".into()); }
...
pub fn row_major_strides(target: &[i64]) -> Result<Vec<i64>, String> {
    if target.iter().any(|n| *n < 0) { return Err("negative tensor extent".into()); }
