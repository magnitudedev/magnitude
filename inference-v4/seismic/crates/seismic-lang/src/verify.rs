//! Semantic invariants between transformations of the shared typed IR. These
//! checks validate retained facts; they do not infer optimization permissions.
use crate::{
    ast::AssignOp,
    ir::*,
...
pub enum Stage {
    /// Logical construct calls still participate in decomposition/composition.
    Decomposed,
    /// Backend bodies are expanded; storage and reduction choices may remain.
    Expanded,
    /// Every load and reduction choice must be resolved before realization.
    Executable,
}

pub fn lowered(function: &LoweredIr, stage: Stage) -> Result<(), String> {
    check(function, stage, None)
        .map_err(|error| format!("{}: invalid {stage:?} IR: {error}", function.name))
}
...
fn check_mode(
    ...
    for decision in &function.decisions {
        if !decision.domain.alternatives.contains(&decision.selected) {
            return Err("retained decision is outside its declared domain".into());
        }
    }
...
impl Verifier<'_> {
        ...
        for statement in body {
            match &statement.kind {
                ...
                StmtKind::LoadLoop {
                    ...
                    if self.stage == Stage::Executable && modes.is_none() {
                ...
                StmtKind::Reduction(reduction) => {
                    if reduction.inputs.is_empty()
                        || reduction.preparation.len() != reduction.inputs.len()
                        || reduction.unroll <= 0
                    {
                        return Err("reduction input/preparation metadata disagree".into());
                    }
                    for input in &reduction.inputs {
                        if input
                            .ty
                            .shaped()
                            .and_then(|s| s.shape.get(reduction.axis))
                            .is_none()
                        {
                            return Err("reduction axis is outside its input".into());
                        }
                    }
                    for operand in reduction.operands() {
                        self.expr(operand, bound)?;
                    }
                    if self.stage == Stage::Executable && reduction.tree.is_none() {
                        return Err("unresolved reduction tree".into());
                    }
                    if reduction
                        .tree
                        .is_some_and(|tree| !reduction.trees().contains(&tree))
                    {
                        return Err("reduction tree exceeds its source permission".into());
                    }
                    for implementation in reduction.implementations() {
                            ...
                            .chain(&implementation.output)
                        {
                            let ExprKind::Var(id) = value.kind else {
                                return Err("merge parameter has no binding identity".into());
                            };
                            self.reference(id, &value.ty)?;
                            inner.insert(id);
                        }
                        self.body(&implementation.body, &mut inner)?;
                        self.snapshots(&implementation.body)?;
                    }
                }
            }
        }
    ...
    fn expr(&self, expr: &Expr, bound: &HashSet<VarId>) -> Result<(), String> {
        ...
        match &expr.kind {
            ...
            ExprKind::Call { args, .. } => {
                if self.stage != Stage::Decomposed {
    ...
    fn snapshots(&self, body: &[Stmt]) -> Result<(), String> {
        for site in crate::normalize::loads::sites(body) {
            ...
            if self.stage == Stage::Executable && site.selected.is_none() {
