...
pub(super) fn children(e: &Expr) -> Vec<&Expr> {
    match &e.kind {
        ExprKind::Int(_) | ExprKind::Float(_) | ExprKind::Bool(_) | ExprKind::Var(_) | ExprKind::ShapeParam(_) | ExprKind::TileAlloc | ExprKind::CoordOf(_) => Vec::new(),
        ...
        ExprKind::Math { args, .. } | ExprKind::Intrinsic { args, .. } | ExprKind::Call { args, .. } => args.iter().collect(),
...
pub(super) fn direct_exprs(s: &Stmt) -> Vec<&Expr> {
    match &s.kind {
        ...
        StmtKind::Range { lo, hi, .. } => vec![lo, hi],
...
pub(super) fn nested_blocks(s: &Stmt) -> Vec<&Block> {
    ...
    match &s.kind {
        ...
        StmtKind::Range { body, .. } | StmtKind::Coordinates { body, .. } | StmtKind::Members { body, .. } => out.push(body),
...
fn pure(e: &Expr) -> bool {
    let mut pure = true;
    each_expr(e, &mut |x| {
        pure &= !matches!(x.kind, ExprKind::Call { .. } | ExprKind::Region(_) | ExprKind::Intrinsic { .. } | ExprKind::Atomic { .. } | ExprKind::TileAlloc)
    });
    pure
}
...
fn consumer(body: &Body, block: &Block, uses: &[usize], i: usize) -> Option<usize> {
    ...
    if body.vars.get(*var)?.kind != VarKind::Value || !matches!(value.ty, Ty::Tile(_)) || !pure(value) || uses.get(*var) != Some(&1) {
        return None;
    }
    let direct = |s: &Stmt| {
        let mut n = 0;
        for e in direct_exprs(s) {
            each_expr(e, &mut |x| n += usize::from(matches!(x.kind, ExprKind::Var(v) if v == *var)));
        }
        n
    };
    let later = &block[i + 1..];
    let j = later.iter().position(|s| direct(s) == 1)?;
    matches!(later[j].kind, StmtKind::Bind { .. } | StmtKind::Assign { .. } | StmtKind::Publish { .. } | StmtKind::Expr(_) | StmtKind::Yield(_) | StmtKind::Return(_)).then_some(i + 1 + j)
}
...
fn fold_block(body: &Body, block: &Block, uses: &[usize], out: &mut BTreeSet<VarId>) {
    ...
    for i in (0..block.len()).rev() {
        let Some(consumer) = consumer(body, block, uses, i) else { continue };
        let h = head[consumer];
        ...
        head[i] = h;
        start[h] = i;
        if let StmtKind::Bind { pattern: Pattern::Var(v), .. } = &block[i].kind {
            out.insert(*v);
        }
    }
}
...
pub(super) fn folded(body: &Body, uses: &[usize], sequences: &[(&Block, &[std::ops::Range<usize>])]) -> Result<BTreeSet<VarId>, String> {
    ...
    each_block(&body.block, &mut |block| {
        ...
        for range in ranges.iter() {
            for s in block.get(range.start..range.end.saturating_sub(1)).unwrap_or_default() {
                match &s.kind {
                    StmtKind::Bind { pattern: Pattern::Var(v), .. } => {
                        out.insert(*v);
                    }
                    _ => failure = Some(format!("a unit range {range:?} folds a statement that is not a `let`")),
                }
            }
        }
    });
...
pub(super) fn scope<'a>(body: &'a Body, steps: &[ScopeStep]) -> Result<&'a Block, String> {
    ...
    for step in steps {
        let statement = |n: usize| block.get(n).ok_or_else(|| format!("scope step {step:?} names no statement"));
        block = match step {
            ScopeStep::Region(id) => &find_region(block, *id).ok_or_else(|| format!("scope step {step:?} names no region of its block"))?.body,
            ScopeStep::Stage(k) => {
                let stage = block
                    .iter()
                    ...
                    .flatten()
                    .nth(*k);
                &stage.ok_or_else(|| format!("scope step {step:?} names no stage of its block"))?.body
            }
            ...
            ScopeStep::Loop(n) => match &statement(*n)?.kind {
                StmtKind::Range { body, .. } | StmtKind::Coordinates { body, .. } | StmtKind::Members { body, .. } => body,
...
pub(super) fn coordinate_local(block: &Block, body: &Body) -> bool {
    ...
    fn current(indices: &[Index], body: &Body) -> bool {
        indices.iter().all(|i| match i {
            Index::Coord(_) => true,
            Index::Point(Expr { kind: ExprKind::Var(v), .. }) => body.vars.get(*v).is_some_and(|v| v.kind == VarKind::Coordinate),
            _ => false,
        })
    }
    fn local(block: &Block, body: &Body, bound: &mut BTreeSet<VarId>, written: &mut BTreeSet<VarId>) -> bool {
        block.iter().all(|s| {
            let effect_free = direct_exprs(s).into_iter().all(|e| {
                let mut ok = true;
                each_expr(e, &mut |x| ok &= !matches!(x.kind, ExprKind::Call { .. } | ExprKind::Region(_) | ExprKind::Intrinsic { .. } | ExprKind::Atomic { .. }));
                ok
            });
            effect_free
                && match &s.kind {
                    ...
                    StmtKind::Range { body: inner, .. } | StmtKind::Coordinates { body: inner, .. } | StmtKind::Members { body: inner, .. } => local(inner, body, bound, written),
                    StmtKind::If { then, els, .. } => local(then, body, bound, written) && local(els, body, bound, written),
                    _ => false,
                }
        })
    }
    fn reads_current(e: &Expr, body: &Body, written: &BTreeSet<VarId>) -> bool {
        match &e.kind {
            ...
            ExprKind::Index { base, indices } if matches!(base.kind, ExprKind::Var(v) if written.contains(&v)) => {
                current(indices, body) && children(e).into_iter().skip(1).all(|c| reads_current(c, body, written))
            }
            _ => children(e).into_iter().all(|c| reads_current(c, body, written)),
        }
    }
    ...
    each_block(block, &mut |b| {
        for s in b {
            ok &= direct_exprs(s).into_iter().all(|e| reads_current(e, body, &written));
        }
    });
    ok
}
