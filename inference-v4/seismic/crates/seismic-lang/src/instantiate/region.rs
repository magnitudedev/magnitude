...
impl<'a> Instantiation<'a> {
    fn geometry(
        ...
        if let Some(result) = source {
            if result.binders.len() != region.binders.len() {
                return Err(format!(
                    "region#{} of `{name}` rebinds {} binders of a {}-binder result",
                    region.id.0,
                    region.binders.len(),
                    result.binders.len()
                ));
        ...
        for (k, var) in region.binders.iter().enumerate() {
            let sir::VarKind::Slice(slice) = f.declared(*var)?.kind else {
                return Err(format!(
                    "binder `{}` of region#{} in `{name}` is not a slice",
                    f.name(*var),
                    region.id.0
                ));
            };
            ...
            let decl = f
                ...
                .ok_or_else(|| format!("slice#{} is outside the body of `{name}`", slice.0))?;
    ...
    fn bind_slices(
        ...
        for (g, atom) in geometry.iter().zip(&t.atoms) {
            ...
            let slice = Slice {
                hi: lo.add(&Sym::constant(g.width)),
                lo,
                capacity: g.width,
                ordinal,
            };
    ...
    fn launch(
        ...
        Ok(stmt(
            StmtKind::Parallel {
                vars: t.vars.clone(),
                extents: geometry.iter().map(|g| g.count.clone()).collect(),
                body,
            },
            span,
        ))
    ...
    pub fn region(
        ...
        let source = match &region.source {
            ...
            sir::RegionSource::Results(e) => match self.value(f, e, out)? {
                ...
                _ => {
                    return Err(format!(
                        "region#{} of `{}` traverses a value that is not a region result",
                        region.id.0, f.definition.name
                    ))
                }
            },
        };
        ...
        if region.mode == RegionMode::Pipeline
            && (region.merge.is_some() || region.result.is_some())
        {
            return Err(format!(
                "pipeline region#{} of `{}` is used as a result expression",
                region.id.0, f.definition.name
            ));
        }
    ...
    fn region_body(
        ...
        if let Some(merge) = &region.merge {
            ...
            f.yields.push(YieldSink::Result {
                member,
                pieces: vec![symbol(flat, span)],
                loops: f.loops,
            });
            let body = self.block(f, &region.body, false);
            let Some(YieldSink::Result { member, .. }) = f.yields.pop() else {
                return Err(format!(
                    "region#{} of `{name}` lost its yield boundary",
                    region.id.0
                ));
            };
        ...
        if let Some(st::Ty::Result(ty)) = &region.result {
            ...
            let binders = geometry
                ...
                .collect();
            return Ok(Value::Result(ResultValue {
                binders,
                atoms: t.atoms.clone(),
                member,
            }));
        ...
        if region.result.is_some() {
            return Err(format!(
                "region#{} of `{name}` has a result type that is not a region result",
                region.id.0
            ));
        }
        ...
        let enclosing = match (launch, owners) {
            (true, _) => std::mem::replace(
                &mut self.launch_owner,
                Some(LaunchOwner {
                    candidate: f.candidate,
                    binders: geometry.iter().map(|g| g.slice).collect(),
                }),
            ),
    ...
    fn inner_owners(&self, f: &Frame<'a>, region: &sir::Region, geometry: &[Geometry]) -> bool {
        let Some(owner) = self
            .launch_owner
            .as_ref()
            .filter(|owner| owner.candidate == f.candidate)
        else {
            return false;
        };
        super::owner_regions(&self.family.target)
            && super::inner_owner_region(f.body, &owner.binders, region)
            && geometry.iter().all(|g| g.count.as_constant().is_some())
    }
    ...
    pub fn fused_regions(
        ...
        for region in &regions {
            ...
            if geometry.len() != shared.len()
                || geometry.iter().zip(&shared).any(|(a, b)| !a.same(b))
            {
    ...
    fn member_storage(
        ...
        match ty {
            st::Ty::Scalar(_) | st::Ty::Index(_) => {
                ...
                Ok(Member::Scalar(allocate(
                    self,
                    Shaped::new(axes().collect(), Elem::Dtype(dtype)),
                    out,
                )))
            }
            st::Ty::Tile(shaped) => {
                ...
                Ok(Member::Tile(allocate(
                    self,
                    Shaped::new(axes().chain(shaped.shape).collect(), shaped.elem),
                    out,
                )))
            }
            st::Ty::Tuple(items) => Ok(Member::Tuple(
                items
                    .iter()
                    .map(|t| self.member_storage(f, t, leading, span, out))
                    .collect::<Result<_, _>>()?,
            )),
            st::Ty::Result(inner) => {
                ...
                Ok(Member::Result(Box::new(ResultValue {
                    binders,
                    atoms: Vec::new(),
                    member,
                })))
            ...
            other => Err(format!(
                "a region result of `{}` yields `{other}`, which has no surviving local storage",
                f.definition.name
            )),
        }
    }
        ...
        match (member, value) {
            (Member::Scalar(store), Value::Scalar(e)) => {
                ...
                let Ty::Scalar(dtype) = place.ty else {
                    return Err("region-result storage lost its piece axes".into());
                };
            ...
            (Member::Tuple(members), Value::Tuple(values)) if members.len() == values.len() => {
                members
                    .iter_mut()
                    .zip(values)
                    .try_for_each(|(m, v)| self.store_member(m, pieces, v, out))
            }
            (Member::Result(slot), Value::Result(inner)) => {
                if slot.binders.len() != inner.binders.len()
                    || slot
                        .binders
                        .iter()
                        .zip(&inner.binders)
                        .any(|(a, b)| a.width != b.width || a.count != b.count)
                {
    ...
    fn store_nested(
        ...
        match (slot, inner) {
            (Member::Scalar(store), Member::Scalar(source))
            | (Member::Tile(store), Member::Tile(source)) => {
                self.store_tile(store, pieces, source.clone(), out)
            }
            (Member::Tuple(slots), Member::Tuple(inners)) if slots.len() == inners.len() => slots
                .iter()
                .zip(inners)
                .try_for_each(|(s, i)| self.store_nested(s, pieces, i, out)),
            (Member::Result(slot), Member::Result(inner)) => {
                self.store_nested(&slot.member, pieces, &inner.member, out)
            }
            _ => Err(
                "a yielded nested region result differs from the member schema of its storage"
                    .into(),
            ),
        }
    }
    ...
    fn view(member: &Member, pieces: &[Expr]) -> Result<Member, String> {
        Ok(match member {
            ...
            Member::Tuple(members) => Member::Tuple(
                members
                    .iter()
                    .map(|m| Self::view(m, pieces))
                    .collect::<Result<_, _>>()?,
            ),
            Member::Result(inner) => Member::Result(Box::new(ResultValue {
                binders: inner.binders.clone(),
                atoms: inner.atoms.clone(),
                member: Self::view(&inner.member, pieces)?,
            })),
        })
    ...
    pub fn member(
        ...
        let Value::Result(result) = self.value(f, result, out)? else {
            return Err(format!(
                "member selection in `{}` is not over a region result",
                f.definition.name
            ));
        };
        if slices.len() != result.binders.len() {
            return Err(format!(
                "member selection in `{}` uses {} slices for a {}-binder result",
                f.definition.name,
                slices.len(),
                result.binders.len()
            ));
        ...
        let ordinals = slices
            .iter()
            .map(|s| f.slice(*s).map(|s| s.ordinal.clone()))
            .collect::<Result<Vec<_>, _>>()?;
        let pieces: Vec<Expr> = ordinals
            .iter()
            .map(|o| symbol(o.clone(), Span::default()))
            .collect();
        let atoms = result.atoms.clone();
        let rebase = move |sym: &Sym| {
            substitute(sym, &|atom| {
                Ok(atoms
                    .iter()
                    .position(|a| a == atom)
                    .map(|k| ordinals[k].clone()))
            })
        };
    ...
    fn read_partial(
        ...
        Ok(match member {
            ...
            Member::Tuple(members) => Value::Tuple(
                members
                    .iter()
                    .map(|m| self.read_partial(m, at, out))
                    .collect::<Result<_, _>>()?,
            ),
            Member::Result(_) => {
                return Err("a merge over nested region results has no execution form".into())
            }
        })
    }
    ...
    fn detach(&mut self, value: Value<'a>, out: &mut Vec<Stmt>) -> Result<Value<'a>, String> {
        Ok(match value {
            ...
            Value::Shaped(e) if !matches!(e.kind, ExprKind::Var(_)) => {
                Value::Shaped(self.copy(e, out)?)
            }
            Value::Tuple(items) => Value::Tuple(
                items
                    .into_iter()
                    .map(|v| self.detach(v, out))
                    .collect::<Result<_, _>>()?,
            ),
            other => other,
        })
    ...
    fn combine(
        ...
        while live > 1 {
            ...
            let right = self.read_partial(
                &member,
                &symbol(ordinal.scale(2).add(&Sym::constant(1)), span),
                &mut body,
            )?;
            ...
            let Some(YieldSink::Slots(slots)) = f.yields.pop() else {
                return Err(format!(
                    "merge of `{}` lost its yield boundary",
                    f.definition.name
                ));
            };
            let merged = merged?;
            ...
            let mut values = slots
                .locals
                .or(slots.direct)
                .ok_or_else(|| format!("merge body of `{}` yields no value", f.definition.name))?;
    ...
    fn bind_operand(
        ...
        match (pattern, value) {
            ...
            (sir::Pattern::Tuple(patterns), Value::Tuple(values))
                if patterns.len() == values.len() =>
            {
                patterns
                    .iter()
                    .zip(values)
                    .try_for_each(|(p, v)| self.bind_operand(f, p, v))
            }
            _ => Err(format!(
                "merge operand pattern in `{}` does not match the partial schema",
                f.definition.name
            )),
        }
    }
