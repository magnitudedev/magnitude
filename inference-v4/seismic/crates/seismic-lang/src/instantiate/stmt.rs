...
fn items_of(block: &sir::Block) -> Vec<Item<'_>> {
    ...
    block.iter().enumerate().map(|(index, stmt)| Item { stmt, block: key, index }).collect()
}

impl<'a> Instantiation<'a> {
    pub fn block(&mut self, f: &mut Frame<'a>, block: &'a sir::Block, root: bool) -> Result<Vec<Stmt>, String> {
        self.items(f, &items_of(block), root)
    }

    fn items(&mut self, f: &mut Frame<'a>, items: &[Item<'a>], root: bool) -> Result<Vec<Stmt>, String> {
        let saved = f.root;
        let mut out = Vec::new();
        let mut i = 0;
        while i < items.len() {
            ...
            if let Some(fused) = f.fused_at(item.block, item.index).cloned() {
                let members: Vec<Item<'a>> = items[i..].iter().copied().take_while(|m| m.block == item.block && m.index <= fused.last).collect();
                if members.last().map(|m| m.index) != Some(fused.last) {
                    return Err(format!("a fused interval of `{}` is split by terminal control flow", f.definition.name));
                }
                match fused.kind {
    ...
    fn statement(&mut self, f: &mut Frame<'a>, s: &'a sir::Stmt, root: bool, out: &mut Vec<Stmt>) -> Result<(), String> {
        ...
        if matches!(s.kind, sir::StmtKind::Range { .. } | sir::StmtKind::Coordinates { .. } | sir::StmtKind::Members { .. } | sir::StmtKind::If { .. }) {
            let owner = self.launch_owner.take();
    ...
    fn statement_inner(&mut self, f: &mut Frame<'a>, s: &'a sir::Stmt, root: bool, out: &mut Vec<Stmt>) -> Result<(), String> {
        match &s.kind {
            ...
            sir::StmtKind::Assign { target, op, value } => self.assign_to(f, target, *op, value, out),
            sir::StmtKind::Region(region) => self.region(f, region, root, out).map(|_| ()),
            sir::StmtKind::Stages(stages) => self.stages(f, stages, root, out),
            sir::StmtKind::Range { var, lo, hi, body } => {
                let lo = self.scalar(f, lo, out)?;
                let lo = self.symbolic(lo, out)?;
                let hi = self.scalar(f, hi, out)?;
                ...
                f.loops -= 1;
                out.push(stmt(StmtKind::Range { var: id, lo, hi, body: body? }, s.span));
                Ok(())
            }
            sir::StmtKind::Coordinates { vars, of, axes, body } => {
                let tile = self.reference(f, of, out)?;
                let shape = tile_shape(&tile)?.shape.clone();
                if vars.len() != axes.len() || axes.iter().any(|a| *a >= shape.len()) {
                    return Err(format!("coordinate loop in `{}` disagrees with the rank of its operand", f.definition.name));
                }
                ...
                let element_domain = every_axis && matches!(tile.ty, Ty::Tile(_)) && walk::coordinate_local(body, f.body);
                f.loops += 1;
                let inner = self.block(f, body, false);
                f.loops -= 1;
                let mut inner = inner?;
                if element_domain {
                    out.push(stmt(StmtKind::Owned { vars: ids, tile, body: inner }, s.span));
                } else {
                    // Ordered scalar control over the listed axes, first binder outermost.
                    for (id, axis) in ids.into_iter().zip(axes).rev() {
                        inner = vec![stmt(StmtKind::Range { var: id, lo: crate::sym::Sym::constant(0), hi: shape[*axis].clone(), body: inner }, s.span)];
                    }
                    out.extend(inner);
                }
                Ok(())
            ...
            sir::StmtKind::Members { var, slice, body } => {
                ...
                let body = self.block(f, body, false);
                f.loops -= 1;
                out.push(stmt(StmtKind::Range { var: id, lo: bound.lo, hi: bound.hi, body: body? }, s.span));
                Ok(())
            }
            ...
            sir::StmtKind::Publish { value, destination } => {
                ...
                match destination.ty {
                    Ty::Tensor(_) => {
                        ...
                        let store = Expr { kind: ExprKind::Builtin { name: Builtin::Store, args: vec![tile, destination] }, ty: Ty::Void, sym: None, span: s.span };
            ...
            sir::StmtKind::Yield(values) => {
                let values = values.iter().map(|v| self.value(f, v, out)).collect::<Result<Vec<_>, _>>()?;
                let Some(sink) = f.yields.pop() else {
                    return Err(format!("`yield` in `{}` has no structural consumer", f.definition.name));
                };
                let (sink, result) = match sink {
                    ...
                    YieldSink::Result { mut member, pieces, loops } => {
                        let result = if f.loops != loops {
                            Err(format!("`yield` inside a loop of a result-producing visit in `{}`", f.definition.name))
                        } else {
                            let value = if values.len() == 1 { values.into_iter().next().unwrap_or(Value::Void) } else { Value::Tuple(values) };
                            self.store_member(&mut member, &pieces, value, out)
                        };
                        (YieldSink::Result { member, pieces, loops }, result)
                    }
                };
                f.yields.push(sink);
                result
    ...
    pub fn bind(&mut self, f: &mut Frame<'a>, pattern: &sir::Pattern, value: Value<'a>, out: &mut Vec<Stmt>) -> Result<(), String> {
        match pattern {
            sir::Pattern::Var(v) => {
                ...
                let bound = if declared.kind == sir::VarKind::State { self.own(value, &declared.name, out)? } else { self.snapshot(value, &declared.name, out)? };
                f.vars[*v] = Some(bound);
                Ok(())
            }
            sir::Pattern::Tuple(patterns) => match value {
                Value::Tuple(values) if values.len() == patterns.len() => patterns.iter().zip(values).try_for_each(|(p, v)| self.bind(f, p, v, out)),
                _ => Err(format!("tuple pattern in `{}` does not match its value", f.definition.name)),
            },
        }
    }
    ...
    fn fill(&mut self, f: &Frame<'a>, slots: &mut Slots<'a>, values: Vec<Value<'a>>, out: &mut Vec<Stmt>) -> Result<(), String> {
        if f.loops != slots.loops {
            return Err(format!("`yield`/`return` inside a loop in `{}` has no execution form", f.definition.name));
        }
        if f.conditional == slots.conditional && slots.locals.is_none() && slots.direct.is_none() {
            slots.direct = Some(values.into_iter().map(|v| self.snapshot(v, "port", out)).collect::<Result<_, _>>()?);
            return Ok(());
        }
        if slots.locals.is_none() {
            let mut prologue = Vec::new();
            let locals = values.iter().map(|v| self.declare(v, &mut prologue)).collect::<Result<Vec<_>, _>>()?;
        ...
        if locals.len() != values.len() {
            return Err(format!("paths of `{}` yield different arities", f.definition.name));
        }
        locals.iter().zip(values).try_for_each(|(local, value)| self.write_local(local, value, out))
    }

    fn declare(&mut self, value: &Value<'a>, prologue: &mut Vec<Stmt>) -> Result<Value<'a>, String> {
        match value {
            Value::Scalar(e) => {
                let Ty::Scalar(dtype) = e.ty else { return Err("a non-scalar value is yielded as a scalar".into()) };
            ...
            Value::Shaped(e) if matches!(e.ty, Ty::Tile(_) | Ty::Tensor(_)) => {
                ...
                if shaped.shape.iter().any(|d| d.as_constant().is_none()) || !matches!(shaped.elem, crate::types::Elem::Dtype(_)) {
            ...
            Value::Tuple(items) => Ok(Value::Tuple(items.iter().map(|v| self.declare(v, prologue)).collect::<Result<_, _>>()?)),
            Value::Void => Ok(Value::Void),
            _ => Err("a conditionally yielded view, slice or region result has no execution form".into()),
        }
    }

    fn write_local(&mut self, local: &Value<'a>, value: Value<'a>, out: &mut Vec<Stmt>) -> Result<(), String> {
        match (local, value) {
            (Value::Scalar(target), Value::Scalar(e)) => {
                let Ty::Scalar(dtype) = target.ty else { return Err("scalar port lost its type".into()) };
                out.push(assign(target.clone(), conform(e, dtype)));
                Ok(())
            }
            (Value::Shaped(target), Value::Shaped(e)) => {
                let source = if matches!(e.ty, Ty::Tensor(_)) { self.load(e, out)? } else { e };
            ...
            (Value::Tuple(locals), Value::Tuple(values)) if locals.len() == values.len() => locals.iter().zip(values).try_for_each(|(l, v)| self.write_local(l, v, out)),
            (Value::Void, Value::Void) => Ok(()),
            _ => Err("conditional paths yield different value schemas".into()),
        }
    }

    fn stages(&mut self, f: &mut Frame<'a>, stages: &'a [sir::Stage], root: bool, out: &mut Vec<Stmt>) -> Result<(), String> {
        let mut ports: Vec<Value<'a>> = Vec::new();
        for (k, stage) in stages.iter().enumerate() {
            if stage.ports.len() != ports.len() {
                return Err(format!("stage `{}` of `{}` binds {} ports but its predecessor yields {}", stage.name, f.definition.name, stage.ports.len(), ports.len()));
            }
            ...
            f.yields.push(YieldSink::Slots(Slots { conditional: f.conditional, loops: f.loops, ..Slots::default() }));
            let body = self.block(f, &stage.body, root);
            let Some(YieldSink::Slots(slots)) = f.yields.pop() else {
                return Err(format!("stage `{}` of `{}` lost its port boundary", stage.name, f.definition.name));
            };
            let body = body?;
    ...
    fn assign_to(&mut self, f: &mut Frame<'a>, target: &'a sir::Expr, op: AssignOp, value: &'a sir::Expr, out: &mut Vec<Stmt>) -> Result<(), String> {
        match &target.kind {
            sir::ExprKind::Tuple(targets) => {
                if op != AssignOp::Assign {
                    return Err(format!("compound tuple assignment in `{}`", f.definition.name));
                }
                ...
                let values = match self.value(f, value, out)? {
                    ...
                    _ => return Err(format!("tuple assignment in `{}` does not match its value", f.definition.name)),
                };
                let mut held = Vec::with_capacity(values.len());
                for v in values {
                    held.push(self.own(v, "old", out)?);
                }
                targets.iter().zip(held).try_for_each(|(t, v)| self.assign_value(f, t, v, out))
            }
            sir::ExprKind::Var(_) | sir::ExprKind::Index { .. } => {
                let place = match self.value(f, target, out)? {
                    Value::Scalar(place) | Value::Shaped(place) => place,
                    _ => return Err(format!("assignment target in `{}` has no storage", f.definition.name)),
                };
                match place.ty {
                    Ty::Scalar(dtype) => {
            ...
            _ => Err(format!("assignment target in `{}` has no storage", f.definition.name)),
        }
    }

    fn assign_value(&mut self, f: &mut Frame<'a>, target: &'a sir::Expr, value: Value<'a>, out: &mut Vec<Stmt>) -> Result<(), String> {
        if let (sir::ExprKind::Tuple(targets), Value::Tuple(values)) = (&target.kind, &value) {
            if targets.len() == values.len() {
                return targets.iter().zip(values.clone()).try_for_each(|(t, v)| self.assign_value(f, t, v, out));
            }
        }
        let place = match self.value(f, target, out)? {
            Value::Scalar(place) | Value::Shaped(place) => place,
        ...
        match (place.ty.clone(), value) {
            ...
            _ => return Err(format!("assignment in `{}` changes the kind of its target", f.definition.name)),
        }
        Ok(())
    }

    /// `place op= source` between complete tiles.
    fn copy_into(&mut self, place: Expr, op: AssignOp, source: Expr, out: &mut Vec<Stmt>) -> Result<(), String> {
        let source = if matches!(source.ty, Ty::Tensor(_)) { self.load(source, out)? } else { source };
        if let ExprKind::Var(id) = source.kind {
            self.temporaries.remove(&id);
        }
        if op == AssignOp::Assign && matches!(place.kind, ExprKind::Var(_)) {
            out.push(assign(place, source));
            return Ok(());
        }
        let span = source.span;
        let (vars, at) = self.coordinates(tile_shape(&source)?.shape.len(), span);
        let body = vec![assign_op(points(place, &at)?, op, points(source.clone(), &at)?)];
    ...
    pub fn assign_tile(&mut self, f: &mut Frame<'a>, place: Expr, op: AssignOp, value: &'a sir::Expr, target: Option<sir::VarId>, out: &mut Vec<Stmt>) -> Result<(), String> {
        let whole = matches!(
            value.kind,
            sir::ExprKind::Var(_)
                | sir::ExprKind::Call { .. }
                ...
                | sir::ExprKind::Reduce { .. }
                | sir::ExprKind::Load(_)
        ) || !matches!(value.ty, st::Ty::Tile(_) | st::Ty::View(_) | st::Ty::Tensor(_));
        ...
        fn remaps(f: &Frame<'_>, e: &sir::Expr, target: sir::VarId, under: bool) -> bool {
            match &e.kind {
                ...
                sir::ExprKind::Var(v) => matches!(f.vars.get(*v), Some(Some(Value::Deferred(d))) if remaps(f, d, target, under)),
                sir::ExprKind::Load(_) | sir::ExprKind::Decode(_) | sir::ExprKind::Reduce { .. } => false,
                sir::ExprKind::Cast { expr, .. } if matches!(expr.ty, st::Ty::View(_) | st::Ty::Tensor(_)) => false,
                sir::ExprKind::Transpose(_) | sir::ExprKind::Index { .. } => walk::children(e).into_iter().any(|c| remaps(f, c, target, true)),
                _ => walk::children(e).into_iter().any(|c| remaps(f, c, target, under)),
            }
        }
        let hazard = target.is_some_and(|v| remaps(f, value, v, false));
        if whole || hazard {
            if !matches!(value.ty, st::Ty::Tile(_) | st::Ty::View(_) | st::Ty::Tensor(_)) {
                ...
                out.push(stmt(StmtKind::Owned { vars, tile: place, body }, value.span));
                return Ok(());
            }
        ...
        let element = self.element(f, value, &Coordinate { at: &at, group: &group, reads: &reads, reversed: false }, out)?;
    ...
    fn fused_elementwise(&mut self, f: &mut Frame<'a>, members: &[Item<'a>], out: &mut Vec<Stmt>) -> Result<(), String> {
        let definition: &'a sir::Definition = f.definition;
        let unsupported = || format!("a fused elementwise interval of `{}` contains a statement that is not a tile binding or tile state update", definition.name);
        ...
        for member in members {
            match &member.stmt.kind {
                ...
                sir::StmtKind::Bind { pattern: sir::Pattern::Var(v), value } if matches!(value.ty, st::Ty::Tile(_)) => {
                    group.insert(*v);
                    entries.push(member.stmt);
                }
                sir::StmtKind::Assign { target, value, .. } if matches!(target.kind, sir::ExprKind::Var(_)) && matches!(target.ty, st::Ty::Tile(_)) && matches!(value.ty, st::Ty::Tile(_)) => {
                    entries.push(member.stmt)
                }
        ...
        for entry in &entries {
            let sir::StmtKind::Bind { pattern: sir::Pattern::Var(v), value } = &entry.kind else {
                tiles.push(None);
                continue;
            };
            let st::Ty::Tile(declared) = &value.ty else { return Err(unsupported()) };
            let shaped = self.shaped(f, declared)?;
            if shape.get_or_insert_with(|| shaped.shape.clone()) != &shaped.shape {
                return Err(format!("a fused elementwise interval of `{}` spans different element domains", f.definition.name));
            }
            let inside: usize = entries.iter().flat_map(|s| walk::direct_exprs(s)).map(|e| {
                let mut n = 0;
                walk::each_expr(e, &mut |x| n += usize::from(matches!(x.kind, sir::ExprKind::Var(u) if u == *v)));
                n
            }).sum();
            let state = f.declared(*v)?.kind == sir::VarKind::State;
            let escapes = state || f.uses.get(*v).copied().unwrap_or(0) > inside;
            tiles.push(if escapes { Some(self.allocate(f.name(*v), shaped, value.span, out)) } else { None });
        }
        ...
        for entry in &entries {
            if let sir::StmtKind::Assign { target, .. } = &entry.kind {
                ...
                if shape.get_or_insert_with(|| place.ty.shaped().map(|s| s.shape.clone()).unwrap_or_default()) != &tile_shape(&place)?.shape {
                    return Err(format!("a fused elementwise interval of `{}` spans different element domains", f.definition.name));
                }
        ...
        let domain = match domain {
            Some(domain) => domain,
            None => {
                let Some(sir::StmtKind::Bind { value, .. }) = entries.first().map(|s| &s.kind) else { return Ok(()) };
                let st::Ty::Tile(declared) = &value.ty else { return Err(unsupported()) };
                let shaped = self.shaped(f, declared)?;
        ...
        for (entry, tile) in entries.iter().zip(&tiles) {
            let c = Coordinate { at: &at, group: &group, reads: &reads, reversed: false };
            match &entry.kind {
                sir::StmtKind::Bind { pattern: sir::Pattern::Var(v), value } => {
            ...
            if let (sir::StmtKind::Bind { pattern: sir::Pattern::Var(v), .. }, Some(tile)) = (&entry.kind, tile) {
                if let ExprKind::Var(id) = tile.kind {
                    self.temporaries.remove(&id);
                    if f.declared(*v)?.kind == sir::VarKind::State {
                        self.mutable.insert(id);
        ...
        out.extend(pre);
        out.push(stmt(StmtKind::Owned { vars, tile: domain.clone(), body }, domain.span));
        Ok(())
    }
    ...
    pub fn call(&mut self, f: &mut Frame<'a>, call: sir::CallId, args: &'a [sir::Expr], out: &mut Vec<Stmt>) -> Result<Value<'a>, String> {
        if self.depth >= 64 {
            return Err(format!("call nesting under `{}` exceeds the instantiation limit", f.definition.name));
        }
        ...
        let occurrence = parent
            ...
            .ok_or_else(|| format!("call {} of `{}` has no occurrence under the selected candidate", call.0, f.definition.name))?;
        let choice = *self.witness.choices.get(&occurrence.id).ok_or_else(|| format!("witness has no choice for active occurrence {} in `{}`", occurrence.id.0, f.definition.name))?;
        let selected = CandidateRef { occurrence: occurrence.id, candidate: choice };
        let mut callee = self.frame(selected)?;
        let via = self.candidate(selected)?.via;
        let site = f.body.calls.get(call.0 as usize).ok_or_else(|| format!("call {} is outside the body of `{}`", call.0, f.definition.name))?;
        let binding = site
            .bindings
            .iter()
            .find(|b| b.definition == callee.definition.id)
            .or_else(|| site.bindings.iter().find(|b| b.definition == via))
            .ok_or_else(|| format!("call to `{}` in `{}` has no argument binding for the selected definition", callee.definition.name, f.definition.name))?;
        let values = args.iter().map(|a| self.value(f, a, out)).collect::<Result<Vec<_>, _>>()?;
        for (ordinal, param) in callee.definition.params.iter().enumerate() {
            let value = binding.arg_order.get(ordinal).and_then(|a| values.get(*a)).cloned().ok_or_else(|| {
                format!("parameter `{}` of `{}` has no argument in `{}`", param.name, callee.definition.name, f.definition.name)
            })?;
            let bound = match (param.mode, value) {
                (Mode::In, value @ Value::Scalar(_)) => self.snapshot(value, &param.name, out)?,
                (Mode::In, Value::Shaped(e)) => {
                    let e = if matches!(param.ty, st::Ty::Tile(_)) && matches!(e.ty, Ty::Tensor(_)) { self.load(e, out)? } else { e };
                    if let ExprKind::Var(id) = e.kind {
                        self.temporaries.remove(&id);
                    }
                    Value::Shaped(e)
                }
                (Mode::Out | Mode::Inout, Value::Shaped(e)) if root(&e).is_some() => Value::Shaped(e),
                (Mode::Out | Mode::Inout, _) => {
                    return Err(format!("`{}` argument `{}` of `{}` is not a place", if param.mode == Mode::Out { "out" } else { "inout" }, param.name, callee.definition.name));
                }
                (_, other) => other,
            };
            ...
            let slot = callee.vars.get_mut(param.var).ok_or_else(|| format!("parameter `{}` of `{}` has no variable", param.name, callee.definition.name))?;
            *slot = Some(bound);
        }
        ...
        for name in &template.dynamic {
            let extent = callee.definition.params.iter().find_map(|param| {
                let axis = param.ty.shaped()?.axes.iter().position(|a| a.semantic() == Some(&crate::sym::Sym::param(name)))?;
        ...
        fn release(value: &Value<'_>, first: usize, temporaries: &mut BTreeSet<usize>) {
            match value {
                Value::Shaped(Expr { kind: ExprKind::Var(id), ty: Ty::Tile(_), .. }) if *id >= first => {
        ...
        values.iter().for_each(|v| release(v, first_local, &mut self.temporaries));
        Ok(match (&callee.definition.result, values.len()) {
            (st::Ty::Void, _) => Value::Void,
            (_, 1) => values.swap_remove(0),
            (_, 0) => return Err(format!("`{}` returns no value on the instantiated path", callee.definition.name)),
