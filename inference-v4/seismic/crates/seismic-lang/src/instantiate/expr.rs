...
impl<'a> Instantiation<'a> {
    pub fn value(
        ...
        match &e.kind {
            ...
            sir::ExprKind::Range { lo, hi } => Ok(Value::Range(
                self.scalar(f, lo, out)?,
                self.scalar(f, hi, out)?,
            )),
            ...
            sir::ExprKind::Reshape { base, axes } => {
                ...
                let shaped = Shaped { packed_axis: source.packed_axis.map(|_| shape.len().saturating_sub(1)), elem: source.elem, shape: shape.clone() };
                let ty = if matches!(base.ty, Ty::Tensor(_)) { Ty::Tensor(shaped) } else { Ty::Tile(shaped) };
            ...
            sir::ExprKind::Intrinsic { op, args } if op.produces_owned_result() => {
                let declared = e
                    .ty
                    .shaped()
                    .ok_or_else(|| format!("logical intrinsic `{op}` has no shaped result"))?;
                Ok(Value::Shaped(self.intrinsic_result(
                    f, *op, args, declared, e.span, out,
                )?))
            }
            _ => match &e.ty {
                ...
                // Logical owned tensor snapshots are realized into compiler-owned execution
                // storage. `tensor` remains a source ownership category; it does not become
                // an author-visible tile contract.
                st::Ty::Tensor(_) if matches!(e.kind, sir::ExprKind::Load(_) | sir::ExprKind::TileAlloc) => {
                    Ok(Value::Shaped(self.materialize(f, e, out)?))
                }
    ...
    pub fn scalar(
        ...
        match self.value(f, e, out)? {
            Value::Scalar(x) => Ok(x),
            _ => Err(format!(
                "`{}` in `{}` is used as a scalar",
                e.ty, f.definition.name
            )),
        }
    }
    ...
    fn operand(
        ...
        match self.value(f, e, out)? {
            ...
            _ => Err(format!(
                "`{}` in `{}` is not an intrinsic operand",
                e.ty, f.definition.name
            )),
        }
    }
    ...
    fn native(
        ...
        let sir::ExprKind::Intrinsic { op, .. } = &e.kind else {
            return Err(format!(
                "native `{}.{}` in `{}` is not produced by an intrinsic",
                native.target, native.name, f.definition.name
            ));
        };
        if *op != crate::intrinsics::Operation::Matrix {
            return Err(format!(
                "intrinsic `{op}` in `{}` does not declare a native value",
                f.definition.name
            ));
        }
        let dtype = match native.elem.as_ref().map(|e| self.elem(f, e)).transpose()? {
            ...
            _ => {
                return Err(format!(
                    "native `{}` in `{}` needs a dense element type",
                    native.name, f.definition.name
                ))
            }
        };
    ...
    fn compute(
        ...
        let span = e.span;
        Ok(match &e.kind {
            sir::ExprKind::Int(n) if dtype.is_int() => Expr {
                kind: ExprKind::Int(*n),
                ty,
            ...
            sir::ExprKind::ShapeParam(name) => {
                coerce(symbol(self.resolve(f, &Sym::param(name))?, span), dtype)
            }
            ...
            sir::ExprKind::Unary { op, expr } => {
                ...
                let sym = match op {
                    crate::syntax::ast::UnaryOp::Neg if dtype.is_int() => {
                        inner.sym.as_ref().map(Sym::neg)
                    }
                    _ => None,
                };
                Expr {
            ...
            sir::ExprKind::Binary { op, lhs, rhs } => binary(
                *op,
                self.scalar(f, lhs, out)?,
                self.scalar(f, rhs, out)?,
                ty,
            ),
            sir::ExprKind::Math { op, args } => {
                let args = args
                    .iter()
                    .map(|a| self.scalar(f, a, out))
            ...
            sir::ExprKind::Select { cond, then, els } => {
                let args = vec![
                    self.scalar(f, cond, out)?,
                    coerce(self.scalar(f, then, out)?, dtype),
                    coerce(self.scalar(f, els, out)?, dtype),
                ];
                Expr {
                    kind: ExprKind::Builtin {
                        name: Builtin::Select,
            ...
            sir::ExprKind::CoordOf(v) => {
                let coordinate = match f.var(*v)? {
                    ...
                    _ => {
                        return Err(format!(
                            "`coord({})` in `{}` is not over a coordinate",
                            f.name(*v),
                            f.definition.name
                        ))
                    }
                ...
                if origin.is_zero() {
                    ...
                    binary(BinaryOp::Add, symbol(origin, span), coordinate, ty)
                }
            }
            sir::ExprKind::ExtentOf { base, axis } | sir::ExprKind::Geometry { base, axis, .. } => {
                ...
                let base = self.reference(f, base, out)?;
                let extent = tile_shape(&base)?
                    .shape
                    .get(*axis)
                    .cloned()
                    .ok_or_else(|| {
                        format!(
                            "axis {axis} is outside its operand in `{}`",
                            f.definition.name
                        )
                    })?;
            ...
            sir::ExprKind::Intrinsic { op, args } => {
                let args = args
                    .iter()
                    .map(|a| self.operand(f, a, out))
                    .collect::<Result<Vec<_>, _>>()?;
                Expr {
                    kind: ExprKind::Intrinsic { op: *op, args },
                    ty,
                    sym: None,
                    span,
                }
            ...
            other => {
                return Err(format!(
                    "scalar expression {other:?} in `{}` has no execution form",
                    f.definition.name
                ))
            }
        })
    ...
    fn structural_origin(&self, f: &Frame<'a>, extent: &Sym) -> Sym {
        match extent.params().as_slice() {
            [name] if *extent == Sym::param(name) => f
                .structural
                .get(name)
                .map_or(Sym::constant(0), |s| s.lo.clone()),
            _ => Sym::constant(0),
        }
    }
    ...
    pub fn indexed(
        ...
        index_with(storage, lowered, &extents, base.span)
            .map_err(|e| format!("{e} in `{}`", f.definition.name))
    }
    ...
    fn reduce(
        ...
        let operand = if matches!(operand.kind, ExprKind::Var(_)) {
            operand
        } else {
            self.copy(operand, out)?
        };
        ...
        let dtype = if tag == ir::ReduceOp::Argmax {
            DType::I32
        } else {
            source
                .elem
                .read_dtype()
                .ok_or("reduction of an unbound element type")?
        };
    ...
    /// Fresh compiler-owned execution storage for a computed shaped value.
    pub fn materialize(
        &mut self,
        f: &mut Frame<'a>,
        e: &'a sir::Expr,
        ...
        let declared = match &e.ty {
            st::Ty::Tile(declared) | st::Ty::Tensor(declared) => declared,
            _ => return Err(format!(
                "`{}` in `{}` is not a materializable shaped value",
                e.ty, f.definition.name
            )),
        };
        let same_representation = match &e.kind {
            sir::ExprKind::Cast { expr, .. } if shaped_type(&expr.ty) => {
                expr.ty
                    .shaped()
                    .map(|s| self.elem(f, &s.elem))
                    .transpose()?
                    == Some(self.elem(f, &declared.elem)?)
            }
            _ => false,
        };
        ...
        if matches!(shaped.elem, Elem::Repr(_)) {
            return Err(format!(
                "`{}` computes a packed tile; encoding needs an explicit operation",
                f.definition.name
            ));
        }
    ...
    fn intrinsic_result(
        ...
        if matches!(shaped.elem, Elem::Repr(_)) {
            return Err(format!(
                "logical intrinsic `{op}` in `{}` cannot return a packed value without an explicit encoding operation",
                f.definition.name
            ));
        }
        let ty = Ty::Tile(shaped);
        let target = self.local("intrinsic_result", ty.clone(), span);
        let args = arguments
            .iter()
            .map(|argument| self.operand(f, argument, out))
            .collect::<Result<Vec<_>, _>>()?;
        out.push(assign(
            target.clone(),
            Expr {
                kind: ExprKind::Intrinsic { op, args },
                ty,
                sym: None,
                span,
            },
    ...
    fn element_dtype(&self, f: &Frame<'a>, ty: &st::Ty) -> Result<DType, String> {
        match ty {
            st::Ty::Tile(s) | st::Ty::View(s) | st::Ty::Tensor(s) => self
                .elem(f, &s.elem)?
                .read_dtype()
                .ok_or_else(|| "unbound element type".to_string()),
    ...
    pub fn element(
        ...
        if !matches!(e.ty, st::Ty::Tile(_) | st::Ty::View(_) | st::Ty::Tensor(_)) {
            ...
            walk::each_expr(e, &mut |x| {
                inside |= matches!(x.kind, sir::ExprKind::Var(v) if c.group.contains(&v))
            });
            if inside {
                return Err(format!(
                    "a fused interval of `{}` has a non-elementwise dependency between its units",
                    f.definition.name
                ));
            }
        ...
        Ok(match &e.kind {
            ...
            sir::ExprKind::Cast { dtype, expr } => {
                let inner = if shaped_type(&expr.ty) {
                    self.snapshot_element(f, expr, c, pre)?
                } else {
                    self.element(f, expr, c, pre)?
                };
                coerce(inner, *dtype)
            }
            sir::ExprKind::Load(view) => self.snapshot_element(f, view, c, pre)?,
            sir::ExprKind::Decode(view) => {
                coerce(self.snapshot_element(f, view, c, pre)?, DType::F32)
            }
            sir::ExprKind::Unary { op, expr } => Expr {
                kind: ExprKind::Unary {
                    op: *op,
                    expr: Box::new(self.element(f, expr, c, pre)?),
                },
                ty,
                sym: None,
                span,
            ...
            sir::ExprKind::Binary { op, lhs, rhs } => binary(
                *op,
                self.element(f, lhs, c, pre)?,
                self.element(f, rhs, c, pre)?,
                ty,
            ),
            sir::ExprKind::Math { op, args } => {
                let args = args
                    .iter()
                    .map(|a| self.element(f, a, c, pre))
                    .collect::<Result<Vec<_>, _>>()?;
                Expr {
                    kind: ExprKind::Builtin {
                        name: math(*op),
            ...
            sir::ExprKind::Select { cond, then, els } => {
                let args = vec![
                    self.element(f, cond, c, pre)?,
                    coerce(self.element(f, then, c, pre)?, dtype),
                    coerce(self.element(f, els, c, pre)?, dtype),
                ];
                Expr {
                    kind: ExprKind::Builtin {
                        name: Builtin::Select,
    ...
    pub fn snapshot(
        ...
        Ok(match value {
            ...
            Value::Tuple(items) => Value::Tuple(
                items
                    .into_iter()
                    .map(|v| self.snapshot(v, name, out))
                    .collect::<Result<_, _>>()?,
            ),
            other => other,
        })
...
fn accessor_type(base: &Expr, name: &str) -> Result<Ty, String> {
    ...
    let Elem::Repr(repr) = &shaped.elem else {
        return Err(format!(
            "`.{name}` needs a packed tile, found element type {}",
            shaped.elem
        ));
    };
    let repr =
        crate::repr::lookup(repr).ok_or_else(|| format!("unknown representation `{repr}`"))?;
    let axis = shaped
        .packed_axis
        .ok_or("this packed tile has no packet axis left")?;
    ...
    let (extent, dtype) = if name == "scale" || name == "bias" {
        ...
        let plane = repr
            .plane(name)
            .ok_or_else(|| format!("`{}` has no physical plane `{name}`", repr.name))?;
