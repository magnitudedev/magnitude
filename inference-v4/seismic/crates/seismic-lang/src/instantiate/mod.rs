...
pub fn inner_owner_region(body: &crate::sir::Body, launch_binders: &[st::SliceId], region: &crate::sir::Region) -> bool {
    use crate::sir::{RegionSource, SliceParent, VarKind};
    if region.mode != crate::syntax::ast::RegionMode::Parallel || region.merge.is_some() || region.result.is_some() || region.source != RegionSource::Domains || region.binders.is_empty() {
        return false;
    }
    let mut refined = Vec::new();
    for binder in &region.binders {
        let Some(VarKind::Slice(slice)) = body.vars.get(*binder).map(|declared| &declared.kind) else { return false };
        let Some(SliceParent::Refine(parent)) = body.slices.get(slice.0 as usize).map(|declared| &declared.parent) else { return false };
...
pub fn instantiate(program: &Program, family: &Family, witness: &Witness) -> Result<LoweredIr, String> {
    let mut inst = Instantiation::new(program, family, witness);
    let entry = family.occurrences.first().ok_or_else(|| format!("family of `{}` has no entry occurrence", family.entry))?;
    let choice = *witness.choices.get(&entry.id).ok_or_else(|| format!("witness selects no implementation of entry `{}`", family.entry))?;
    let mut frame = inst.frame(CandidateRef { occurrence: entry.id, candidate: choice })?;
    ...
    for (ordinal, param) in definition.params.iter().enumerate() {
        ...
        let reference = match &kind {
            crate::exec::ir::VarKind::Index(atom) => context::symbol(crate::sym::Sym::atom(atom.clone()), definition.span),
            _ => crate::exec::ir::Expr { kind: crate::exec::ir::ExprKind::Var(id), ty: ty.clone(), sym: None, span: definition.span },
        };
        inst.vars.push(crate::exec::ir::Var { name: param.name.clone(), ty: ty.clone(), span: definition.span, kind });
        let slot = frame.vars.get_mut(param.var).ok_or_else(|| format!("entry parameter `{}` has no variable", param.name))?;
        *slot = Some(if matches!(ty, Ty::Scalar(_)) { Value::Scalar(reference) } else { Value::Shaped(reference) });
    ...
    for (left, a) in definition.params.iter().enumerate() {
        for (right, b) in definition.params.iter().enumerate().skip(left + 1) {
            let tensors = matches!(params[left].1, Ty::Tensor(_)) && matches!(params[right].1, Ty::Tensor(_));
            if tensors && (a.mode != Mode::In || b.mode != Mode::In) {
                let exact_allowed = definition.aliases.iter().any(|&(x, y)| (x, y) == (left, right) || (y, x) == (left, right));
                alias_requirements.push(AliasRequirement { left, right, exact_allowed });
            }
        }
    }
    ...
    let ir = LoweredIr {
        ...
        vars: inst.vars,
        body: prologue,
        shapes: family.workload.shapes.iter().map(|(k, v)| (k.clone(), *v)).collect(),
    };
...
export admit fn sum_squares[R, W](x: tensor[R, W] T, out y: tensor[R, 1] f32):
    parallel [rows] in 0..R:
        for row in rows:
            ...
            merge (left, right) identity f32(0.0):
                yield left + right
            var t = zeros_like(y[row], dtype=f32)
            for i in owned(t):
                t[i] = total
            publish t to y[row]
lower sum_squares for cpu = portable
";

    fn workload(shapes: &[(&str, i64)], elems: &[&str]) -> Workload {
        Workload {
            shapes: shapes.iter().map(|(k, v)| (k.to_string(), *v)).collect(),
            elems: elems.iter().map(|k| (k.to_string(), crate::types::Elem::Dtype(DType::F32))).collect(),
            ..Workload::default()
        }
    }
    ...
    fn build(sources: &[&str], entry: &str, workload: &Workload, numbers: &[i64]) -> (Family, LoweredIr) {
        let files: Vec<SourceFile> = sources
            .iter()
            .enumerate()
            .map(|(i, text)| SourceFile { path: format!("test{i}.seismic.portable"), text: text.to_string(), scope: Scope::Portable })
            .collect();
        let program = compile(&files, &["cpu".to_string()]).unwrap_or_else(|d| panic!("{}", d.iter().map(|d| d.render()).collect::<Vec<_>>().join("\n")));
        ...
        while let Some(occurrence) = pending.pop() {
            ...
            for site in &candidate.sites {
                witness.sites.insert(*site, *numbers.next().expect("a number per active site"));
            }
            for sequence in &candidate.sequences {
                let units = family.sequences[sequence.0 as usize].units.len() as u32;
                witness.covers.insert(*sequence, (0..units).map(|u| (u, u + 1)).collect());
            }
        }
    ...
    fn any(body: &[Stmt], test: &dyn Fn(&Stmt) -> bool) -> bool {
        body.iter().any(|s| {
            test(s)
                || match &s.kind {
                    StmtKind::Parallel { body, .. } | StmtKind::Owned { body, .. } | StmtKind::Range { body, .. } => any(body, test),
    ...
    fn linear_is_one_launch_over_the_selected_pieces() {
        let (family, ir) = build(&[MATMUL, LINEAR], "linear", &workload(&[("M", 2), ("N", 8), ("K", 128)], &["T", "U", "V"]), &[1, 4, 64]);
        assert!(family.sites.iter().all(|s| matches!(s.kind, SiteKind::Width { .. })));
        let [Stmt { kind: StmtKind::Parallel { extents, body, .. }, .. }] = ir.body.as_slice() else { panic!("{:#?}", ir.body) };
        assert_eq!(extents.iter().map(|e| e.as_constant()).collect::<Vec<_>>(), vec![Some(2), Some(2)]);
        // The ordered K traversal is a loop over 128 / 64 windows inside each owner.
        assert!(any(body, &|s| matches!(&s.kind, StmtKind::Range { hi, .. } if hi.as_constant() == Some(2))));
        assert!(any(body, &|s| matches!(&s.kind, StmtKind::Expr(e) if matches!(e.kind, ExprKind::Builtin { name: Builtin::Store, .. }))));
        assert_eq!(ir.alias_requirements.len(), 2);
    }

    #[test]
    fn rms_norm_reduces_and_publishes_each_row_inside_its_owner() {
        let (_, ir) = build(&[RMS_NORM], "rms_norm", &workload(&[("R", 6), ("W", 16)], &["T", "U", "V"]), &[3]);
        let [Stmt { kind: StmtKind::Parallel { extents, body, .. }, .. }] = ir.body.as_slice() else { panic!("{:#?}", ir.body) };
        assert_eq!(extents[0].as_constant(), Some(2));
        assert!(any(body, &|s| matches!(&s.kind, StmtKind::Assign { value, .. } if matches!(value.kind, ExprKind::Builtin { name: Builtin::Reduce, .. }))));
        assert!(any(body, &|s| matches!(&s.kind, StmtKind::Assign { value, .. } if matches!(value.kind, ExprKind::Load { .. }))));
    ...
    fn nested_merge_is_the_adjacent_pair_recurrence_over_the_selected_parts() {
        ...
        let (_, ir) = build(&[SUM_SQUARES], "sum_squares", &workload(&[("R", 2), ("W", 12)], &["T"]), &[1, 4]);
        let [Stmt { kind: StmtKind::Parallel { body, .. }, .. }] = ir.body.as_slice() else { panic!("{:#?}", ir.body) };
        // Four partials: a level of two pairs, then a level of one pair.
        for pairs in [2, 1] {
            assert!(any(body, &|s| matches!(&s.kind, StmtKind::Range { hi, body, .. } if hi.as_constant() == Some(pairs) && !body.is_empty())));
        }
        assert!(any(body, &|s| matches!(&s.kind, StmtKind::Assign { value, .. } if matches!(&value.kind, ExprKind::TileAlloc { shape, .. } if shape[0].as_constant() == Some(4)))));
    }
}
