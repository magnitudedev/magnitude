...
use crate::span::Span;
use crate::sir::{self, CallId, CallSite, CandidateBinding, DefId, DefKind, Expr, ExprKind, Math, ReduceOp, VarKind};
...
impl<'a> Checker<'a> {
    pub fn call(&mut self, callee: &ast::Expr, bindings: &[(ast::Ident, ast::Expr)], args: &[ast::Arg], expected: Option<&Ty>, span: Span) -> Option<Expr> {
        let name = match &callee.kind {
            ...
            A::Attr { base, name } => {
                let namespace = match &base.kind {
                    ...
                    _ => {
                        self.error(callee.span, "only named functions and `target.intrinsic` operations can be called");
                        return None;
                    }
                };
        ...
        if self.lookup(&name.name).is_some() {
            self.error(name.span, format!("`{}` is a value, not a function", name.name));
            return None;
        }
        let toolchain = DType::from_name(&name.name).is_some()
            || math_of(&name.name).is_some()
            || matches!(name.name.as_str(), "load" | "decode" | "zeros_like" | "ones_like" | "select" | "reduce" | "reshape" | "extent" | "coord" | "capacity" | "valid" | "atomic" | "owned" | "axis" | "lanes" | "full");
        if toolchain && !bindings.is_empty() {
            self.error(span, format!("`{}` is a toolchain operation and takes no shape bindings", name.name));
            return None;
        }
        ...
        let positional = |c: &mut Checker, n: usize| -> bool {
            ...
            if !ok {
                c.error(span, format!("`{}` takes {n} positional argument(s)", name.name));
            }
            ok
        };
        match name.name.as_str() {
            "load" => {
                ...
                let (Ty::Tensor(s) | Ty::View(s)) = &v.ty else {
                    self.error(v.span, format!("`load` snapshots a view in its own representation, found {}", v.ty));
                    return None;
                };
                let ty = Ty::Tile(s.clone());
                Some(Expr { partial: v.partial, kind: ExprKind::Load(Box::new(v)), ty, sym: None, span })
            }
            "decode" => {
                if !positional(self, 1) {
                    return None;
                ...
                Some(Expr { partial: v.partial, kind: ExprKind::Decode(Box::new(v)), ty, sym: None, span })
            }
            "zeros_like" | "ones_like" => {
                let (Some(like), dtype) = (args.first().filter(|a| a.name.is_none()), args.get(1)) else {
                    self.error(span, format!("`{}(v, dtype=f32)` takes a shaped value and an optional dtype", name.name));
                    return None;
                };
                if args.len() > 2 {
                    self.error(span, format!("`{}(v, dtype=f32)` takes a shaped value and an optional dtype", name.name));
                ...
                let Some(s) = like.ty.shaped().cloned() else {
                    self.error(like.span, format!("`{}` takes the shape of a tensor, view or tile, found {}", name.name, like.ty));
                    return None;
                };
                let dtype = match dtype {
                    Some(ast::Arg { name: Some(label), value }) if label.name == "dtype" => match &value.kind {
                ...
                Some(Expr { kind: ExprKind::Filled { like: Box::new(like), value }, ty, sym: None, partial: false, span })
            }
            "select" => {
                ...
                if dtypes[0] != DType::Bool {
                    self.error(cond.span, format!("the first operand of `select` is a mask (`bool`), found {}", dtypes[0].name()));
                    return None;
                }
                let Some(dtype) = DType::promote(dtypes[1], dtypes[2]) else {
                    self.error(span, format!("`select` between {} and {} needs an explicit cast", dtypes[1].name(), dtypes[2].name()));
                    return None;
                };
                let ty = self.elementwise(shape, dtype);
                Some(Expr { kind: ExprKind::Select { cond: Box::new(cond), then: Box::new(then), els: Box::new(els) }, ty, sym: None, partial: false, span })
            }
            "reduce" => {
                let unordered = match args.get(3) {
                    Some(ast::Arg { name: Some(label), value: ast::Expr { kind: A::Bool(value), .. } }) if label.name == "unordered" && args.len() == 4 => Some(*value),
                    None => Some(false),
                    _ => None,
                };
                let Some(unordered) = unordered.filter(|_| args.len() >= 3 && args[..3].iter().all(|a| a.name.is_none())) else {
            ...
            "extent" | "capacity" | "valid" => {
                ...
                let Some(s) = base.ty.shaped().cloned() else {
                    self.error(base.span, format!("`{}` needs a tensor, view or tile, found {}", name.name, base.ty));
                    return None;
                };
                ...
                let sym = match (&s.axes[axis], geometry) {
                    ...
                    (Extent::Structural(slice), true) => {
                        ...
                        self.facts.set_range_lower(capacity.clone(), Sym::constant(1));
                        if name.name == "capacity" {
                            Sym::atom(capacity)
                        } else {
                            let valid = Atom::Param(format!("@valid#{}", slice.0));
                            self.facts.set_range(valid.clone(), Sym::constant(1), Sym::atom(capacity));
                            Sym::atom(valid)
                        }
                    }
                };
                let kind = if geometry { ExprKind::Geometry { base: Box::new(base), axis, valid: name.name == "valid" } } else { ExprKind::ExtentOf { base: Box::new(base), axis } };
            ...
            "coord" => {
                ...
                let var = match &args[0].value.kind {
                    A::Name(n) => self.lookup(&n.name).filter(|id| self.vars[*id].kind == VarKind::Coordinate),
                    _ => None,
                };
                let Some(var) = var else {
                    self.error(span, "`coord(i)` takes a tile coordinate bound by `owned` or `axis`");
                    return None;
                };
                ...
                let sym = if generic { None } else { self.atoms.get(&var).map(|a| Sym::atom(a.clone())) };
            ...
            "atomic" => {
                ...
                let Ty::Scalar(dtype) = place.ty else {
                    self.error(place.span, format!("the `atomic` place selects one element, found {}", place.ty));
                    return None;
                };
                let value = self.expr(&args[2].value, Some(&Ty::Scalar(dtype)))?;
                if value.ty.scalar_dtype() != Some(dtype) {
                    self.error(value.span, format!("the `atomic` value must be {}, found {}", dtype.name(), value.ty));
                    return None;
                }
                self.forbid_partial(&value, "an atomic update");
                let root = self.root_var(&place)?;
                if !matches!(self.vars[root].kind, VarKind::Param(i) if self.sig.params[i].mode != Mode::In) && self.vars[root].kind != VarKind::State {
                    self.error(place.span, "an `atomic` place is an `out`/`inout` parameter or local state");
                    return None;
                }
                self.published.insert(root);
                self.mutated.push(root);
                Some(Expr { kind: ExprKind::Atomic { op, place: Box::new(place), value: Box::new(value) }, ty: Ty::Void, sym: None, partial: false, span })
            }
            "owned" | "axis" | "lanes" => {
                self.error(span, format!("`{}` is an iterator and appears only in `for`", name.name));
                None
            }
            "full" => {
    ...
    fn math(&mut self, op: Math, arity: usize, name: &str, args: &[ast::Arg], expected: Option<&Ty>, span: Span) -> Option<Expr> {
        if args.len() != arity || args.iter().any(|a| a.name.is_some()) {
            self.error(span, format!("`{name}` takes {arity} positional argument(s)"));
            return None;
        }
        let float_only = !matches!(op, Math::Max | Math::Min | Math::Abs);
        let default = Ty::Scalar(DType::F32);
        let mut hint: Option<Ty> = expected.cloned().or_else(|| float_only.then(|| default.clone()));
        ...
        for d in &dtypes[1..] {
            match DType::promote(dtype, *d) {
                Some(p) => dtype = p,
                None => {
                    self.error(span, format!("`{name}` between {} and {} needs an explicit cast", dtype.name(), d.name()));
                    return None;
                }
            }
        }
        if if float_only { !dtype.is_float() } else { !dtype.is_numeric() } {
            self.error(span, format!("`{name}` needs {} operands, found {}", if float_only { "float" } else { "numeric" }, dtype.name()));
            return None;
        }
        let ty = self.elementwise(shape, dtype);
        Some(Expr { kind: ExprKind::Math { op, args: out }, ty, sym: None, partial: false, span })
    }
    ...
    pub fn math_exprs(&mut self, op: Math, l: Expr, r: Expr, span: Span) -> Option<Expr> {
        ...
        let Some(dtype) = DType::promote(dtypes[0], dtypes[1]).filter(|d| d.is_numeric()) else {
            self.error(span, format!("`max`/`min` between {} and {} needs an explicit cast", dtypes[0].name(), dtypes[1].name()));
            return None;
        };
        let ty = self.elementwise(shape, dtype);
        Some(Expr { kind: ExprKind::Math { op, args: vec![l, r] }, ty, sym: None, partial: false, span })
    }
    ...
    fn reduce(&mut self, args: &[ast::Arg], unordered: bool, span: Span) -> Option<Expr> {
        ...
        let Ty::Tile(s) = &t.ty else {
            self.error(t.span, format!("`reduce` reduces a dense tile, found {}; read views with `load` or a cast", t.ty));
            return None;
        };
        let Some(dtype) = (match &s.elem {
            ...
            self.error(t.span, "`reduce` reduces a dense tile; decode packed values first");
            return None;
        };
        let axis = self.constant_axis(&args[1].value, s.rank())?;
        ...
        let elem = if op == ReduceOp::Argmax { DType::I32 } else { dtype };
        let mut axes = s.axes.clone();
        axes.remove(axis);
        let ty = if axes.is_empty() { Ty::Scalar(elem) } else { Ty::Tile(Shaped::new(axes, Elem::Dtype(elem))) };
        Some(Expr { kind: ExprKind::Reduce { value: Box::new(t), axis, op, unordered }, ty, sym: None, partial, span })
    }

    fn reshape(&mut self, args: &[ast::Arg], span: Span) -> Option<Expr> {
        let base = self.expr_inner(&args[0].value, None, true)?;
        let Some(s) = base.ty.shaped().cloned() else {
            self.error(base.span, format!("`reshape` needs a tensor, view or tile, found {}", base.ty));
            return None;
        };
        ...
        let A::Tuple(dimensions) = &args[1].value.kind else {
            self.error(args[1].value.span, "the `reshape` target is a tuple of shape expressions");
            return None;
        };
        ...
        let ty = if matches!(base.ty, Ty::Tensor(_)) { Ty::Tensor(shaped) } else { Ty::View(shaped) };
        Some(Expr { partial: base.partial, kind: ExprKind::Reshape { base: Box::new(base), axes }, ty, sym: None, span })
    }

    // ---- target intrinsics ----

    fn intrinsic(&mut self, namespace: &ast::Ident, name: &ast::Ident, args: &[ast::Arg], span: Span) -> Option<Expr> {
        let Some(table) = intrinsics::table(&namespace.name) else {
            self.error(namespace.span, format!("`{}` is not a value or a target namespace", namespace.name));
            return None;
        };
        let Some(intrinsic) = table.into_iter().find(|i| i.operation.name() == name.name) else {
            self.error(name.span, format!("target `{}` has no intrinsic `{}`", namespace.name, name.name));
            return None;
        };
        if !self.target_form(span, &format!("`{}.{}`", namespace.name, name.name), Some(&namespace.name)) {
            return None;
        }
        self.intrinsic_call(&namespace.name, &intrinsic, args, span)
    }

    fn intrinsic_call(&mut self, target: &str, intrinsic: &Intrinsic, args: &[ast::Arg], span: Span) -> Option<Expr> {
        let operation = intrinsic.operation;
        if args.len() != intrinsic.params.len() || args.iter().any(|a| a.name.is_some()) {
            self.error(span, format!("`{target}.{operation}` takes {} positional argument(s)", intrinsic.params.len()));
            return None;
        }
            ...
            match param {
                ...
                IntrinsicParam::FloatScalar => {
                    ...
                    let Some(d) = e.ty.scalar_dtype().filter(|d| d.is_float()) else {
                        self.error(e.span, format!("`{target}.{operation}` needs a float scalar, found {}", e.ty));
                        return None;
                    };
                    float_dtype = Some(d);
                    out.push(e);
                ...
                IntrinsicParam::Tile2 => {
                    ...
                    let e = self.expr_inner(&arg.value, None, operation == Operation::MatrixStore)?;
                    if !matches!(&e.ty, Ty::Tile(s) | Ty::View(s) if s.rank() == 2) {
                        self.error(e.span, format!("`{target}.{operation}` needs a rank-2 tile or view, found {}", e.ty));
                        return None;
                    }
                    out.push(e);
                }
                ...
                IntrinsicParam::Int => {
                    let e = self.expr(&arg.value, Some(&Ty::Scalar(DType::I32)))?;
                    if e.sym.is_none() {
                        self.error(e.span, format!("`{target}.{operation}` needs a symbolic integer offset"));
                        return None;
                    }
                    out.push(e);
                }
        ...
        if let Semantics::Load { rows, columns, .. } | Semantics::Store { rows, columns } = operation.semantics() {
            let axes = out[1].ty.shaped().map(|s| s.axes.clone()).unwrap_or_default();
            for (k, (axis, window)) in axes.iter().zip([rows, columns]).enumerate() {
                let (Extent::Semantic(dim), Some(offset)) = (axis, out[2 + k].sym.clone()) else { continue };
                ...
                self.require_nonneg(&dim.sub(&interval.hi).sub(&Sym::constant(window as i64)), offset_span, &format!("{window}-wide block may exceed extent `{dim}`"));
            }
        }
        for written in operation.writes_arguments() {
            ...
            if operation == Operation::MatrixStore {
                let covers = place.ty.shaped().is_some_and(|s| s.axes.iter().all(|a| a.semantic().and_then(Sym::as_constant) == Some(8))) && out[2..4].iter().all(|e| e.sym.as_ref().and_then(Sym::as_constant) == Some(0));
        ...
        let ty = match intrinsic.result {
            ...
            IntrinsicResult::Frag8x8OfNamedDtype => {
                let Semantics::Fragment { rows, columns } = operation.semantics() else {
                    self.error(span, format!("`{target}.{operation}` does not declare a native fragment"));
                    return None;
                };
                out.clear();
                Ty::Native(NativeTy { target: target.to_string(), name: operation.name().to_string(), shape: vec![Sym::constant(rows as i64), Sym::constant(columns as i64)], elem: Some(Elem::Dtype(named_dtype.unwrap_or(DType::F32))) })
            }
        };
        Some(Expr { kind: ExprKind::Intrinsic { op: operation, args: out }, ty, sym: None, partial: false, span })
    }
    ...
    fn arg_order(sig: &Sig, args: &[ast::Arg]) -> Result<Vec<usize>, String> {
        if args.len() != sig.params.len() {
            return Err(format!("takes {} arguments, {} given", sig.params.len(), args.len()));
        }
        ...
        if let Some(ordinal) = into {
            let open: Vec<usize> = (0..order.len()).filter(|i| order[*i].is_none() && sig.params[*i].mode != Mode::In).collect();
            let [slot] = open.as_slice() else {
                return Err("`into=` binds the one unbound `out`/`inout` parameter".to_string());
            };
            order[*slot] = Some(ordinal);
        }
        order.into_iter().collect::<Option<Vec<usize>>>().ok_or_else(|| "leaves a parameter unbound".to_string())
    }
    ...
    fn substitute(s: &Sym, binding: &Binding) -> Result<Sym, String> {
        for p in s.params() {
            match binding.shapes.get(&p) {
                ...
                Some(Extent::Structural(_)) => return Err(format!("`{s}` computes with `{p}`, which is bound to a structural extent")),
                None => return Err(format!("shape parameter `{p}` is not determined by the arguments")),
            }
        }
    ...
    fn substitute_ty(ty: &Ty, binding: &Binding) -> Result<Ty, String> {
        let shaped = |s: &Shaped| -> Result<Shaped, String> {
            ...
            for axis in &s.axes {
                axes.push(match axis {
                    Extent::Semantic(sym) => match single_param(sym).and_then(|p| binding.shapes.get(&p)) {
            ...
            let elem = match &s.elem {
                Elem::Param(p) => binding.elems.get(p).cloned().ok_or_else(|| format!("element parameter `{p}` is not determined by the arguments"))?,
                other => other.clone(),
            };
        ...
        Ok(match ty {
            ...
            Ty::Tuple(items) => Ty::Tuple(items.iter().map(|t| Self::substitute_ty(t, binding)).collect::<Result<_, _>>()?),
            other => other.clone(),
        })
    }

    fn unify(&self, param: &Ty, arg: &Ty, binding: &mut Binding, compound: &mut Vec<(Sym, Extent)>) -> Result<(), String> {
        ...
        match (param, arg) {
            ...
            (Ty::Tensor(p), Ty::Tensor(a) | Ty::View(a)) | (Ty::View(p), Ty::Tensor(a) | Ty::View(a) | Ty::Tile(a)) | (Ty::Tile(p), Ty::Tile(a)) => {
                if p.rank() != a.rank() {
                    return mismatch();
                }
                for (pd, ad) in p.axes.iter().zip(&a.axes) {
                    let Extent::Semantic(pd) = pd else { return mismatch() };
                ...
                let ok = match (&p.elem, &a.elem) {
                    ...
                    (concrete, Elem::Param(own)) => match binding.requires.iter().find(|(p, _)| p == own) {
                ...
                let packets_aligned = !matches!(a.elem, Elem::Repr(_)) || a.packed_axis == Some(a.rank() - 1);
                if ok && packets_aligned { Ok(()) } else { mismatch() }
            }
            (Ty::Tuple(p), Ty::Tuple(a)) if p.len() == a.len() => p.iter().zip(a).try_for_each(|(p, a)| self.unify(p, a, binding, compound)),
    ...
    fn bind_candidate(&self, sig: &Sig, explicit: &[(String, Extent)], ast_args: &[ast::Arg], args: &[Expr]) -> Result<(Vec<usize>, Binding), String> {
        ...
        for (param, ordinal) in sig.params.iter().zip(&order) {
            ...
            let sub_tile = match (&param.ty, &argument.ty) {
                (Ty::Tile(_), Ty::View(shaped)) if param.mode != Mode::In && self.root_var(argument).is_some_and(|root| matches!(self.vars[root].kind, VarKind::State) && matches!(self.vars[root].ty, Ty::Tile(_))) => Some(Ty::Tile(shaped.clone())),
                _ => None,
            };
            self.unify(&param.ty, sub_tile.as_ref().unwrap_or(&argument.ty), &mut binding, &mut compound).map_err(|e| format!("parameter `{}` {e}", param.name))?;
        }
        ...
        loop {
            ...
            for (pd, ad) in &compound {
                ...
                let unknown: Vec<String> = pd.params().into_iter().filter(|p| !binding.shapes.contains_key(p)).collect();
                let [u] = unknown.as_slice() else { continue };
                let hole = Atom::Param(format!("?{u}"));
                let mut trial = Binding { shapes: binding.shapes.clone(), ..Binding::default() };
                trial.shapes.insert(u.clone(), Extent::Semantic(Sym::atom(hole.clone())));
                let Ok(e) = Self::substitute(pd, &trial) else { continue };
                let Some((c, rest)) = e.linear_in(&hole) else { continue };
        ...
        if let Some(p) = sig.shape_params.iter().find(|p| !binding.shapes.contains_key(*p)) {
        ...
        for (pd, ad) in &compound {
            ...
            if !self.same_extent(&Extent::Semantic(expected.clone()), ad) {
                return Err(format!("extent `{pd}` is `{expected}` under this binding but the argument has `{ad}`"));
            }
        }
        // Index parameters: symbolic arguments are proved inside the bound; data-dependent
        // arguments keep a runtime obligation.
        for (param, ordinal) in sig.params.iter().zip(&order) {
            let (Ty::Index(bound), Some(value)) = (&param.ty, &args[*ordinal].sym) else { continue };
            let bound = Self::substitute(bound, &binding)?;
            let data_dependent = value.params().iter().any(|p| !self.sig.shape_params.contains(p) && self.facts.upper_of(&Atom::Param(p.clone())).is_none());
            let proved = self.prover().nonneg(value) && self.prover().lt(value, &bound);
            if !proved && !data_dependent {
                return Err(format!("parameter `{}` needs `0 <= {value} < {bound}`, which is not provable here", param.name));
            }
        }
        Ok((order, binding))
    }

    fn user_call(&mut self, name: &ast::Ident, bindings: &[(ast::Ident, ast::Expr)], ast_args: &[ast::Arg], span: Span) -> Option<Expr> {
        ...
        let mut definitions: Vec<usize> = families.iter().flat_map(|f| {
            let family = &resolved.families[*f];
            family.bodies.iter().chain(&family.contracts).chain(&family.lowerings).map(|id| id.0 as usize)
        }).collect();
        definitions.sort_unstable();
        if matches!(self.kind, DefKind::Lower { .. }) && families.contains(&resolved.declared[self.def].family) {
            self.error(span, format!("a lowering of `{}` cannot call the contract it implements; `= portable` adopts the portable bodies instead", name.name));
            return None;
        }
        ...
        for (param, value) in bindings {
            let identity = match &value.kind {
                A::Name(n) => match self.lookup(&n.name).map(|id| self.vars[id].ty.clone()) {
                    ...
                    None if self.sig.shape_params.contains(&n.name) => Some(Extent::Semantic(Sym::param(&n.name))),
                    _ => None,
                },
                _ => None,
            };
            let extent = match identity {
                ...
                None => {
                    ...
                    let Some(sym) = value.sym.clone() else {
                        self.error(value.span, "a shape binding is a slice or a symbolic integer expression");
                        return None;
                    };
                    Extent::Semantic(sym)
                }
        ...
        let guide = definitions.iter().map(|d| &resolved.declared[*d].sig).find_map(|sig| Self::arg_order(sig, ast_args).ok().map(|order| (sig, order)));
        let mut args = Vec::new();
        for (ordinal, arg) in ast_args.iter().enumerate() {
            let param = guide.as_ref().and_then(|(sig, order)| order.iter().position(|o| *o == ordinal).map(|slot| &sig.params[slot]));
        ...
        let Some(family) = candidates.first().map(|(d, _, _)| resolved.declared[*d].family) else {
            reasons.dedup();
            self.error(span, format!("no definition of `{}` accepts these arguments: {}", name.name, reasons.join("; ")));
            return None;
        };
        ...
        if self.env.enforce {
            ...
            candidates.retain(|(d, _, binding)| {
                ...
                let offending: Vec<&String> = binding.shapes.iter().filter(|(p, extent)| matches!(extent, Extent::Structural(_)) && numeric.contains(*p)).map(|(p, _)| p).collect();
                for p in &offending {
                    rejected.push(format!("`{p}`"));
                }
                offending.is_empty()
            });
            let implemented = candidates.iter().any(|(d, _, _)| matches!(resolved.declared[*d].kind, DefKind::Body | DefKind::Lower { .. }));
        ...
        let Some((first, first_order, first_binding)) = candidates.first() else {
            self.error(span, format!("no definition of `{}` remains applicable to these arguments", name.name));
            return None;
        };
        ...
        let result = match Self::substitute_ty(&first_sig.result, first_binding) {
            Ok(ty) => ty,
            Err(reason) => {
                self.error(span, format!("the result of `{}` is not expressible at this call: {reason}", name.name));
                return None;
            }
        };

        // Effects of `out`/`inout` parameters.
        let modes: Vec<(Mode, usize)> = first_sig.params.iter().zip(first_order).map(|(p, o)| (p.mode, *o)).collect();
        for (mode, ordinal) in modes {
            let arg = args[ordinal].clone();
            if !callee_admit {
                self.forbid_partial(&arg, &format!("an argument of `{}`, which is not an `admit fn`", name.name));
            }
            if mode == Mode::In {
                continue;
            }
        ...
        for (d, order, binding) in &candidates {
            ...
            for p in &sig.shape_params {
                let Some(extent) = binding.shapes.get(p) else { continue };
                match extent {
                    Extent::Structural(_) => partial |= !callee_admit && summary.reduces.contains(p),
                    Extent::Semantic(sym) => {
                        if let Some(own) = single_param(sym).filter(|q| self.sig.shape_params.contains(q)) {
            ...
            let elem_args = sig.elem_params.iter().filter_map(|p| binding.elems.get(p).map(|e| (p.clone(), e.clone()))).collect();
            site.push(CandidateBinding { definition: DefId(*d as u32), shape_args, elem_args, arg_order: order.clone(), requires_elems: binding.requires.clone() });
        }
        let call = CallId(self.calls.len() as u32);
        self.calls.push(CallSite { family, bindings: site, result: result.clone(), span });
        Some(sir::Expr { kind: ExprKind::Call { call, args }, ty: result, sym: None, partial, span })
    }
}
