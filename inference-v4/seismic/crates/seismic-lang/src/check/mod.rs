//! The checker: resolution, overload families and lowering boundaries, typing of every
...
use crate::sir::{self, CallSite, DefKind, Predicate, RegionDecl, SliceDecl, SliceParent, Var, VarId, VarKind};
...
pub(crate) enum FrameKind {
    Region { id: RegionId, mode: RegionMode, origin: Option<RegionId> },
    Stage { pipeline: bool },
    Merge,
}
...
pub(crate) struct Checker<'a> {
    ...
    pub kind: DefKind,
    pub admit: bool,
    /// The target whose forms this body may name: the lowering's target or the file's scope.
...
impl<'a> Checker<'a> {
    fn new(env: &'a Env<'a>, def: usize) -> Checker<'a> {
        ...
        let target = match (&declared.kind, &declared.scope) {
            (DefKind::Lower { target }, _) => Some(target.clone()),
            (_, Scope::Backend(b)) => Some(b.clone()),
            _ => None,
        };
        let mut c = Checker {
            env,
        ...
        for p in &c.sig.shape_params {
            let admits_zero = c.sig.predicates.iter().any(|q| matches!(q, Predicate::NonNegative(e) if *e == Sym::param(p)));
            c.facts.set_range_lower(Atom::Param(p.clone()), Sym::constant(if admits_zero { 0 } else { 1 }));
        }
    ...
    pub fn declare(&mut self, name: &str, ty: Ty, span: Span, kind: VarKind) -> VarId {
        ...
        self.vars.push(Var { name: name.to_string(), ty, kind, partial: false, span });
    ...
    pub fn same_axes(&self, a: &Shaped, b: &Shaped) -> bool {
        a.rank() == b.rank() && a.axes.iter().zip(&b.axes).all(|(x, y)| self.same_extent(x, y))
    }

    pub fn same_ty(&self, a: &Ty, b: &Ty) -> bool {
        match (a, b) {
            (Ty::Tensor(x), Ty::Tensor(y)) | (Ty::View(x), Ty::View(y)) | (Ty::Tile(x), Ty::Tile(y)) => x.elem == y.elem && self.same_axes(x, y),
            (Ty::Index(x), Ty::Index(y)) => self.prover().zero(&x.sub(y)),
            (Ty::Tuple(x), Ty::Tuple(y)) => x.len() == y.len() && x.iter().zip(y).all(|(p, q)| self.same_ty(p, q)),
    ...
    pub fn assignable(&self, target: &Ty, value: &Ty) -> bool {
        match (target, value) {
            (Ty::Scalar(a), _) => value.scalar_dtype().is_some_and(|b| *a == b || (a.is_float() && b.is_float())),
            (Ty::Tile(a), Ty::Tile(b)) => self.same_axes(a, b) && elem_rounds(&b.elem, &a.elem),
            (Ty::Tuple(a), Ty::Tuple(b)) => a.len() == b.len() && a.iter().zip(b).all(|(x, y)| self.assignable(x, y)),
    ...
    pub fn rebind_ty(&self, ty: &Ty, map: &[(SliceId, SliceId)]) -> Ty {
        let slice = |s: SliceId| map.iter().find(|(from, _)| *from == s).map_or(s, |(_, to)| *to);
        let shaped = |s: &Shaped| Shaped {
            axes: s.axes.iter().map(|a| match a {
        ...
        match ty {
            ...
            Ty::Result(r) => Ty::Result(Box::new(ResultTy { origin: r.origin, producer: r.producer, binders: r.binders.clone(), member: self.rebind_ty(&r.member, map) })),
            other => other.clone(),
        }
    }
    ...
    pub fn target_form(&mut self, span: Span, what: &str, namespace: Option<&str>) -> bool {
        let Some(target) = self.target.clone() else {
            self.error(span, format!("{what} is target-dependent (language.md section 6) and cannot appear in a portable body; write it in a `lower … for <target>` body or a target file"));
            return false;
        };
        if namespace.is_some_and(|ns| ns != target) {
            self.error(span, format!("{what} belongs to target `{}` but this body is for `{target}`", namespace.unwrap_or_default()));
            return false;
        }
        self.requires_target = Some(target);
        true
    }
    ...
    pub fn forbid_partial(&mut self, e: &sir::Expr, use_: &str) {
        if e.partial && !self.partial_free() {
            self.error(e.span, format!("partial-domain value used as {use_}: outside an `admit fn` a value yielded per slice or reduced over a structural axis may only be forwarded, stored in results, combined by `merge`, accumulated into `var` state by `+`, `max`, `min` inside a traversal of the same result, or passed to an `admit fn`"));
        }
    }
    ...
    pub fn root_var(&self, e: &sir::Expr) -> Option<VarId> {
        match &e.kind {
            ...
            sir::ExprKind::Index { base, .. } | sir::ExprKind::Transpose(base) | sir::ExprKind::Reshape { base, .. } | sir::ExprKind::Accessor { base, .. } => self.root_var(base),
            _ => None,
        }
    }
    ...
    fn selecting_slices(&self, e: &sir::Expr, out: &mut HashSet<SliceId>) {
        fn mentions(c: &Checker, e: &sir::Expr, out: &mut HashSet<SliceId>) {
            match &e.kind {
                sir::ExprKind::Var(v) | sir::ExprKind::CoordOf(v) => match (&c.vars[*v].kind, &c.vars[*v].ty) {
                    (VarKind::SliceMember(s), _) | (_, Ty::Coord(s)) | (_, Ty::Slice(s)) => c.with_parents(*s, out),
                    _ => {}
                },
                ...
                sir::ExprKind::Unary { expr, .. } | sir::ExprKind::Cast { expr, .. } => mentions(c, expr, out),
                _ => {}
            }
        }
        match &e.kind {
            sir::ExprKind::Index { base, indices } => {
                for index in indices {
                    match index {
                        ...
                        sir::Index::Range { start, end } => start.iter().chain(end).for_each(|b| mentions(self, b, out)),
                        _ => {}
                    }
                }
            ...
            sir::ExprKind::Transpose(base) | sir::ExprKind::Reshape { base, .. } | sir::ExprKind::Accessor { base, .. } => self.selecting_slices(base, out),
            _ => {}
        }
    }
    ...
    pub fn write(&mut self, place: &sir::Expr, span: Span, whole: bool) -> Option<VarId> {
        let Some(root) = self.root_var(place) else {
            self.error(span, "a write needs a place: an `out`/`inout` parameter, local `var` state, or a view of one");
            return None;
        };
        let name = self.vars[root].name.clone();
        match &self.vars[root].kind {
            VarKind::Param(i) if self.sig.params[*i].mode == Mode::In => {
                self.error(span, format!("`{name}` is a read-only parameter; writing requires `out` or `inout`"));
                return None;
            }
            VarKind::Param(_) | VarKind::State => {}
            VarKind::Value => {
                self.error(span, format!("`{name}` is an immutable `let` value; mutable state is declared with `var`"));
                return None;
            }
            _ => {
        ...
        for frame in self.frames.clone().iter().rev().filter(|f| f.floor > root) {
            match &frame.kind {
                ...
                FrameKind::Region { id, mode: RegionMode::Parallel, .. } => {
                    if whole {
        ...
        self.dyn_slices.retain(|(start, end, _, _)| !tensor_effect && !start.iter().chain(end).any(|b| expr::mentions_var(b, root)));
        Some(root)
    }

    // ---- result ----

    fn finish(mut self, block: sir::Block, span: Span) -> (Option<sir::Body>, Option<String>, Summary, Vec<Diagnostic>) {
        ...
        for (i, p) in self.sig.params.iter().enumerate().filter(|_| well_formed) {
            ...
            let Some(id) = self.vars.iter().position(|v| v.kind == VarKind::Param(i)) else { continue };
        ...
        if well_formed && self.sig.result != Ty::Void && !self.yields.first().is_some_and(|y| y.done) {
            self.error(span, format!("`{}` returns {} but not every path ends in `return`", self.sig.name, self.sig.result));
        }
        let body = sir::Body { vars: self.vars, slices: self.slices, regions: self.regions, calls: self.calls, block };
        (Some(body), self.requires_target, self.summary, self.diagnostics)
    }
}
...
struct CheckedBody {
    body: Option<sir::Body>,
    requires_target: Option<String>,
    summary: Summary,
...
fn check_definition(env: &Env, def: usize) -> CheckedBody {
    ...
    let Some(block) = declared.body else {
        return CheckedBody { body: None, requires_target: None, summary: Summary::default(), diagnostics: Vec::new() };
    };
    let mut c = Checker::new(env, def);
    c.yields.push(YieldCtx { kind: YieldKind::Function, done: false, loops: 0, schema: None, partials: Vec::new(), failed: false });
    let stmts = c.block(block);
    let (body, requires_target, summary, diagnostics) = c.finish(stmts, declared.name_span);
    CheckedBody { body, requires_target, summary, diagnostics }
}
    ...
    loop {
        ...
        for i in 0..summaries.len() {
            for (callee, callee_param, own) in summaries[i].passes.clone() {
                if summaries[callee].numeric.contains(&callee_param) && summaries[i].numeric.insert(own.clone()) {
                    changed = true;
                }
                if summaries[callee].reduces.contains(&callee_param) && summaries[i].reduces.insert(own) {
                    changed = true;
                }
            }
        }
...
pub(crate) fn check_program(files: &[(usize, Scope, ast::File)], targets: &[String], diagnostics: &mut Vec<Located>) -> (Vec<sir::Definition>, Vec<sir::ContractFamily>) {
    let resolved = resolve::resolve(files, diagnostics);
    resolve::coverage(&resolved, targets, diagnostics);
    ...
    let mut summaries = vec![Summary::default(); count];
    {
        ...
        let env = Env { resolved: &resolved, summaries: &empty, enforce: false };
        for (def, summary) in summaries.iter_mut().enumerate() {
            *summary = check_definition(&env, def).summary;
        }
    }
    close_summaries(&mut summaries);
    let env = Env { resolved: &resolved, summaries: &summaries, enforce: true };
    ...
    for (def, declared) in resolved.declared.iter().enumerate() {
        ...
        diagnostics.extend(checked.diagnostics.into_iter().map(|diagnostic| Located { file: declared.file, diagnostic }));
        let body = checked.body.unwrap_or_else(|| sir::Body { vars: Vec::new(), slices: Vec::new(), regions: Vec::new(), calls: Vec::new(), block: Vec::new() });
        // Parameters are the first variables of a checked body.
        let params = declared.sig.params.iter().enumerate().map(|(i, p)| sir::Param { name: p.name.clone(), mode: p.mode, ty: p.ty.clone(), var: i }).collect();
        definitions.push(sir::Definition {
            ...
            requires_target: checked.requires_target,
            body: declared.body.map(|_| body),
            file: declared.file,
            span: declared.span,
        });
    }
...
mod tests {
    ...
    fn check(sources: &[(&str, &str)]) -> Result<Program, String> {
        let files: Vec<SourceFile> = sources
            .iter()
            .map(|(path, text)| SourceFile { path: path.to_string(), text: text.to_string(), scope: crate::scope_of_path(path).unwrap_or(Scope::Portable) })
            .collect();
        compile(&files, &["cpu".to_string()]).map_err(|d| d.iter().map(|d| d.render()).collect::<Vec<_>>().join("\n"))
    }

    fn rejected(source: &str, rule: &str) {
        match check(&[("case.seismic.portable", source)]) {
    ...
    fn reference_sources_check() {
        let program = check(&[
            ("rms_norm.seismic.portable", include_str!("../../../../../seismic-std/lib/kernels/rms_norm.seismic.portable")),
            ("linear.seismic.portable", include_str!("../../../../../seismic-std/lib/kernels/linear.seismic.portable")),
            ("matmul.seismic.portable", include_str!("../../../../../seismic-std/lib/constructs/matmul.seismic.portable")),
            ("matmul.seismic.cpu", include_str!("../../../../../seismic-std/lib/constructs/matmul.seismic.cpu")),
        ])
        .unwrap_or_else(|e| panic!("{e}"));
        let linear = program.export("linear").expect("linear is exported");
        let body = program.definition(linear.bodies[0]).body.as_ref().expect("linear has a body");
        ...
        assert!(call.bindings[0].shape_args.iter().all(|(_, e)| matches!(e, crate::types::Extent::Structural(_))));
    ...
    fn concrete_lowering_requires_caller_elem_and_unordered_needs_admit() {
        let source = "fn mm[M, K](a: tile[M, K] T, inout into: tile[M] f32);\nlower mm[M, K](a: tile[M, K] bf16, inout into: tile[M] f32) for cpu:\n    for i in owned(into):\n        into[i] = into[i] + f32(a[i, 0])\nlower mm[M, K](a: tile[M, K] T, inout into: tile[M] f32) for cpu:\n    for i in owned(into):\n        into[i] = into[i] + f32(a[i, 0])\nadmit fn g[M, K](x: tensor[M, K] A, out y: tensor[M] f32):\n    var acc = reduce(f32(x), 1, sum, unordered=true)\n    mm(load(x), acc)\n    publish acc to y\n";
        let program = check(&[("case.seismic.cpu", source)]).unwrap_or_else(|e| panic!("{e}"));
        let g = program.definitions.iter().find(|d| d.name == "g").expect("g is defined");
        let bindings = &g.body.as_ref().expect("g has a body").calls[0].bindings;
        let concrete = bindings.iter().find(|b| !b.requires_elems.is_empty()).expect("the bf16 lowering is a provisional candidate");
        assert_eq!(concrete.requires_elems, vec![("A".to_string(), crate::types::Elem::Dtype(crate::types::DType::BF16))]);
        let generic = bindings.iter().find(|b| b.requires_elems.is_empty() && !b.elem_args.is_empty()).expect("the generic lowering binds T");
        assert_eq!(generic.elem_args, vec![("T".to_string(), crate::types::Elem::Param("A".to_string()))]);
