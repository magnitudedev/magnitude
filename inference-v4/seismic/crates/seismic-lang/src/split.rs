...
pub fn split_candidates(body: &[Stmt], vars: &[Var]) -> Vec<SplitCandidate> {
    ...
    for (i, s) in body.iter().enumerate() {
        ...
        for (j, inner) in block.iter().enumerate() {
            ...
            out.push(SplitCandidate { stmt: i, loop_at: j, carried, lo, hi });
        }
    }
    out
}
...
fn input_axis(indices: &[Index], mut axis: usize) -> usize {
    for (at, index) in indices.iter().enumerate() {
        if matches!(index, Index::Point(_)) { continue; }
        if axis == 0 { return at; }
        axis -= 1;
    }
    indices.len() + axis
...
fn range_of(view: &Expr, mut axis: usize) -> Option<(Expr, Expr)> {
    ...
    loop {
        match &node.kind {
            ...
            ExprKind::Transpose(inner) => { axis = 1usize.checked_sub(axis)?; node = inner; }
...
pub fn narrow_range(sp: &SplitCandidate, body: &mut [Stmt], part: VarId, part_atom: &Atom, parts: i64) -> Result<(), String> {
    ...
    if parts <= 0 { return Err("split part count must be positive".into()); }
    for (view_index, v) in views.iter_mut().enumerate() {
        let (lo, hi) = range_of(v, *axis).ok_or("split requires an explicit range for every streamed view")?;
    let i32_ty = Ty::Scalar(crate::types::DType::I32);
    let p = Expr { kind: ExprKind::Var(part), ty: i32_ty.clone(), sym: Some(Sym::atom(part_atom.clone())), span: lo.span };
    let n = |v: i64| Expr { kind: ExprKind::Int(v), ty: i32_ty.clone(), sym: Some(Sym::constant(v)), span: lo.span };
    let bin = |op: crate::ast::BinaryOp, a: Expr, b: Expr| -> Expr {
        ...
        Expr { kind: ExprKind::Binary { op, lhs: Box::new(a), rhs: Box::new(b) }, ty: i32_ty.clone(), sym, span: lo.span }
    };
    ...
    let start = Expr { kind: ExprKind::Builtin { name: crate::ir::Builtin::Min, args: vec![hi.clone(), raw_start] },
        ty: i32_ty.clone(), sym: None, span: lo.span };
    let unclamped = bin(Add, start.clone(), span);
    let end = Expr {
        kind: ExprKind::Builtin { name: crate::ir::Builtin::Min, args: vec![hi.clone(), unclamped] },
        ty: i32_ty,
        sym: None,
        span: lo.span,
    };
        if !replace_slice(v, *axis, &start, &end, &Sym::param(&format!("split_extent#{part}_{view_index}"))) {
...
fn replace_slice(view: &mut Expr, axis: usize, lo: &Expr, hi: &Expr, extent: &Sym) -> bool {
    let changed = match &mut view.kind {
        ExprKind::Tuple(items) => items.iter_mut().all(|i| replace_slice(i, axis, lo, hi, extent)),
        ...
        ExprKind::Index { base, indices } => {
            ...
            if let Some(slot @ Index::Slice { .. }) = indices.get_mut(input) {
                *slot = Index::Slice { start: Some(lo.clone()), end: Some(hi.clone()) };
                true
            } else { replace_slice(base, input, lo, hi, extent) }
        }
        _ => false,
    };
    if changed {
        if let Ty::Tensor(shape) = &mut view.ty { shape.shape[axis] = extent.clone(); }
    }
    changed
}
