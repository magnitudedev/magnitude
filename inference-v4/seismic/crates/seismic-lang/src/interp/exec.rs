...
use crate::sir::{
    Block, Body, CallId, ContractFamily, DefId, DefKind, Definition, Expr, ExprKind, Merge, Pattern, Predicate, Region, RegionSource, SliceParent, Stage,
    Stmt, StmtKind, VarKind,
};
...
impl<'a> Frame<'a> {
    ...
    fn lookup(&self, name: &str) -> Option<i64> {
        ...
        if let Some((base, id)) = name.rsplit_once('#') {
            if let Some(id) = id.parse::<usize>().ok().filter(|id| self.body.vars.get(*id).is_some_and(|v| v.name == base)) {
                return integer(id);
            }
        }
        self.body.vars.iter().enumerate().rev().filter(|(_, var)| var.name == name).find_map(|(i, _)| integer(i))
    }

    pub fn sym(&self, s: &Sym) -> Result<i64, String> {
        s.eval(&|n| self.lookup(n)).ok_or_else(|| format!("cannot evaluate `{s}` here"))
    }
    ...
    fn holds(&self, p: &Predicate) -> Result<bool, String> {
        let env = |n: &str| self.caps.get(n).map(|piece| piece.width).or_else(|| self.lookup(n));
        let eval = |s: &Sym| s.eval(&env).ok_or_else(|| format!("cannot evaluate predicate operand `{s}`"));
        Ok(match p {
            Predicate::NonNegative(s) => eval(s)? >= 0,
            Predicate::Zero(s) => eval(s)? == 0,
            Predicate::NonZero(s) => eval(s)? != 0,
            Predicate::Full(name) => self.caps.get(name).is_none_or(|piece| piece.width > 0 && piece.domain % piece.width == 0),
        })
    }
}
...
impl<'a> Interpreter<'a> {
    ...
    pub(super) fn run_entry(&mut self, name: &str, args: &[Arg], workload: &Workload) -> Result<(), String> {
        let program = self.program;
        let families: Vec<&'a ContractFamily> = program.families.iter().filter(|f| f.name == name).collect();
        if families.is_empty() {
            return Err(format!("no function `{name}`"));
        }
        let shapes: HashMap<String, i64> = workload.shapes.iter().map(|(k, v)| (k.clone(), *v)).collect();
        let elems: HashMap<String, Elem> = workload.elems.iter().map(|(k, v)| (k.clone(), v.clone())).collect();
    ...
    fn entry_values(&self, d: &Definition, args: &[Arg], shapes: &HashMap<String, i64>) -> Result<Vec<Value>, String> {
        if args.len() != d.params.len() {
            return Err(format!("`{}` takes {} arguments, {} given", d.name, d.params.len(), args.len()));
        }
        ...
        for (i, (arg, param)) in args.iter().zip(&d.params).enumerate() {
            let scalar = |dtype: DType, bound: Option<u64>, v: f64| -> Result<Value, String> {
                ...
                Ok(Value::Scalar(dtype, if dtype.is_float() { round_to(dtype, v) } else { v }))
            };
            values.push(match (arg, &param.ty) {
                (Arg::Tensor(id), Ty::Tensor(_) | Ty::View(_)) => {
                    let t = self.tensors.get(*id).ok_or_else(|| format!("argument {i} names tensor {id}, which does not exist"))?;
                    Value::View(Shaped::tensor(*id, t.shape()))
                }
                (Arg::Scalar(v), Ty::Scalar(dtype)) => scalar(*dtype, None, *v)?,
                (Arg::Scalar(v), Ty::Index(bound)) => {
                    let bound = bound.eval(&|n| shapes.get(n).copied()).and_then(|n| u64::try_from(n).ok()).ok_or_else(|| format!("unresolved index bound for `{}`", param.name))?;
                    scalar(DType::I32, Some(bound), *v)?
                }
                _ => return Err(format!("argument {i} does not match parameter type {}", param.ty)),
            });
        }
        Ok(values)
    }
    ...
    fn instantiate(&self, def: &'a Definition, mut shapes: HashMap<String, i64>, caps: HashMap<String, Piece>, mut elems: HashMap<String, Elem>, values: Vec<Value>) -> Result<Frame<'a>, String> {
        let body = def.body.as_ref().ok_or("definition has no body")?;
        if values.len() != def.params.len() {
            return Err(format!("takes {} arguments, {} given", def.params.len(), values.len()));
        }
            ...
            match (&param.ty, value) {
                (Ty::Tensor(t) | Ty::View(t) | Ty::Tile(t), Value::Tile(s) | Value::View(s)) => {
                    if t.axes.len() != s.shape.len() {
                        return Err(format!("`{}` has rank {}, argument has rank {}", param.name, t.axes.len(), s.shape.len()));
                    }
                    let actual = self.elem_of(s);
                    match &t.elem {
                        Elem::Param(p) => {
                            if elems.insert(p.clone(), actual.clone()).is_some_and(|previous| previous != actual) {
                                return Err(format!("inconsistent element parameter {p}"));
                            }
                        }
                        declared if declared != &actual => return Err(format!("`{}` requires element {declared}, argument has {actual}", param.name)),
                        _ => {}
                    }
                    for (axis, n) in t.axes.iter().zip(&s.shape) {
                        if let Extent::Semantic(sym) = axis {
                            if let Some(p) = def.shape_params.iter().find(|p| !shapes.contains_key(*p) && sym == &Sym::param(p)) {
                                shapes.insert(p.clone(), *n as i64);
                            }
                        }
                    }
                }
                (Ty::Scalar(_) | Ty::Index(_), Value::Scalar(..)) | (Ty::Tuple(_), Value::Tuple(_)) | (Ty::Result(_), Value::Result(_)) | (Ty::Native(_), Value::Native(_)) => {}
                ...
                (ty, v) => return Err(format!("`{}`: {} where {ty} is required", param.name, v.kind())),
            }
        }
        let mut vars = vec![None; body.vars.len()];
        for (param, value) in def.params.iter().zip(values) {
            if !matches!(value, Value::Void) {
                *vars.get_mut(param.var).ok_or("parameter variable outside the body")? = Some(value);
            }
        }
        let frame = Frame { def, body, vars, slices, shapes, caps, elems, dynamic: HashMap::new() };
        for param in &def.params {
            let Some(value) = &frame.vars[param.var] else { continue };
            match (&param.ty, value) {
                (Ty::Tensor(t) | Ty::View(t) | Ty::Tile(t), Value::Tile(s) | Value::View(s)) => {
                    for (k, (axis, n)) in t.axes.iter().zip(&s.shape).enumerate() {
                        if let Extent::Semantic(sym) = axis {
                            ...
                            if expected != *n as i64 {
                                return Err(format!("`{}` axis {k} has extent {n}, expected {sym} = {expected}", param.name));
                            }
                        }
                    }
                }
    ...
    /// Whether an adoption for `target` admits the portable body bound in `frame`. An
    /// adoption's predicates are read over the adopted body's shape parameter names.
    fn adopted(&self, families: &[&'a ContractFamily], target: &str, frame: &Frame<'a>) -> bool {
        families.iter().flat_map(|f| &f.lowerings).any(|id| {
            let d = self.program.definition(*id);
            matches!(&d.kind, DefKind::Adopt { target: t } if t == target) && d.predicates.iter().all(|p| frame.holds(p).unwrap_or(false))
        })
    }

    /// Deterministic definition choice: on a boundary of the target, the first applicable
    /// lowering body, else the first applicable adopted portable body; otherwise the first
    /// applicable portable body. `choice` may force any applicable body.
    fn select(&self, name: &str, families: &[&'a ContractFamily], make: &dyn Fn(&'a Definition) -> Result<Frame<'a>, String>) -> Result<Frame<'a>, String> {
        let program = self.program;
        let target = self.target.as_deref();
        let boundary = target.is_some_and(|t| families.iter().any(|f| f.is_boundary(t, program)));
        let mut ids: Vec<DefId> = families.iter().flat_map(|f| f.bodies.iter().chain(&f.lowerings)).copied().collect();
        ids.sort();
        ids.dedup();
        let mut applicable: Vec<(DefId, bool, Frame<'a>)> = Vec::new();
        let mut reasons = Vec::new();
        for id in ids {
            let d = program.definition(id);
            let lowering = match &d.kind {
                DefKind::Lower { target: t } if Some(t.as_str()) == target => true,
                DefKind::Body if d.requires_target.as_deref().is_none_or(|r| Some(r) == target) => false,
                _ => continue,
            };
            if d.body.is_none() {
                continue;
            ...
            match make(d) {
                Ok(frame) => {
                    let preferred = lowering || !boundary || target.is_some_and(|t| self.adopted(families, t, &frame));
                    applicable.push((id, preferred, frame));
                }
                Err(reason) => reasons.push(format!("definition {}: {reason}", id.0)),
            }
        }
        let forced = match &self.choice {
            Some(choose) => choose(name, &applicable.iter().map(|(id, ..)| *id).collect::<Vec<_>>()),
            None => None,
        };
        let position = match forced {
            Some(id) => applicable.iter().position(|(candidate, ..)| *candidate == id).ok_or_else(|| format!("forced definition {} of `{name}` is not applicable", id.0))?,
            None => {
                let lowered = applicable.iter().position(|(id, preferred, _)| *preferred && matches!(program.definition(*id).kind, DefKind::Lower { .. }));
                lowered.or_else(|| applicable.iter().position(|(_, preferred, _)| *preferred)).ok_or_else(|| {
                    format!("no applicable definition of `{name}`{}{}", if reasons.is_empty() { "" } else { ": " }, reasons.join("; "))
                })?
            }
        };
        Ok(applicable.swap_remove(position).2)
    }

    pub(super) fn call(&mut self, call: CallId, args: &'a [Expr], f: &mut Frame<'a>) -> Result<Value, String> {
        let program = self.program;
        let body = f.body;
        let site = body.calls.get(call.0 as usize).ok_or("unknown call site")?;
        let family = program.families.get(site.family).ok_or("call names an unknown family")?;
        let mut outs = vec![false; args.len()];
        if let Some(b) = site.bindings.first() {
            for (param, ordinal) in program.definition(b.definition).params.iter().zip(&b.arg_order) {
        ...
        for (a, out) in args.iter().zip(&outs) {
            ...
            values.push(if *out && unset { Value::Void } else { self.expr(a, f)? });
        }
        let mut inner = {
            let caller: &Frame<'a> = f;
            let this: &Interpreter<'a> = self;
            let make = |d: &'a Definition| -> Result<Frame<'a>, String> {
                let b = site.bindings.iter().find(|b| b.definition == d.id).ok_or("the arguments do not bind its parameters")?;
                ...
                for (name, extent) in &b.shape_args {
                    match extent {
                        Extent::Semantic(sym) => {
                            ...
                            if let Some((_, piece)) = caller.caps.iter().find(|(p, _)| sym == &Sym::param(p)) {
                ...
                let ordered = b.arg_order.iter().map(|i| values.get(*i).cloned().ok_or("argument order names a missing argument")).collect::<Result<Vec<_>, _>>()?;
        ...
        let binding = site.bindings.iter().find(|b| b.definition == inner.def.id).ok_or("selected definition lost its binding")?;
        for (param, ordinal) in inner.def.params.iter().zip(&binding.arg_order) {
            if param.mode == Mode::In {
                continue;
            }
            let arg = args.get(*ordinal).ok_or("argument order names a missing argument")?;
            match inner.vars[param.var].take() {
                Some(v @ (Value::Scalar(..) | Value::Tuple(_))) => self.assign(arg, AssignOp::Assign, v, f)?,
                Some(Value::Tile(s)) => {
    ...
    fn locate(&self, def: &Definition, span: Span, message: String) -> String {
        ...
        let line = self.program.files.get(def.file).map(|(_, text)| line_col(text, span.start).0).unwrap_or(0);
    ...
    fn bind(&mut self, pattern: &Pattern, value: Value, f: &mut Frame<'a>) -> Result<(), String> {
        match (pattern, value) {
            (Pattern::Var(id), value) => {
                *f.vars.get_mut(*id).ok_or("binding outside the body's variables")? = Some(value);
                Ok(())
            }
            ...
            (Pattern::Tuple(patterns), other) => Err(format!("cannot destructure {} into {} names", other.kind(), patterns.len())),
        }
    }
    ...
    fn values(&mut self, items: &'a [Expr], f: &mut Frame<'a>) -> Result<Value, String> {
        let value = match items {
            [] => Value::Void,
            [one] => self.expr(one, f)?,
            many => Value::Tuple(many.iter().map(|e| self.expr(e, f)).collect::<Result<_, _>>()?),
        };
        self.snapshot(value)
    }

    pub(super) fn assign(&mut self, target: &'a Expr, op: AssignOp, value: Value, f: &mut Frame<'a>) -> Result<(), String> {
        match &target.kind {
            ExprKind::Tuple(targets) => match value {
                ...
                other => Err(format!("cannot assign {} to {} places", other.kind(), targets.len())),
            },
            ExprKind::Var(id) => {
                let current = f.vars.get(*id).ok_or("assignment outside the body's variables")?.clone();
                match (current, value) {
                    (Some(Value::Scalar(d, x)), Value::Scalar(vd, v)) if op != AssignOp::Assign => {
                        f.vars[*id] = Some(Value::scalar(super::scalar::assign(op, (d, x), (vd, v))?));
                        Ok(())
                    }
                    (Some(Value::View(s)), v @ (Value::Scalar(..) | Value::Tile(_) | Value::View(_))) => self.write(&s, op, &v),
                    (Some(Value::Tile(s)), v @ (Value::Scalar(..) | Value::Tile(_) | Value::View(_))) if matches!(s.backing, Backing::Owned(_)) && (op != AssignOp::Assign || v.shaped().is_some_and(|n| n.shape == s.shape && self.dtype_of(n) == self.dtype_of(&s))) => self.write(&s, op, &v),
                    (_, v) if op == AssignOp::Assign => {
                        f.vars[*id] = Some(self.snapshot(v)?);
                        Ok(())
                    }
                    (current, v) => Err(format!("`{}` of {} into {}", op.text(), v.kind(), current.map_or("an unset variable", |c| c.kind()))),
                }
            }
            ExprKind::Index { .. } | ExprKind::Transpose(_) => {
                let dst = self.place(target, f)?;
                if matches!(value, Value::Scalar(..)) && !dst.shape.is_empty() && op == AssignOp::Assign {
    ...
    fn counted(&mut self, vars: &[usize], bounds: &[(i64, i64)], body: &'a Block, f: &mut Frame<'a>) -> Result<Flow, String> {
        ...
        match &s.kind {
            ...
            StmtKind::Coordinates { vars, of, axes, body } => {
                ...
                if vars.len() != axes.len() {
                    return Err("coordinate loop binds a different number of names than axes".into());
                }
                let mut bounds = Vec::with_capacity(axes.len());
                for axis in axes {
                    let n = *shaped.shape.get(*axis).ok_or_else(|| format!("axis {axis} of a rank-{} value", shaped.shape.len()))? as i64;
            ...
            StmtKind::Publish { value, destination } => {
                ...
                match &v {
                    Value::Scalar(..) if !dst.shape.is_empty() => return Err(format!("publish of a scalar to a destination of shape {:?}", dst.shape)),
    ...
    fn stages(&mut self, stages: &'a [Stage], f: &mut Frame<'a>) -> Result<Flow, String> {
        ...
        for (i, stage) in stages.iter().enumerate() {
            match (stage.ports.as_slice(), port.take()) {
                ...
                (many, _) => return Err(self.locate(f.def, stage.span, format!("stage `{}` binds {} ports the previous stage did not yield", stage.name, many.len()))),
            }
    ...
    fn pieces(&self, f: &Frame<'a>, region: &Region, slice: SliceId, merge: bool) -> Result<Vec<Piece>, String> {
        let decl = f.body.slices.get(slice.0 as usize).ok_or("unknown slice binder")?;
        ...
        let width = self.partitioner.width(&f.def.name, region.id, slice, domain);
        if width < 1 || width > domain {
            return Err(format!("partitioner chose width {width} for an extent of {domain}"));
        }
        let mut out = Vec::new();
        if merge {
            ...
            for p in 0..parts {
                ...
                out.push(Piece { lo: at, hi: at + n, width: capacity, domain });
                at += n;
            }
        } else {
            let mut at = lo;
            while at < hi {
                out.push(Piece { lo: at, hi: (at + width).min(hi), width, domain });
                at += width;
            }
        }
        Ok(out)
    ...
    pub(super) fn region(&mut self, r: &'a Region, f: &mut Frame<'a>) -> Result<Flow, String> {
        let body = f.body;
        let binders = r.binders.iter().map(|v| match body.vars.get(*v).map(|var| &var.kind) {
        ...
        let visits: Vec<Vec<Piece>> = match &r.source {
            RegionSource::Results(source) => match self.expr(source, f)? {
                ...
                other => return Err(format!("region traverses {}, not a region result", other.kind())),
            },
            RegionSource::Domains => {
                let axes = binders.iter().map(|(_, id)| self.pieces(f, r, *id, r.merge.is_some())).collect::<Result<Vec<_>, _>>()?;
                let mut visits = vec![Vec::new()];
                for axis in &axes {
                    visits = visits.iter().flat_map(|prefix| axis.iter().map(move |p| prefix.iter().copied().chain([*p]).collect())).collect();
                }
                visits
            }
        };
        ...
        for (k, (_, id)) in binders.iter().enumerate() {
            bound.push((k, *id));
            if let Some(SliceParent::Rebind(origin)) = body.slices.get(id.0 as usize).map(|d| &d.parent) {
                if matches!(r.source, RegionSource::Results(_)) && f.slices.get(origin.0 as usize).is_some_and(|s| s.is_none()) {
                    bound.push((k, *origin));
                }
            }
        }
        let saved: Vec<Option<Piece>> = bound.iter().map(|(_, id)| f.slices.get(id.0 as usize).copied().flatten()).collect();
        let producing = r.result.is_some() || r.merge.is_some();
        let mut values = Vec::with_capacity(visits.len());
        for visit in &visits {
            if visit.len() != binders.len() {
                return Err(format!("region binds {} slices over a result of {} axes", binders.len(), visit.len()));
            }
            for (k, id) in &bound {
                *f.slices.get_mut(id.0 as usize).ok_or("unknown slice binder")? = Some(visit[*k]);
            }
            ...
            match self.block(&r.body, f)? {
                ...
                Flow::Next => return Err("a visit of a result-producing region did not yield".into()),
        ...
        match &r.merge {
            ...
            None if producing => Ok(Flow::Yield(Value::Result(Rc::new(ResultVal::new(visits, values))))),
            None => Ok(Flow::Next),
        }
    }
    ...
    fn merge(&mut self, merge: &'a Merge, mut level: Vec<Value>, f: &mut Frame<'a>) -> Result<Value, String> {
        if level.is_empty() {
        ...
        level.pop().ok_or_else(|| "merge lost its value".to_string())
    }
}
