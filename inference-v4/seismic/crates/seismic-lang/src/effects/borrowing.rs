...
use crate::{ast::{AssignOp, UnaryOp}, ir::*, types::Ty};
...
impl State {
    fn join(&mut self, other: &Self) -> bool {
        ...
        for (&variable, &value) in &other.aliases { *self.aliases.entry(variable).or_default() |= value; }
        self.assumptions.retain(|variable| other.assumptions.contains(variable));
        *self != previous
    }
}
...
fn condition(expression: &Expr) -> Option<(VarId, bool)> {
    match &expression.kind {
        ...
        ExprKind::Unary { op: UnaryOp::Not, expr } => condition(expr).map(|(variable, value)| (variable, !value)),
        _ => None,
    }
}
...
fn load(statement: &Stmt) -> Option<(VarId, &Expr, bool)> {
    let StmtKind::Assign { target: Expr { kind: ExprKind::Var(variable), .. }, op: AssignOp::Assign, value } = &statement.kind else { return None; };
    match &value.kind {
        ...
        ExprKind::Builtin { name: Builtin::Load, args } if args.len() == 1 => Some((*variable, &args[0], true)),
        _ => None,
    }
}
fn rebindings(statement: &Stmt) -> Vec<VarId> {
    match &statement.kind {
        StmtKind::Assign { target: Expr { kind: ExprKind::Var(variable), .. }, op: AssignOp::Assign, .. } => vec![*variable],
...
fn alias_sources(statement: &Stmt) -> Vec<(VarId, VarId)> {
    ...
    if let Some((target, source, true)) = load(statement) {
        if let Some(source) = backing(source) { aliases.push((target, source)); }
    }
    if let StmtKind::Assign { target: Expr { kind: ExprKind::Var(target), .. }, op: AssignOp::Assign, value } = &statement.kind {
        if matches!(value.ty, Ty::Tensor(_)) {
            if let Some(source) = backing(value) { aliases.push((*target, source)); }
        }
    }
    if let StmtKind::LoadLoop { vars, views, modes, .. } = &statement.kind {
...
fn shallow(statement: &Stmt) -> Stmt {
    let kind = match &statement.kind {
        StmtKind::If { cond, .. } => StmtKind::If { cond: cond.clone(), then: Vec::new(), els: Vec::new() },
        StmtKind::Range { var, lo, hi, .. } => StmtKind::Range { var: *var, lo: lo.clone(), hi: hi.clone(), body: Vec::new() },
        StmtKind::Parallel { vars, extents, .. } => StmtKind::Parallel { vars: vars.clone(), extents: extents.clone(), body: Vec::new() },
        StmtKind::Owned { vars, tile, .. } => StmtKind::Owned { vars: vars.clone(), tile: tile.clone(), body: Vec::new() },
        StmtKind::Lanes { var, extent, width, .. } => StmtKind::Lanes { var: *var, extent: extent.clone(), width: *width, body: Vec::new() },
        StmtKind::LoadLoop { domain, offset, modes, vars, views, axes, piece, capacity, .. } => StmtKind::LoadLoop {
            domain: domain.clone(), offset: *offset, modes: modes.clone(), vars: vars.clone(), views: views.clone(), axes: axes.clone(),
            piece: piece.clone(), capacity: *capacity, body: Vec::new(),
        },
    ...
    Stmt { id: statement.id, span: statement.span, kind }
}
fn backing_rebound(statement: &Stmt, variable: VarId) -> bool {
    match &statement.kind {
        // Rebinding a view changes the reference, not its previous backing.
        StmtKind::Assign { target: Expr { kind: ExprKind::Var(target), ty, .. }, op: AssignOp::Assign, value }
            if *target == variable && (matches!(ty, Ty::Tensor(_))
                || matches!(value.kind, ExprKind::Load { mode: LoadMode::Borrow, .. })) => false,
        StmtKind::Parallel { body, .. } | StmtKind::Range { body, .. } | StmtKind::Lanes { body, .. }
        | StmtKind::Owned { body, .. } | StmtKind::LoadLoop { body, .. } =>
            tile_mutated(&shallow(statement), variable) || body.iter().any(|statement| backing_rebound(statement, variable)),
        StmtKind::If { then, els, .. } => tile_mutated(&shallow(statement), variable)
            || then.iter().chain(els).any(|statement| backing_rebound(statement, variable)),
        _ => tile_mutated(statement, variable),
    }
}
fn reads(statement: &Stmt, variable: VarId) -> bool {
    match &statement.kind {
        StmtKind::Assign { target: Expr { kind: ExprKind::Var(_), .. }, op: AssignOp::Assign, value } =>
            expressions(value, &|expr| matches!(expr.kind, ExprKind::Var(id) if id == variable)),
...
impl<'a> LoadLifetimes<'a> {
    pub(crate) fn new(body: &'a [Stmt]) -> Self {
        let mut result = Self { nodes: Vec::new(), sites: HashMap::new(), backing_sources: HashMap::new(), backing_targets: HashMap::new() };
        result.block(body, None, &BTreeMap::new());
        for node in &result.nodes {
            for &(target, source) in &node.alias_sources {
                result.backing_sources.entry(target).or_default().insert(source);
                result.backing_targets.entry(source).or_default().insert(target);
            }
        }
        result
    }
    fn block(&mut self, body: &'a [Stmt], after: Option<usize>, assumptions: &BTreeMap<VarId, bool>) -> Option<usize> {
        ...
        let entries = body.iter().map(|statement| {
        ...
        for (original, assumptions) in body.iter().zip(&entries).rev() {
            ...
            self.nodes.push(Node { original, operation, alias_sources, next: Vec::new(), assumptions: assumptions.clone() });
            if load(original).is_some() { self.sites.insert(original as *const Stmt, id); }
            let successors = match &original.kind {
                StmtKind::If { cond, then, els } => {
                    let mut yes = assumptions.clone(); let mut no = assumptions.clone();
                    if let Some((variable, value)) = condition(cond) { yes.insert(variable, value); no.insert(variable, !value); }
                ...
                StmtKind::Parallel { body, .. } => {
                    ...
                    next.into_iter().chain(self.block(body, next, assumptions)).collect()
                },
                StmtKind::Range { body, .. } | StmtKind::Owned { body, .. }
                | StmtKind::Lanes { body, .. } | StmtKind::LoadLoop { body, .. } => {
                    let mut repeated = assumptions.clone();
                    repeated.retain(|variable, _| !tile_mutated(original, *variable));
                    next.into_iter().chain(self.block(body, Some(id), &repeated)).collect()
                },
                StmtKind::Reduction(reduction) => {
                    // Until reduction expansion, callbacks may be invoked
                    // repeatedly. Treat their execution as an arbitrary repeat.
                    let mut repeated = assumptions.clone();
                    repeated.retain(|variable, _| !tile_mutated(original, *variable));
                    let mut callbacks = Some(id);
                    for implementation in reduction.implementations().collect::<Vec<_>>().into_iter().rev() {
                        callbacks = self.block(&implementation.body, callbacks, &repeated);
                    }
                    next.into_iter().chain(callbacks).collect()
                },
    ...
    fn backing_aliases(&self, root: VarId) -> BTreeSet<VarId> {
        ...
        while let Some(variable) = pending.pop() {
            for &alias in self.backing_sources.get(&variable).into_iter().flatten() {
                if aliases.insert(alias) { pending.push(alias); }
            }
        }
    ...
    pub(crate) fn can_borrow(&self, statement: &Stmt) -> bool {
        let Some(&start) = self.sites.get(&(statement as *const Stmt)) else { return false; };
        let Some((variable, source, _)) = load(statement) else { return false; };
        let Some(root) = backing(source).filter(|root| *root != variable) else { return false; };
        let backing_aliases = self.backing_aliases(root);
        let assumptions = &self.nodes[start].assumptions;
        let initial = State { aliases: BTreeMap::from([(variable, VALID)]),
            assumptions: assumptions.keys().copied().filter(|&variable| !tile_mutated(statement, variable)).collect() };
        let mut incoming = vec![None::<State>; self.nodes.len()];
        let mut pending = VecDeque::new();
        for &next in &self.nodes[start].next { if next < self.nodes.len() { incoming[next] = Some(initial.clone()); pending.push_back(next); } }
        while let Some(id) = pending.pop_front() {
            ...
            let operation = if matches!(node.original.kind, StmtKind::Parallel { .. }) { node.original } else { &node.operation };
            let source_changed = tensor_effect(operation) || backing_aliases.iter().any(|&root| backing_rebound(operation, root));
            if source_changed { for value in state.aliases.values_mut() { *value = INVALID; } }
            for (&alias, &value) in &state.aliases {
                ...
                if (value & INVALID != 0 && reads(&node.operation, alias))
                    ...
                    || (!reborrowed && tile_mutated(&node.operation, alias)) { return false; }
            }
            let derived = node.alias_sources.iter().filter_map(|&(target, source)|
                state.aliases.get(&source).copied().map(|value| (target, value))).collect::<Vec<_>>();
            for variable in rebound { state.aliases.remove(&variable); state.assumptions.remove(&variable); }
            ...
            state.assumptions.retain(|variable| !tile_mutated(&node.operation, *variable));
            for (target, value) in derived { state.aliases.insert(target, value); }
            if state.aliases.is_empty() { continue; }
            let selected_branch = match &node.original.kind {
                StmtKind::If { cond: Expr { kind: ExprKind::Bool(value), .. }, .. } => Some(usize::from(!*value)),
                StmtKind::If { cond, .. } => condition(cond).and_then(|(variable, positive)|
                    state.assumptions.contains(&variable).then(|| usize::from(assumptions[&variable] != positive))),
                _ => None,
            };
            for (ordinal, &next) in node.next.iter().enumerate() {
                if next >= self.nodes.len() || selected_branch.is_some_and(|selected| selected != ordinal) { continue; }
                let changed = match &mut incoming[next] {
                    Some(previous) => previous.join(&state),
                    entry @ None => { *entry = Some(state.clone()); true },
                };
    ...
    pub(crate) fn variable_can_borrow(&self, variable: VarId) -> bool {
        let sites = self.sites.values().copied().filter(|&id| load(self.nodes[id].original).is_some_and(|(target, _, _)| target == variable)).collect::<Vec<_>>();
        !sites.is_empty() && sites.into_iter().all(|id| self.can_borrow(self.nodes[id].original))
    }
}
