...
pub fn validate_element_bindings(f: &crate::hir::Function, elements: &HashMap<String, crate::types::Elem>) -> Result<(), String> {
    ...
    for parameter in &f.elem_params {
        match elements.get(parameter) {
            Some(crate::types::Elem::Dtype(dtype)) if dtype.is_float() => {},
            Some(crate::types::Elem::Repr(name)) if crate::repr::lookup(name).is_some() => {},
            _ => return Err(format!("element parameter `{parameter}` of `{name}` requires a concrete supported binding")),
        }
    }
    for parameter in elements.keys() {
        if !f.elem_params.contains(parameter) { return Err(format!("unknown element parameter `{parameter}` of `{name}`")); }
    }
    Ok(())
}
