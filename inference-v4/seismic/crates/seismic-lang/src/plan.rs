...
pub struct Step {
    ...
    /// Concrete element arguments inferred at the composition call site.
...
pub fn plan(program: &Program, name: &str, shapes: &HashMap<String, i64>) -> Result<Plan, String> {
    plan_specialized(program, name, shapes, &HashMap::new())
}

/// Bind composition element parameters before inferring each kernel specialization.
pub fn plan_specialized(program: &Program, name: &str, shapes: &HashMap<String, i64>, elements: &HashMap<String, Elem>) -> Result<Plan, String> {
...
impl<'a> Planner<'a> {
    ...
    fn stmt(&mut self, s: &Stmt) -> Result<(), String> {
        match &s.kind {
            ...
            StmtKind::Expr(e) => match &e.kind {
                ExprKind::Call { callee, shape_args, elem_args, args } => self.call(callee, shape_args, elem_args, args),
    ...
    fn call(&mut self, callee: &str, shape_args: &[Sym], elem_args: &[Elem], args: &[Expr]) -> Result<(), String> {
        ...
        let elements = sig.elem_params.iter().cloned().zip(elem_args.iter().map(|e| crate::lower::subst_elem(e, &self.elements))).collect();
        self.steps.push(Step { kernel: callee.to_string(), shapes, elements, tensors, scalars });
        Ok(())
    }
    ...
    fn view(&self, e: &Expr) -> Result<(String, i64, Elem), String> {
        match &e.kind {
            ExprKind::Builtin { name: crate::ir::Builtin::Reshape, args } => {
                ...
                let source = dims(&args[0])?;
                crate::layout::reshape_strides(&source,&crate::layout::row_major_strides(&source)?,&dims(e)?)?;
                Ok(binding)
            }
            ...
            ExprKind::Var(v) => {
                ...
                Ok((var.name.clone(), 0, crate::lower::subst_elem(&s.elem, &self.elements)))
            }
            ExprKind::Index { base, indices } => {
                ...
                Ok((var.name.clone(), offset, crate::lower::subst_elem(&s.elem, &self.elements)))
            }
