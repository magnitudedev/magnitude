...
use seismic_compiler::target::{
    DenseRepresentationGeometry, DeviceContract, KernelEmissionLayout,
    PackedRepresentationGeometry, ReadableRepresentationGeometry, RepresentationGeometry,
};
...
fn op_values<B: seismic_compiler::target::Backend>(op: &Op<B>) -> Vec<ErasedValue> {
    match op {
        Op::Constant { out, .. }
        | Op::Geometry { out, .. }
        | Op::NatArg { out, .. }
