...
//! form `DeviceContract<Cuda>`; fixed service probes on the production
...
use seismic_compiler::target::{
    core_service_classes, CompatibilityIdentity, CompositionQualificationCase,
    CompositionQualificationParts, DataTypeSupport, DeviceContract, DeviceContractParts,
    DurationInterval, ExecutionProfile, ExecutionProfileParts, FactProvenance, KernelAbiAllocation,
    KernelAbiAllocationRole, KernelAbiFootprint, KernelAbiLayout, KernelAbiModel, KernelWordLayout,
    LocalRealization, LocalRealizationPolicy, MeasurementBatch, MeasurementSeries,
    NumericalEnvironment, ProfileAcquisitionMetrics, ResourceTopology, ServiceAccuracyClass,
    ServiceCorrelationId, ServiceCurve, ServiceCurveRegime, ServiceDefinition,
    ServiceQualificationDomain, TargetLimits, VectorSupport, SERVICE_COPY, SERVICE_DATA_CHECK,
    SERVICE_FILL, SERVICE_SCALAR_MOVE, SERVICE_SCALAR_READ, SERVICE_SUBMISSION,
};
...
pub(crate) fn discover(ordinal: u32) -> Result<DeviceContractParts<Cuda>, TargetError> {
    let driver = Driver::load().map_err(TargetError::DeviceUnavailable)?;
    let device = c_int::try_from(ordinal).map_err(|_| {
        TargetError::DeviceUnavailable(format!("ordinal {ordinal} exceeds the driver ABI"))
    })?;
...
pub(crate) fn profile_for_opened(
    ...
    context: &Arc<Context>,
    stream: &Arc<Stream>,
) -> Result<(DeviceContract<Cuda>, ExecutionProfile<Cuda>), TargetError> {
    ...
    let identity_and_limits_ns =
        u64::try_from(identity_started.elapsed().as_nanos()).unwrap_or(u64::MAX);
    let device = DeviceContract::assemble(parts, capability::registry())?;
    let (services, composition_qualification, probe_build_ns, probe_execution_ns) =
        acquire_services(context, stream, device.facts())?;
    let execution = ExecutionProfile::assemble(
        &device,
        ExecutionProfileParts {
            probe_suite_revision: "seismic-cuda-probes-v5-classified-host-confidence95",
            services,
...
> {
    ...
    let required = core_service_classes();
    ...
    let composition_qualification = CompositionQualificationParts {
        ...
        cases: vec![CompositionQualificationCase {
            ...
            demands: vec![seismic_compiler::target::ConcreteExecutionDemand {
                class: SERVICE_SUBMISSION,
                units: 8_192,
                mode: seismic_compiler::target::DemandMode::DependencyLatency,
            }],
...
fn measured_definition(
    class: seismic_compiler::target::ServiceClassId,
    topology: ResourceTopology,
    probe: &'static str,
...
fn service_accuracy(class: seismic_compiler::target::ServiceClassId) -> ServiceAccuracyClass {
    if matches!(
        class,
        SERVICE_GLOBAL_READ
            | SERVICE_GLOBAL_WRITE
