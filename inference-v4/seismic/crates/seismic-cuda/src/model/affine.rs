...
use seismic_accounting::workload::{IntegerInput, IntegerRange};
...
impl Affine {
    ...
    /// PTX integer interpretation at the specified width. A single wrap/sign
...
mod tests {
    use super::*;
    #[test]
    fn shared_coordinates_cancel_and_integer_residues_remain_exact() {
        let row = Affine::domain(
            IntegerInput::Scalar { slot: 0 },
            IntegerRange {
                min: 0,
                max: 131071,
                stride: 1,
        ...
        .unwrap();
        let a = row.scale(4096).unwrap().add(&Affine::constant(12)).unwrap();
        let b = row.scale(4096).unwrap().add(&Affine::constant(16)).unwrap();
        assert_eq!(a.disjoint(4, &b, 4), Some(true));
        assert_eq!(a.residue(32), Some(12));
        assert_eq!(
            a.quotient(32)
                .unwrap()
                .sub(&b.quotient(32).unwrap())
                .unwrap()
                .exact(),
            Some(0)
        );
        let independent = Affine::domain(
            IntegerInput::Scalar { slot: 1 },
            IntegerRange {
                min: 0,
                max: 131071,
                stride: 1,
        ...
        .unwrap();
        assert_eq!(
            a.disjoint(4, &independent.add(&Affine::constant(12)).unwrap(), 4),
            None
        );
    }
    #[test]
    fn wrapping_requires_a_uniform_integer_region() {
        let x = Affine::domain(
            IntegerInput::Scalar { slot: 0 },
            IntegerRange {
                min: 2147483647,
        ...
        .unwrap();
        assert!(x.interpreted(32, true).is_none());
        let y = Affine::domain(
            IntegerInput::Scalar { slot: 0 },
            IntegerRange {
                min: -10,
        ...
        .unwrap();
        assert_eq!(
            y.interpreted(32, false).unwrap().bounds(),
            Some((4294967286, 4294967295))
        );
    }
}
