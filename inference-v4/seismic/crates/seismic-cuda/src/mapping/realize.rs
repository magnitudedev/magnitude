...
pub(super) fn threads_per_block(limits: &Limits, work_items: u64, lanes: u64) -> Result<u32, String> {
    if lanes == 0 || u64::from(limits.max_threads_per_block) < lanes {
        return Err(format!("a block of {} threads cannot hold one work item of {lanes} lanes", limits.max_threads_per_block));
    }
    let preferred = u64::from(BLOCK_THREADS.min(limits.max_threads_per_block)) / lanes;
    let items = work_items.clamp(1, preferred.max(1)).max(work_items.div_ceil(u64::from(limits.max_grid_x.max(1))));
    let threads = items.checked_mul(lanes).filter(|t| *t <= u64::from(limits.max_threads_per_block));
    threads.and_then(|t| u32::try_from(t).ok()).ok_or_else(|| {
        format!("{work_items} work items of {lanes} lanes exceed a grid of {} blocks of {} threads", limits.max_grid_x, limits.max_threads_per_block)
    })
}

pub(super) fn realize(limits: &Limits, lowered: &LoweredIr) -> Result<Launches, SelectionError> {
    let mut selected = lowered.clone();
    seismic_compiler::scalar_load_rule(&mut selected).map_err(|reason| SelectionError::UnsupportedMapping(format!("`{}`: {reason}", selected.name)))?;
    let participation = if seismic_compiler::subgroup_required(&selected) {
        if limits.warp_size != 32 {
            return Err(SelectionError::UnsupportedMapping(format!("`{}`: participant intrinsics need a 32-lane warp; the device has {}", selected.name, limits.warp_size)));
        }
        Participation::Subgroup { lanes: 32 }
    } else {
        Participation::Thread
    };
    let sequence = seismic_compiler::scalar_sequence_participants_resolved(&selected, CallConv::SystemV, Dispatch::ParallelRoot, participation).map_err(|reason| {
        if reason.contains("static") {
            SelectionError::AnalysisUnavailable(format!("`{}`: {reason}", selected.name))
        } else {
            SelectionError::UnsupportedMapping(format!("`{}`: scalar realization: {reason}", selected.name))
        }
    })?;
    if sequence.phases.is_empty() {
        return Err(SelectionError::Reconstruction(format!("`{}`: the phase plan has no launch", selected.name)));
    }
    ...
    for (ordinal, phase) in sequence.phases.into_iter().enumerate() {
        ...
        let threads = threads_per_block(limits, program.work_items, lanes)
            .map_err(|reason| SelectionError::IncompatibleComposition(format!("`{}` launch {ordinal}: {reason}", selected.name)))?;
        let execution = Execution::new(program, threads, limits.launch())
            .map_err(|reason| SelectionError::UnsupportedMapping(format!("`{}` launch {ordinal}: PTX: {reason}", selected.name)))?;
        // Hard limits are rechecked on the realized launch.
        let storage = execution.storage();
        let invocation_bytes = (storage.scratch_bytes as u64).checked_add(storage.status_bytes as u64);
        if invocation_bytes.is_none_or(|bytes| bytes > limits.max_scratch_bytes) {
            return Err(SelectionError::IncompatibleComposition(format!(
                ...
                selected.name, storage.scratch_bytes, execution.dispatch().participating_lanes(), limits.max_scratch_bytes
            )));
        }
        phases.push(execution);
    }
    Ok(Launches { name: selected.name, phases })
}
