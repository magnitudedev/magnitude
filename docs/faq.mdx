...
Yes. Enable **Network access** for direct connections and use the server address and API key. SSH tunnels and mirrored WSL networking can use the local endpoint without enabling it. Only inference is available from other devices. See [Network access](/api/network-access) and [Windows and WSL](/api/windows-wsl).

## Can I keep models on another drive?

Yes. Change **Model storage** in Settings, then move the existing models with the command it shows. See [Settings](/settings#model-storage).
