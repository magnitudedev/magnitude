...
When a conversation gets long, Magnitude automatically compacts context to stay within model limits. Old messages are summarized and recent messages are kept — the session continues without interruption.

You may briefly see an indicator in the context bar while compaction is running. No action is needed; it's fully automatic and transparent.

Each subagent compacts its context independently.