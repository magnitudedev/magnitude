...
description: "How Magnitude runs local models without a separate model server"
icon: cpu
---

Magnitude has its own inference engine built in. It downloads, configures, loads, and runs local models inside the agent, so there is no Ollama server or separate inference runtime to install and manage.
