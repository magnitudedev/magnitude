...
| Command | Purpose |
|---|---|
| `magnitude app open` | Explicitly open the desktop window |
| `magnitude hardware` | Inspect hardware and available inference memory |
| `magnitude catalog status` | Inspect catalog preparation |
| `magnitude catalog show <model-id>` | Inspect one catalog model |
| `magnitude catalog recommendations [--preference <value>] [--limit <count>]` | Rank compatible catalog models |
| `magnitude service install` | Enable login startup and start the desktop in the background |
| `magnitude service start` | Start the desktop in the background and await its service |
| `magnitude service status` | Show service, tray, login-startup, and active-model status |
| `magnitude service stop` | Quit the desktop and service while preserving login startup |
| `magnitude service uninstall` | Disable login startup and quit while preserving models and settings |
...
| `magnitude update` | Update the headless CLI |

Run `magnitude --help` or append `--help` to a command for its complete options. Read `magnitude docs cli` for the non-interactive workflow. CLI output is text; structured integrations use the SDK. Onboarding and harness selection live in the desktop application. Connection commands configure external harnesses without launching them.
