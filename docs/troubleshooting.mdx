...
If the CLI does not start, check `magnitude --version`, then update or reinstall it. Native
Windows startup is supported; if a selected model or backend is not available yet, choose a
currently supported configuration. If you were using interactive setup, you can run
`magnitude setup` again.
