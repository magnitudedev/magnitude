...
This starts Magnitude in development mode with the debug panel available (toggle with Ctrl+X). You can also pass `--debug` when running the installed CLI (`magnitude --debug`) for the same debug panel.
...
| `packages/agent/` | Agent core (lead agent, workers, execution, tools) |
