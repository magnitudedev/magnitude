...
Native Windows is supported while we continue to expand and validate hardware backends.
Install the CLI from PowerShell using the commands above. If a selected model or backend is
not available on your hardware yet, choose a currently supported configuration.
