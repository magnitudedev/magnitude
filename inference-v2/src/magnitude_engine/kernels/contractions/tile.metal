...
template <typename T, int BITS, int PACK, int K, int N, int GROUP, int R, bool PREPARED = false, typename X = T>
void magnitude_contract(
    const device X* x, const device uint* w, const device T* scales,
    const device T* biases, const thread int* rows, uint first, uint lane,
    thread float (&acc)[R][4], const device float* sums = nullptr) {
    ...
    for (uint k = lane * PACK; k < K; k += 32 * PACK) {
        ...
        for (uint r = 0; r < R; ++r) {
            ...
            if constexpr (PREPARED) {
                for (uint i = 0; i < PACK; ++i)
                    values[i] = x[size_t(rows[r]) * K + k + i];
                sum = sums[size_t(rows[r]) * (K / PACK) + k / PACK];
            } else {
                sum = magnitude_load<X, BITS, PACK>(x + size_t(rows[r]) * K + k, values);
            }
