...
function findOpentuiIndexFile(): string {
  ...
  if (!match)
    throw new Error("[patch] Could not find @opentui/core/index-*.js");
    ...
    patch: (content, platform, arch) =>
      content.replace(
        ...
        'var module = await import("' +
          getOpentuiNativePackage(platform, arch) +
          '/index.ts");'
      ),
  },
];

async function applyBuildPatches(
  platform: string,
  arch: string
): Promise<void> {
...
export async function buildCliBinary(
  ...
  const binaryFile = resolve(PROJECT_ROOT, "bin", "magnitude-cli" + ext);
  ...
  const entryName = "magnitude-cli" + ext;
