...
describe("notification area model projections", () => {
  ...
  test("derives low memory for the selected model only while insufficient", () => {
        ...
        assessment: {
          ...
          memory: {
            ...
            currentHeadroomState: {
              ...
              minimumAdditionalAvailableBytes: 13,
            },
          },
        },
      },
    ...
    expect(deriveSelectedModelLowMemoryNotificationState(
      ...
      message: "Low memory: close memory-intensive apps (need 0.1 GB) to load model",
      compactMessage: Option.some("Low memory: Free 0.1 GB to load"),
      priority: "warning",
    })
