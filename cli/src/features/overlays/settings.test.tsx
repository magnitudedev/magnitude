...
test("settings starts with detected hardware followed by explicit Magnitude Cloud status", async () => {
  ...
  try {
    ...
    expect(frame).toContain("55.8 GB / 68.7 GB used")
    expect(frame).toContain("Weights       31.1 GB")
    expect(frame).toContain("KV cache      6.4 GB")
    expect(frame).toContain("System & apps 18.3 GB")
    expect(frame).toContain("Free          12.9 GB")
