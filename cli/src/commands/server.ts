...
export const registerServerCommand = (program: Commander): void => {
  const server = program.command("server").description("Manage the Magnitude local inference service")
  server.command("start")
    .description("Install and start the per-user Magnitude service")
    .action(() => runCommand(startServer, "Magnitude server is ready at 127.0.0.1:10100."))
  server.command("stop")
    .description("Stop and disable the per-user Magnitude service")
    .action(() => runCommand(stopService, "Magnitude server stopped."))
}
