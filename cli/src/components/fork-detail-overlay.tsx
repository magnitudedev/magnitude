...
export const ForkDetailOverlay = memo(function ForkDetailOverlay({
  ...
  const allMessages = display?.messages ?? []
  const messages = allMessages.filter(m => m.type === 'think_block' || m.type === 'approval_request')
      ...
      >
        {messages.length === 0 ? (
          ...
          messages.map((msg: DisplayMessage) => {
            const isStreamingMsg = isStreaming && msg === messages[messages.length - 1] && msg.type === 'think_block'
            return (
              <MessageView
                key={msg.id}
                message={msg}
