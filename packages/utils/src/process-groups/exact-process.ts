 ...
 * `@magnitudedev/utils/process-groups/native`, never from the
 * platform-free coordination index.
 */
import { execFile } from "node:child_process"
import { readFile } from "node:fs/promises"
import { Clock, Data, Duration, Effect, Option } from "effect"
import { ProcessStartIdentitySchema } from "./schemas"
import {
  ...
  Effect.tryPromise({
    try: async () => {
      const stat = await readFile(`/proc/${pid}/stat`, "utf8").catch((error: NodeJS.ErrnoException) => {
        // procfs can lose the task after open and report ESRCH from the read.
...
const stop: ProcessGroupController["stop"] = (group, timing) => Effect.gen(function* () {
  ...
  const escalate = (
    ...
    Effect.gen(function* () {
      const delivery = yield* signalGroup(group, signal).pipe(
        Effect.catchTag("ProcessGroupSignalPermissionDenied", error =>
          // Darwin reports EPERM for an exited, unreaped group. Only proven
          // disappearance during the existing grace period can clear that error.
          waitForGroupExit(group, wait).pipe(Effect.flatMap(absent => absent
            ? Effect.succeed<SignalDelivery>({ _tag: "AlreadyAbsent" }) : Effect.fail(error))),
        ),
      )
  ...
  const afterTerm = yield* escalate("term", Duration.decode(timing?.termWait ?? PROCESS_GROUP_TERM_WAIT))
  if (Option.isSome(afterTerm)) return afterTerm.value
  const afterKill = yield* escalate("kill", Duration.decode(timing?.killWait ?? PROCESS_GROUP_KILL_WAIT))
