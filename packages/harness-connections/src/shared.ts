...
export const harnessEndpoints = (origin?: string) => {
  const atOrigin = (url: string) => origin === undefined ? url : new URL(new URL(url).pathname, origin).href.replace(/\/$/, "")
  return { openai: atOrigin(OPENAI_BASE_URL), codex: atOrigin(CODEX_PROXY_BASE_URL), anthropic: atOrigin(ANTHROPIC_BASE_URL) }
}
...
export const tomlTopLevelValue = (source: string, key: string): unknown => {
  const value = parseToml(source.trim() === "" ? "" : source)
...
export const defineConnector = (definition: ConnectorDefinition): HarnessConnector => {
  ...
  return {
    ...
    detect: (searchPath) => Effect.sync(() => Option.fromNullable(findExecutable(definition.executable, searchPath)).pipe(
