...
export const hermesProviderConfig = (baseUrl = OPENAI_BASE_URL) => ({
  name: "Magnitude",
  base_url: baseUrl,
  api_key: LOCAL_TOKEN,
...
export const makeHermesConnector = (paths: HarnessConnectionPaths, baseUrl = OPENAI_BASE_URL) => defineConnector({
  id: "hermes",
  name: "Hermes",
  executable: "hermes",
  ...
  connect: (spec) => Effect.gen(function* () {
    ...
    const changes: Array<readonly [ReadonlyArray<string>, unknown]> = [[
      ["providers", "magnitude"], hermesProviderConfig(baseUrl),
    ]]
