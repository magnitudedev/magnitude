...
export const makePiCompanion = (paths: HarnessConnectionPaths, desiredSource: string = PI_COMPANION_PACKAGE_SOURCE): HarnessCompanionPackage => {
  ...
  return {
    inspect: Effect.gen(function* () {
      ...
      const current = find(document.settings, desired, true)
      return Option.isSome(current)
        && piPackageExtensionEnabled(current.value.entry, rootFor(current.value.source))
        && (yield* inspect(current.value.source))
    }).pipe(Effect.catchTags({ PiPackageError: () => Effect.succeed(false), PiSettingsInvalid: () => Effect.succeed(false), ParseError: () => Effect.succeed(false) })),
