...
export const ohMyPiProviderConfig = (models: HarnessConnectionSpec["models"], baseUrl = OPENAI_BASE_URL) => ({
  baseUrl,
  auth: "none",
...
export const makeOhMyPiConnector = (paths: HarnessConnectionPaths, baseUrl = OPENAI_BASE_URL) => defineConnector({
  id: "oh-my-pi",
  name: "Oh My Pi",
  executable: "omp",
  ...
  connect: (spec) => Effect.gen(function* () {
    ...
    yield* writeIfChanged(paths.ompModels, source, updateYaml(source, [[
      ["providers", "magnitude"], ohMyPiProviderConfig(spec.models, baseUrl),
    ]]))
