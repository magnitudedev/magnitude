...
export const clineProviderSettings = (current: Option.Option<string>, baseUrl = OPENAI_BASE_URL) => ({
...
export const makeClineConnector = (paths: HarnessConnectionPaths, baseUrl = OPENAI_BASE_URL) => defineConnector({
  id: "cline",
  name: "Cline",
  executable: "cline",
  ...
  connect: (spec) => Effect.gen(function* () {
    ...
    const changes: Array<readonly [ReadonlyArray<string>, unknown]> = [
      ...
      [["providers", "openai-compatible"], {
        settings: clineProviderSettings(configuredModel, baseUrl),
  ...
  disconnect: (spec) => Effect.gen(function* () {
    ...
    const next = currentProvider === "openai-compatible" && currentBaseUrl === baseUrl && Option.isSome(spec.restore)
