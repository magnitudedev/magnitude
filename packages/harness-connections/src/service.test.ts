...
import { fixturePackageFiles, piPackageFiles } from "./pi-package-fixture"
...
import { PiPackageSourceSchema } from "./connectors/pi-package-state"
...
const PI_COMPANION_PACKAGE_SOURCE = PiPackageSourceSchema.make("npm:@magnitudedev/pi-extension@0.0.2")
...
const installedService = (paths: HarnessConnectionPaths, resolvedModels = models as ReadonlyArray<(typeof models)[number]>, serviceEndpoint?: string) =>
  makeHarnessConnectionService({
    paths,
    ...(serviceEndpoint === undefined ? {} : { serviceEndpoint }),
...
if (command === "install") {
  ...
  for (const [path, contents] of Object.entries(${JSON.stringify(fixturePackageFiles)})) await Bun.write(resolve(root, path), contents)
}
...
describe("HarnessConnector contract and registry", () => {
  ...
  it("composes one complete connector module per harness in canonical order", () => {
    ...
    for (const connector of registry.ordered) {
      ...
      expect(connector).not.toHaveProperty("launch")
...
describe("Pi without a bundled extension", () => {
  it("connects models and skills without installing or changing Pi packages", async () => {
    await Effect.runPromise(Effect.scoped(Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "magnitude-pi-no-extension-" })
      const paths = fixturePaths(root)
      const packages = ["npm:user-owned-extension"]
      yield* writeFixtures({ ...initialFiles(paths), [paths.piSettings]: stringifyJson({ packages }) })
      const executable = yield* writeFakePiExecutable(paths)
      const service = yield* makeHarnessConnectionService({ paths,
        detect: () => Effect.succeed(Option.some({ executable })), resolveModels: Effect.succeed(models), installStartup: Effect.void })
      const result = yield* service.connect(HarnessIdSchema.make("pi"), { model: Option.some(model), installSkill: true })
      expect(Option.isNone(result.companion)).toBe(true)
      expect(result.skillInstalled).toBe(true)
      expect(readJson(yield* fs.readFileString(paths.piModels))).toMatchObject({ providers: { magnitude: piProviderConfig(models) } })
      expect(readJson(yield* fs.readFileString(paths.piSettings))).toMatchObject({ packages, defaultProvider: "magnitude", defaultModel: model })
...
describe("Pi companion package lifecycle", () => {
  it.each([
    ...
    await Effect.runPromise(Effect.scoped(Effect.gen(function* () {
      ...
      const executable = yield* writeFakePiExecutable(paths)
      const service = yield* makeHarnessConnectionService({ registry: makeHarnessConnectorRegistry(paths, { piCompanionSource: PI_COMPANION_PACKAGE_SOURCE }), paths, detect: () => Effect.succeed(Option.some({ executable })), resolveModels: Effect.succeed(models), installStartup: Effect.void })
  ...
  "packages": [{
    ...
    "autoload": false,
      ...
      yield* writeFixtures({
        ...
        ...piPackageFiles(`${root}/pi/npm/node_modules/@magnitudedev/pi-extension`),
      })
      const executable = yield* writeFakePiExecutable(paths)
      const service = yield* makeHarnessConnectionService({ registry: makeHarnessConnectorRegistry(paths, { piCompanionSource: PI_COMPANION_PACKAGE_SOURCE }), paths, detect: () => Effect.succeed(Option.some({ executable })), resolveModels: Effect.succeed(models), installStartup: Effect.void })
  ...
  it.each(["install-after", "remove-after"] as const)("compensates partial native failure: %s", async (failure) => {
    await Effect.runPromise(Effect.scoped(Effect.gen(function* () {
      ...
      const paths = fixturePaths(root)
      yield* writeFixtures({ ...initialFiles(paths), [paths.manifest]: stringifyJson({ connections: [] }) })
  ...
  it("installs, records ownership, reconciles, and removes an owned package through the shared service", async () => {
    await Effect.runPromise(Effect.scoped(Effect.gen(function* () {
      ...
      const service = yield* makeHarnessConnectionService({
        paths,
        registry: makeHarnessConnectorRegistry(paths, { piCompanionSource: PI_COMPANION_PACKAGE_SOURCE }),
  ...
  it("compensates package-manager and file effects when the enclosing connection fails", async () => {
    await Effect.runPromise(Effect.scoped(Effect.gen(function* () {
      ...
      const executable = yield* writeFakePiExecutable(paths)
      const base = makeHarnessConnectorRegistry(paths, { piCompanionSource: PI_COMPANION_PACKAGE_SOURCE })
...
describe("Magnitude skill installation", () => {
  it("shares one installation and replaces it on every interoperable-harness install", async () => {
    await Effect.runPromise(Effect.scoped(Effect.gen(function* () {
      ...
      expect(installed).toContain("magnitude app open")
...
describe("HarnessConnection model-set behavior", () => {
  ...
  it("registers startup only when explicitly requested", async () => {
    let startupCalls = 0
    await Effect.runPromise(Effect.scoped(Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      ...
      expect(startupCalls).toBe(0)
      yield* service.connect(HarnessIdSchema.make("claude-code"), { model: Option.none(), launchOnStartup: true })
  ...
  it("reconciles repeated setup and selected-model changes for every harness", async () => {
    await Effect.runPromise(Effect.scoped(Effect.gen(function* () {
      ...
      yield* writeFixtures(initial)
      const service = yield* installedService(paths)
      const connectors = makeHarnessConnectorRegistry(paths).ordered
  ...
  it("replaces arbitrary pre-existing Magnitude-owned entries", async () => {
    await Effect.runPromise(Effect.scoped(Effect.gen(function* () {
      ...
      for (const connector of makeHarnessConnectorRegistry(paths).ordered) {
        yield* service.connect(connector.id, { model: Option.some(model) })
      }
  ...
  it("persists model selection and restores it on disconnect", async () => {
    await Effect.runPromise(Effect.scoped(Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
  ...
  it("does not recover during observation; explicit connection recovers invalid entries", async () => {
    await Effect.runPromise(Effect.scoped(Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      ...
      const service = yield* installedService(paths)
      const original = yield* fs.readFileString(paths.manifest)
      expect((yield* Effect.either(service.list))._tag).toBe("Left")
      expect(yield* fs.readFileString(paths.manifest)).toBe(original)
  ...
  it("rejects a model when it is not in the installed Magnitude model set", async () => {
    await Effect.runPromise(Effect.scoped(Effect.gen(function* () {
      ...
      expect(exit._tag).toBe("Failure")
      expect(yield* fs.exists(paths.manifest)).toBe(false)
...
describe("configuration observation", () => {
  for (const id of ["pi", "opencode", "hermes", "openclaw", "codex", "claude-code", "oh-my-pi", "cline"] as const) {
    it.each([undefined, "http://127.0.0.1:11101"])(`${id}: observes configuration at %s without repairing it`, async (serviceEndpoint) => {
      await Effect.runPromise(Effect.scoped(Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem
        const root = yield* fs.makeTempDirectoryScoped({ prefix: "magnitude-connection-integrity-" })
        const paths = fixturePaths(root)
        const service = yield* installedService(paths, models, serviceEndpoint)
        const harness = HarnessIdSchema.make(id)
        yield* service.connect(harness, { model: Option.none() })
        expect((yield* service.list).find((row) => row.id === id)?.connected).toBe(true)
        const file = ({ pi: paths.piModels, opencode: paths.opencode, hermes: paths.hermes,
          ...
          "oh-my-pi": paths.ompModels, cline: paths.clineProviders })[id]
        const original = yield* fs.readFileString(file)
        expect(original).toContain(serviceEndpoint ?? "http://127.0.0.1:10100")
        yield* fs.writeFileString(file, id === "codex" ? 'theme = "light"\n' : '{}\n')
        expect((yield* service.list).find((row) => row.id === id)?.connected).toBe(false)
        expect(yield* fs.readFileString(file)).toBe(id === "codex" ? 'theme = "light"\n' : '{}\n')
        yield* fs.writeFileString(file, original)
        expect((yield* service.list).find((row) => row.id === id)?.connected).toBe(true)
      }).pipe(Effect.provide([BunContext.layer, FetchHttpClient.layer, BunSqliteDriverLayer]))))
    })
  }
  it("ignores unrelated edits but detects a missing required skill", async () => {
    await Effect.runPromise(Effect.scoped(Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "magnitude-connection-skill-" })
      const paths = fixturePaths(root)
      const service = yield* installedService(paths)
      yield* service.connect(HarnessIdSchema.make("pi"), { model: Option.none() })
      const source = yield* fs.readFileString(paths.piModels)
      yield* fs.writeFileString(paths.piModels, updateJsonc(source, [[["theme"], "light"]]))
      expect((yield* service.list).find((row) => row.id === "pi")?.connected).toBe(true)
      yield* fs.remove(paths.skillInstallations["shared-agents"].skillFile)
      expect((yield* service.list).find((row) => row.id === "pi")?.connected).toBe(false)
    }).pipe(Effect.provide([BunContext.layer, FetchHttpClient.layer, BunSqliteDriverLayer]))))
  })
})

describe("read-only connection inspection", () => {
  it("preserves an expected connection failure without embedding its stack in the message", async () => {
    await Effect.runPromise(Effect.scoped(Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "magnitude-connection-no-models-" })
      const service = yield* installedService(fixturePaths(root), [])
      const result = yield* service.connect(HarnessIdSchema.make("claude-code"), { model: Option.none() }).pipe(Effect.either)
      expect(result._tag).toBe("Left")
      if (result._tag === "Left") {
        expect(result.left.message).toBe("No installed Magnitude models are available")
        expect(result.left.operation).toBe("connect")
        expect(result.left.harness).toBe("claude-code")
      }
    }).pipe(Effect.provide([BunContext.layer, FetchHttpClient.layer, BunSqliteDriverLayer]))))
  })

  it("does not create configuration, receipts, or lock files when inspecting a fresh profile", async () => {
    await Effect.runPromise(Effect.scoped(Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "magnitude-inspection-readonly-" })
      const service = yield* installedService(fixturePaths(root))
      expect((yield* service.inspect).every((row) => row.inspection._tag === "Disconnected")).toBe(true)
      expect((yield* service.inspect).every((row) => !row.managed)).toBe(true)
      expect(yield* fs.readDirectory(root)).toEqual([])
    }).pipe(Effect.provide([BunContext.layer, FetchHttpClient.layer, BunSqliteDriverLayer]))))
  })
  it("preserves an invalid receipt and recovers when the file is repaired", async () => {
    await Effect.runPromise(Effect.scoped(Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "magnitude-inspection-corrupt-" })
      const paths = fixturePaths(root)
      yield* writeFixtures({ [paths.manifest]: "{invalid" })
      const original = yield* fs.readFileString(paths.manifest)
      const service = yield* installedService(paths)
      const result = yield* service.inspect.pipe(Effect.either)
      expect(result._tag).toBe("Left")
      if (result._tag === "Left") expect(result.left.message).toBe(`The saved connection record is invalid. Check or restore ${paths.manifest}.`)
      expect(yield* fs.readFileString(paths.manifest)).toBe(original)
      yield* fs.writeFileString(paths.manifest, '{"connections":[]}')
      expect((yield* service.inspect).length).toBe(8)
    }).pipe(Effect.provide([BunContext.layer, FetchHttpClient.layer, BunSqliteDriverLayer]))))
  })
  it("observes intact external configuration without a Magnitude receipt", async () => {
    await Effect.runPromise(Effect.scoped(Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "magnitude-inspection-external-" })
      const paths = fixturePaths(root)
      yield* writeFixtures({ [paths.opencode]: stringifyJson({ provider: { magnitude: openCodeProviderConfig(models) } }) })
      const service = yield* installedService(paths)
      expect((yield* service.inspect).find((row) => row.id === "opencode")?.inspection._tag).toBe("Connected")
      expect(yield* fs.exists(paths.manifest)).toBe(false)
  ...
  it("remembers a chosen optional skill and detects its later removal", async () => {
    await Effect.runPromise(Effect.scoped(Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "magnitude-inspection-optional-skill-" })
      const paths = fixturePaths(root)
      const service = yield* installedService(paths)
      yield* service.connect(HarnessIdSchema.make("opencode"), { model: Option.none(), installSkill: true })
      expect((yield* service.inspect).find((row) => row.id === "opencode")?.inspection._tag).toBe("Connected")
      yield* fs.remove(paths.skillInstallations["shared-agents"].skillFile)
      const reopened = yield* installedService(paths)
      expect((yield* reopened.inspect).find((row) => row.id === "opencode")?.inspection).toEqual({ _tag: "Disconnected", reason: "Magnitude skill is missing or has changed" })
      expect((yield* reopened.inspect).find((row) => row.id === "opencode")?.managed).toBe(true)
      yield* reopened.sync(HarnessIdSchema.make("opencode"))
      expect((yield* reopened.inspect).find((row) => row.id === "opencode")?.inspection._tag).toBe("Connected")
      yield* fs.remove(paths.skillInstallations["shared-agents"].skillFile)
      yield* reopened.disconnect(HarnessIdSchema.make("opencode"))
