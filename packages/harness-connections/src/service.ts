...
const ManifestEntrySchema = Schema.Struct({
  ...
  skillInstalled: Schema.optionalWith(Schema.Boolean, { as: "Option", exact: true }),
...
const upsertManifest = (
  ...
  skillInstalled: Option.Option<boolean>,
  now: () => Date,
) => ({
  connections: [
    ...manifest.connections.filter((entry) => entry.harness !== harness),
    { harness, models: [...models], restore, companion, skillInstalled, updatedAt: now().toISOString() },
  ],
}) satisfies Manifest
...
const discoverMagnitudeModels = (baseUrl?: string) => Effect.gen(function* () {
  const client = yield* makeInferenceClient(baseUrl === undefined ? undefined : { baseUrl })
...
export const makeHarnessConnectionService = (options: HarnessConnectionOptions = {}) => Effect.gen(function* () {
  ...
  const resolveModels = options.resolveModels ?? discoverMagnitudeModels(options.serviceEndpoint === undefined ? undefined : `${options.serviceEndpoint}/inference/`)
  ...
  const inspectConnections = provide(Effect.gen(function* () {
    const manifest = yield* readOr(paths.manifest, '{"connections":[]}').pipe(Effect.flatMap(contents =>
      Schema.decodeUnknown(Schema.parseJson(ManifestSchema))(contents).pipe(Effect.mapError(() =>
        failure("list", `The saved connection record is invalid. Check or restore ${paths.manifest}.`)))))
    return yield* Effect.forEach(registry.ordered, (connector) => Effect.gen(function* () {
      const installation = yield* installed(connector)
      const entry = manifest.connections.find(({ harness }) => harness === connector.id)
      const inspection = yield* inspectHarnessConnection(paths, connector, entry?.models ?? [],
        (connector.skillRequired === true || entry && Option.getOrElse(entry.skillInstalled, () => false)) ? Option.some(skillContents) : Option.none(), options.serviceEndpoint)
      return {
        id: connector.id,
        name: connector.name,
        installed: Option.isSome(installation),
  ...
  const list: HarnessConnection["list"] = inspectConnections.pipe(Effect.map((rows) => rows.map((row): HarnessDestination => {
    const connector = registry.get(row.id)
    return {
      id: row.id, name: row.name,
      availability: row.installed ? "Installed" : "Not installed",
      selectable: row.installed,
      connected: row.inspection._tag === "Connected",
  ...
  const installStartup = provide(options.installStartup ?? Effect.fail(failure("startup", "Application startup registration is unavailable in this host"))).pipe(
  ...
  const withMutationLock = <A, E, R>(operation: HarnessConnectionError["operation"], effect: Effect.Effect<A, E, R>) =>
    provide(mutationLock.withPermits(1)(withConnectionLock(paths.manifest, effect))).pipe(
      Effect.catchAllCause((cause) => Cause.isInterruptedOnly(cause)
        ? Effect.interrupt
        : cause._tag === "Fail" && cause.error instanceof HarnessConnectionError
          ? Effect.fail(cause.error)
          : Effect.fail(failure(operation, Cause.prettyErrors(cause).map(error => error.message).join("; ")))),
    )
  ...
  const connect = (harness: HarnessId, connectOptions: HarnessConnectOptions) => provide(connectionTransaction(Effect.gen(function* () {
    ...
    const shouldInstallStartup = connectOptions.launchOnStartup === true
    ...
    yield* updateManifest((manifest) => upsertManifest(
      ...
      restore,
      companion,
      Option.some(shouldInstallSkill || (existing !== undefined && Option.getOrElse(existing.skillInstalled, () => false))),
      now,
    ))
    return {
  ...
  const sync: HarnessConnection["sync"] = (harness) => provide(Effect.gen(function* () {
    ...
    for (const entry of entries) {
      yield* connectionTransaction(Effect.gen(function* () {
        ...
        if (connector.skillRequired === true || Option.getOrElse(entry.skillInstalled, () => false)) yield* installSkill(entry.harness)
  ...
  const service = {
    list: mutationLock.withPermits(1)(list),
