...
const makeModelSlots = Effect.gen(function* () {
  ...
  const query = effectQuery.Models.GetSlots({})
  const assign = effectQuery.Models.AssignSlot
  const clear = effectQuery.Models.ClearSlot
  const load = effectQuery.Models.LoadSlot
  const stop = effectQuery.Models.StopSlot
  const favorite = effectQuery.Models.SetFavorite
  ...
  const assignments = yield* Mutation.state({
    filters: { mutation: Models.AssignSlot },
  ...
  const selections = Atom.make((get) => Result.map(get(state), (current) => ({
  ...
  return {
    ...
    clearResult,
    favoriteResult,
    retry: queryClient.invalidate(Models.GetSlots.match()),
