...
function classifyTerminalStatus(
  ...
  const failed = terminalDisplayFailed(snapshot)
  if (failed === null) return null
  return root.status._tag === "Interrupted"
    ? "interrupted"
    : failed
      ? "failed"
      : "completed"
...
function terminalDisplayFailed(snapshot: DisplayViewSnapshot): boolean | null {
  ...
  if (latestFailureIndex === -1 && latestSuccessIndex === -1) return null
  return latestFailureIndex > latestSuccessIndex
}
