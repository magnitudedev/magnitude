...
const testProvider = { providerId: 'test-provider', providerName: 'Test Provider' }

describe('classifyHttpError', () => {
  test('maps 401 and 403 to AuthFailed', () => {
    expect(classifyHttpError(401, 'unauthorized', testProvider)).toBeInstanceOf(AuthFailed)
    expect(classifyHttpError(403, 'forbidden', testProvider)).toBeInstanceOf(AuthFailed)
  })

  test('maps auth-signal text to AuthFailed even on non-401/403 status', () => {
    const e1 = classifyHttpError(400, 'missing_scope: api.responses.write', testProvider)
    const e2 = classifyHttpError(500, 'invalid_token', testProvider)
    const e3 = classifyHttpError(429, 'authentication failed: token expired', testProvider)
  ...
  test('keeps context-limit precedence over auth-like words', () => {
    const e = classifyHttpError(401, 'maximum context length reached, unauthorized', testProvider)
    expect(e).toBeInstanceOf(ContextLimitExceeded)
  })

  test('keeps 429 as RateLimited when no auth signal', () => {
    const e = classifyHttpError(429, 'rate limit exceeded, retry-after 2s', testProvider)
  ...
  test('extracts user-facing message from JSON body for UsageLimitExceeded', () => {
    ...
    const e = classifyHttpError(429, rawText, testProvider)
  ...
  test('extracts user-facing message from JSON body for SubscriptionRequired', () => {
    ...
    const e = classifyHttpError(402, rawText, testProvider)
