...
describe("Magnitude provider authentication", () => {
  it("represents missing authentication through each operation's typed failure channel", async () => {
    ...
    const [catalog, webSearch, balance] = await Effect.runPromise(Effect.all([
      Effect.either(instance.catalog.list),
      Effect.either(instance.provider.webSearch("query")),
      Effect.either(instance.provider.balance()),
    ...
    expect(balance).toMatchObject({ _tag: "Left", left: { _tag: "MagnitudeClientError", message: "Magnitude authentication is not configured" } })
  })
