...
it("retries an interrupted installer range and verifies the complete file", async () => {
  ...
  const server = Bun.serve({ hostname: "127.0.0.1", port: 0, fetch(request) {
    ...
    return new Response(new ReadableStream({ start(controller) { controller.enqueue(body); controller.close() } }), {
      status: 206, headers: { "content-range": `bytes ${start}-${end}/${bytes.length}`, etag: '"same-installer"' },
    })
  } })
  try {
