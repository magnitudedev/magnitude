...
export const makeLocalModelInstaller = (
  ...
  const installUnlocked = (
    configurationId: ModelServingConfigurationId,
      ...
      const configuration = yield* resolver.resolve(configurationId).pipe(
      ...
      const download = yield* packages.admitBundle(configuration.bundle)
      const providerModelId = localProviderModelId(configuration.id)
...
export const LocalModelInstallerLive: Layer.Layer<
  LocalModelInstaller,
  never,
  LocalModelPackages | LocalModelConfigurationCoordinator | LocalModelConfigurationResolver
