...
const CountMirror = { name: "GetTestCount" }
const OperationIdsMirror = { name: "GetTestOperationIds" }
...
describe("mirrored state", () => {
  it("updates the snapshot and publishes the matching revision", async () => {
    ...
    expect(Option.getOrThrow(result.invalidation)).toEqual({ query: "GetTestCount", revision: 1 })
  })
