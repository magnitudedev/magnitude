...
const MACT_PROTOCOL_RAW = mactProtocolRaw
...
function renderLensesExample(lenses: ThinkingLens[]): string {
  return lenses
    .map((lens) => `<|think:${lens.name}>\n...${lens.name} reasoning if relevant\n<think|>`)
    .join('\n')
}

/**
 * Generate the Mact protocol prompt for an agent.
 */
export function getMactProtocol(
  lenses: ThinkingLens[],
...
<|invoke:create-task>
<|parameter:id>fix-redirect<parameter|>
<|parameter:title>Fix login redirect bug<parameter|>
<invoke|>

<|invoke:spawn-worker>
<|parameter:id>fix-redirect<parameter|>
<|parameter:message>The redirect function is using '/home' instead of '/login'. Diagnose and fix.<parameter|>
<invoke|>
