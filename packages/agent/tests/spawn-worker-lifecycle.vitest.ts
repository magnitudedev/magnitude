...
describe('spawn-worker lifecycle integration', () => {
  it.live('create-task -> spawn-worker -> message -> worker executes tool -> worker responds', () =>
    Effect.gen(function* () {
      ...
      // Turn 2 (root): spawn worker + send worker instructions
      yield* h.script.next({
        xml: '<spawn-worker id="flow-task" role="builder"/><message to="flow-task">Write output.txt with content "hello"</message><idle/>',
      }, null)
