...
import { serviceProgressFromHealth } from "@magnitudedev/acn-protocol"
...
export const AcnEnsuranceCoordinator = Context.GenericTag<AcnEnsuranceCoordinator>(
  "@magnitudedev/daemon-management/AcnEnsuranceCoordinator",
)
  ...
  const coordinate = Effect.gen(function* () {
    while (true) {
      ...
      if (observation._tag === "AcnRecordedOwnerLiveWithHealth") {
        const progress = serviceProgressFromHealth(observation.health.health.state)
