...
const makeOwnerHttp = (
  ...
  stopOwner: () => void,
  revision = DAEMON_TARGET.revision,
) => {
...
describe("LocalAcnInstanceManager", () => {
  ...
  it("does not adopt a ready owner below the requested revision", async () => {
    await Effect.runPromise(Effect.scoped(Effect.gen(function* () {
      ...
      const selected = AcnRevisionSchema.make(DAEMON_TARGET.revision - 1)
      const exact = yield* ProcessGroupControllerLive.currentProcess
      const server = Bun.serve({
        hostname: "127.0.0.1",
        port: 0,
      ...
      expect(Exit.isFailure(yield* Effect.exit(
        runAcnEnsure(manager.ensure({ target: DAEMON_TARGET })),
      ))).toBe(true)
    }).pipe(Effect.provide(platform))))
  })
  ...
  it("prepares replacement before retiring a lower live owner", async () => {
    await Effect.runPromise(Effect.scoped(Effect.gen(function* () {
      ...
      const lowerRevision = AcnRevisionSchema.make(DAEMON_TARGET.revision - 1)
      ...
      const manager = yield* makeLocalAcnInstanceManagerWithProcessController({
        dataDir,
        launchOverride: { target: DAEMON_TARGET, command: ["unused-test-acn"] },
      }).pipe(
      ...
      const ensuring = yield* runAcnEnsure(manager.ensure({ target: DAEMON_TARGET })).pipe(Effect.fork)
  ...
  it("replaces a stale owner because its dead revision has no authority", async () => {
    await Effect.runPromise(Effect.scoped(Effect.gen(function* () {
      ...
      const ready = yield* runAcnEnsure(manager.ensure({ target: DAEMON_TARGET }))
      expect(ready.id).toBe(id)
      expect(spawns).toBe(1)
    }).pipe(Effect.provide(platform))))
  })
  ...
  it("reports retained stderr when its admitted daemon exits before readiness", async () => {
    await Effect.runPromise(Effect.scoped(Effect.gen(function* () {
      ...
      const result = yield* runAcnEnsure(manager.ensure({ target: DAEMON_TARGET })).pipe(Effect.either)
  ...
  it("fails one stalled candidate at its admission deadline without respawning", async () => {
    await Effect.runPromise(Effect.scoped(Effect.gen(function* () {
      ...
      const ensuring = yield* runAcnEnsure(manager.ensure({ target: DAEMON_TARGET })).pipe(
        Effect.exit,
        Effect.fork,
      )
      while (spawns === 0) yield* Effect.yieldNow()
