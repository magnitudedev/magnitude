...
import { AcnHealthResponseSchema, MAGNITUDE_SERVICE_ORIGIN } from "@magnitudedev/acn-protocol";
import { Context, Data, Effect, Option, Schedule, Schema } from "effect"
...
export const ManagedServiceHost = Context.GenericTag<ManagedServiceHost>("@magnitudedev/daemon-management/ManagedServiceHost")
...
const resolveServiceCommand = (
  acquisitionObserver: Option.Option<ServiceAcquisitionObserver> = Option.none(),
      ...
      const host = yield* ManagedServiceHost
      if (Option.isSome(host.launchCommand)) return host.launchCommand.value
      const resolved = yield* resolveBinaryCommand({
        version: DAEMON_VERSION,
        acnRevision: DAEMON_TARGET.revision,
...
const probeReady = probeHealth.pipe(
  Effect.flatMap((health) => {
    if (health.revision < DAEMON_TARGET.revision || health.state._tag !== "Ready") {
...
export const installService = Effect.gen(function* () {
  ...
  if (Option.isSome(healthy) && healthy.value.revision > DAEMON_TARGET.revision) {
...
const managedServiceIsCurrent = (command: ReadonlyArray<string>) => Effect.gen(function* () {
  ...
  if (compatible.value.revision > DAEMON_TARGET.revision) return true
  const fs = yield* FileSystem.FileSystem
  if (process.platform === "darwin") {
    const service = macServicePath()
