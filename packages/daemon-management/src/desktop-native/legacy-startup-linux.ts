...
export const makeLinuxLegacyStartup = (options: {
  ...
  const verifyDropIns = (paths: string) => Effect.gen(function* () {
    if (paths === "") return
    // Fedora supplies this global timeout policy for every user service.
    const vendorPath = "/usr/lib/systemd/user/service.d/10-timeout-abort.conf"
    if (paths !== vendorPath) return yield* failed("Legacy user service has an unverified override")
    yield* Effect.tryPromise({ try: async () => {
      const info = await lstat(vendorPath)
      if (!info.isFile() || info.uid !== 0 || (info.mode & 0o022) !== 0) throw new Error("Untrusted vendor timeout policy")
      const directives = (await readFile(vendorPath, "utf8")).split(/\r?\n/).map(line => line.trim()).filter(line => line && !line.startsWith("#") && !line.startsWith(";"))
      if (directives.join("\n") !== "[Service]\nTimeoutStopFailureMode=abort") throw new Error("Vendor override changes more than shutdown timing")
    }, catch: error => failed(`Cannot verify systemd vendor policy: ${String(error)}`) })
  })
  const verifyUnit = (value: typeof UnitState.Type) => value.LoadState === "loaded" && value.FragmentPath === path &&
      ["enabled", "enabled-runtime", "disabled"].includes(value.UnitFileState)
    ? verifyDropIns(value.DropInPaths) : Effect.fail(failed("Legacy user service has an unverified source, override, or enablement state"))
  ...
  const unregister = (expected: LegacyLinuxStartup, retireProcesses: Effect.Effect<void, LegacyStartupFailed> = Effect.void) => Effect.gen(function* () {
    ...
    yield* requireSuccess(["stop", "--no-block", unit])
    yield* retireProcesses
    const stopped = yield* Effect.gen(function* () {
      for (;;) {
        const current = yield* state
        yield* verifyUnit(current)
        if (current.MainPID === 0 && ["inactive", "failed"].includes(current.ActiveState)) return current
        yield* Effect.sleep("50 millis")
      }
    }).pipe(Effect.timeoutFail({ duration: "3 seconds", onTimeout: () => failed("Legacy user service remains active") }))
