...
describe("legacy process-tree capture", () => {
  it("ignores unrelated Linux kernel tasks with process group zero", async () => {
    expect(await run(selectLegacyGroups(owner, [...rows, { pid: 2, parent: 0, group: 0 }]))).toEqual([100, 101])
  })
