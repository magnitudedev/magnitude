...
describe.skipIf(process.platform === "win32")("legacy Linux user service", () => {
  ...
  it("disables, stops, removes the verified source, reloads, and tolerates replay", async () => {
    ...
    expect(mutations(f.calls)).toEqual(["disable magnitude.service", "stop --no-block magnitude.service", "daemon-reload"])
  ...
  it("disables restart before process retirement and preserves the record when retirement fails", async () => {
    const f = await fixture()
    const snapshot = Option.getOrThrow(await run(f.adapter.inspect))
    await expect(run(f.adapter.unregister(snapshot, Effect.gen(function* () {
      expect(mutations(f.calls)).toEqual(["disable magnitude.service", "stop --no-block magnitude.service"])
      return yield* new LegacyStartupFailed({ message: "process cleanup interrupted" })
    })))).rejects.toThrow("process cleanup interrupted")
