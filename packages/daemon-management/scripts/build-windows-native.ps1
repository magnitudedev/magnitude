...
try {
  & cl.exe /nologo /c /W4 /WX /O2 /MT /std:c11 /D_WIN32_WINNT=0x0A00 /DNAPI_VERSION=8 /D_CRT_SECURE_NO_WARNINGS "/I$Headers" `
    ...
    (Join-Path $packageRoot 'native\windows-process-napi.c') `
    (Join-Path $packageRoot 'native\windows-process-retirement.c') (Join-Path $packageRoot 'native\windows-retirement-napi.c')
  ...
  & link.exe /nologo /DLL /MACHINE:X64 /DELAYLOAD:node.exe "/OUT:$Output" `
    desktop-host.obj windows-job.obj windows-job-napi.obj windows-security.obj windows-process-napi.obj windows-process-retirement.obj windows-retirement-napi.obj windows-pipe.obj windows-pipe-napi.obj windows-delay-load.obj $NodeLibrary delayimp.lib advapi32.lib shell32.lib ole32.lib user32.lib
  if ($LASTEXITCODE -ne 0) { throw 'Native Windows addon linking failed.' }
  $taskQuery = Join-Path (Split-Path -Parent $Output) 'magnitude-task-query.exe'
  & cl.exe /nologo /W4 /WX /O2 /MT /EHsc /std:c++17 /D_WIN32_WINNT=0x0A00 `
    (Join-Path $packageRoot 'native\windows-task-query.cc') windows-job.obj "/Fe:$taskQuery" `
    /link taskschd.lib ole32.lib oleaut32.lib advapi32.lib uuid.lib bcrypt.lib
  if ($LASTEXITCODE -ne 0) { throw 'Native Windows task query compilation failed.' }
} finally {
  Pop-Location
