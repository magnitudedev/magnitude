...
describe('lens/reason blocks', () => {
  describe('passing sequences', () => {
    it('minimal reason block with about attribute', () => {
      ...
      v.passes(`<magnitude:reason about="alignment">\nsome thought\n</magnitude:reason>\n${YIELD}`)
    })

    it('any lens name is accepted (free-form)', () => {
      const v = shellValidator()
      v.passes(`<magnitude:reason about="tasks">\nthink about tasks\n</magnitude:reason>\n${YIELD}`)
    })

    it('unknown lens name also accepted (not enumerated)', () => {
      const v = shellValidator()
      v.passes(`<magnitude:reason about="anything-at-all">\nsome thought\n</magnitude:reason>\n${YIELD}`)
    })

    it('reason without about attribute (optional)', () => {
      const v = shellValidator()
      v.passes(`<magnitude:reason>\nsome thought\n</magnitude:reason>\n${YIELD}`)
    })
    ...
    it('reason with no newline before next tag', () => {
      const v = shellValidator()
      v.passes(`<magnitude:reason about="turn">\nreasoning\n</magnitude:reason><magnitude:reason about="alignment">\nmore\n</magnitude:reason>\n${YIELD}`)
    })
