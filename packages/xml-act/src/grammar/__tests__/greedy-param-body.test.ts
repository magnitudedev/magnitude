...
describe('greedy param-body — simple embedded close tags', () => {
  it('close tag + space + word', () => {
    shellValidator().passes(
      `<invoke tool="shell">\n<parameter name="command">echo ${CP} done${CP}\n${CI}\n${YIELD}`
    )
  })
  ...
  it('two embedded close tags', () => {
    shellValidator().passes(
      `<invoke tool="shell">\n<parameter name="command">A${CP}B${CP}C${CP}\n${CI}\n${YIELD}`
    )
  })

  it('close tag followed by HTML', () => {
    shellValidator().passes(
      `<invoke tool="shell">\n<parameter name="command">code${CP}<div>more</div>${CP}\n${CI}\n${YIELD}`
    )
  })

  it('close tag mid-sentence', () => {
    shellValidator().passes(
      `<invoke tool="shell">\n<parameter name="command">use ${CP} to end${CP}\n${CI}\n${YIELD}`
    )
  })
})
...
describe('greedy param-body — false commit scenarios (MUST FAIL with eager/deep, PASS with greedy)', () => {
  ...
  it('close + param open with name attr + more content + real close', () => {
    shellValidator().passes(
      `<invoke tool="shell">\n<parameter name="command">START${CP}\n${OP} name="x">MIDDLE${CP}\n${CI}\n${YIELD}`
    )
  })

  it('close + param open (no whitespace) + content + real close', () => {
    shellValidator().passes(
      `<invoke tool="shell">\n<parameter name="command">A${CP}${OP} name="y">B${CP}\n${CI}\n${YIELD}`
    )
  })
  ...
  it('close + invoke close + more content + real close', () => {
    shellValidator().passes(
      `<invoke tool="shell">\n<parameter name="command">fake${CP}${CI}real${CP}\n${CI}\n${YIELD}`
    )
  })

  it('close + newline + invoke close + more content + real close', () => {
    shellValidator().passes(
      `<invoke tool="shell">\n<parameter name="command">fake${CP}\n${CI}\nmore${CP}\n${CI}\n${YIELD}`
    )
  })

  it('close + invoke close + newline + text + real close', () => {
    shellValidator().passes(
      `<invoke tool="shell">\n<parameter name="command">X${CP}${CI}\nY${CP}\n${CI}\n${YIELD}`
    )
  })
  ...
  it('close + filter open + content + real close', () => {
    shellValidator().passes(
      `<invoke tool="shell">\n<parameter name="command">fake${CP}${OF}notreal${CP}\n${CI}\n${YIELD}`
    )
  })
  ...
  it('close + invoke close + garbage + real close + real invoke close', () => {
    shellValidator().passes(
      `<invoke tool="shell">\n<parameter name="command">prefix ${CP}\n${CI}\ngarbage suffix${CP}\n${CI}\n${YIELD}`
    )
  })
  ...
  it('entire fake invoke block in content', () => {
    ...
    shellValidator().passes(
      `<invoke tool="shell">\n<parameter name="command">echo ${CP}\n${CI}\n${OI} tool="fake">\n${OP} name="x">y${CP}\n${CI}\nstill content${CP}\n${CI}\n${YIELD}`
    )
  })
  ...
  it('close + space + invoke close + content', () => {
    shellValidator().passes(
      `<invoke tool="shell">\n<parameter name="command">X${CP} ${CI}Y${CP}\n${CI}\n${YIELD}`
    )
  })
  ...
  it('close + invoke close repeated twice in content', () => {
    shellValidator().passes(
      `<invoke tool="shell">\n<parameter name="command">A${CP}${CI}B${CP}${CI}C${CP}\n${CI}\n${YIELD}`
    )
  })

  it('close + invoke close + spaces + text + real close', () => {
    shellValidator().passes(
      `<invoke tool="shell">\n<parameter name="command">X${CP}${CI}   still content${CP}\n${CI}\n${YIELD}`
    )
  })
  ...
  it('writing XML documentation about invoke close syntax', () => {
    shellValidator().passes(
      `<invoke tool="shell">\n<parameter name="command">echo "close with ${CP} then ${CI} to finish"${CP}\n${CI}\n${YIELD}`
    )
  })

  it('sed replacing invoke close tags', () => {
    shellValidator().passes(
      `<invoke tool="shell">\n<parameter name="command">sed 's/${CP}\\n${CI}/replaced/g' file.xml${CP}\n${CI}\n${YIELD}`
    )
  })

  it('grep searching for close + invoke pattern', () => {
    shellValidator().passes(
      `<invoke tool="shell">\n<parameter name="command">grep -P '${CP}\\s*${CI}' src/*.ts${CP}\n${CI}\n${YIELD}`
    )
  })

  it('python code building XML with close tags', () => {
    shellValidator().passes(
      `<invoke tool="shell">\n<parameter name="command">python3 -c "xml = '${CP}\\n${CI}'; print(xml)"${CP}\n${CI}\n${YIELD}`
    )
  })
...
describe('greedy param-body — realistic content with close-tag-like text', () => {
  ...
  it('sed command replacing XML tags', () => {
    shellValidator().passes(
      `<invoke tool="shell">\n<parameter name="command">sed 's/${CP}/${CF}/g' file.xml${CP}\n${CI}\n${YIELD}`
    )
  })

  it('echo command outputting XML', () => {
    shellValidator().passes(
      `<invoke tool="shell">\n<parameter name="command">echo "${OP} name=\\"x\\">val${CP}"${CP}\n${CI}\n${YIELD}`
    )
  })

  it('grep for close tag pattern', () => {
    shellValidator().passes(
      `<invoke tool="shell">\n<parameter name="command">grep '${CP}' src/*.ts${CP}\n${CI}\n${YIELD}`
    )
  })

  it('documentation about the XML format itself', () => {
    shellValidator().passes(
      `<invoke tool="shell">\n<parameter name="command">echo "Use ${CP} to close a param and ${CI} to close an invoke"${CP}\n${CI}\n${YIELD}`
    )
  })

  it('python code with XML string', () => {
    shellValidator().passes(
      `<invoke tool="shell">\n<parameter name="command">python3 -c "print('${OP} name=x>val${CP}')"${CP}\n${CI}\n${YIELD}`
    )
  })
...
describe('greedy param-body — full turn', () => {
  it('reason + message + invoke with embedded close', () => {
    shellValidator().passes(
      `<reason about="test">thinking</reason>\n` +
      `<message to="user">hello</message>\n` +
      `<invoke tool="shell">\n<parameter name="command">echo ${CP} works${CP}\n${CI}\n${YIELD}`
    )
  })
  ...
  it('multiple invokes, second has embedded close', () => {
    shellValidator().passes(
      `<invoke tool="shell">\n<parameter name="command">ls${CP}\n${CI}\n` +
      `<invoke tool="shell">\n<parameter name="command">echo ${CP} test${CP}\n${CI}\n${YIELD}`
    )
  })
...
describe('greedy param-body — token mask verification', () => {
  it('after close tag, content continuation is accepted', () => {
    ...
    shellValidator().passes(
      `<invoke tool="shell">\n<parameter name="command">hello${CP}world${CP}\n${CI}\n${YIELD}`
    )
  })

  it('after close tag, structural continuation is accepted', () => {
    shellValidator().passes(
      `<invoke tool="shell">\n<parameter name="command">hello${CP}\n${CI}\n${YIELD}`
    )
  })
  ...
  it('after close tag + param open prefix, content is still accepted', () => {
    // This is the key: even after seeing close + "<parameter " (full continuation prefix),
...
describe('greedy param-body — stress tests', () => {
  ...
  it('10 embedded close tags', () => {
    ...
    shellValidator().passes(
      `<invoke tool="shell">\n<parameter name="command">${body}END${CP}\n${CI}\n${YIELD}`
    )
  })
