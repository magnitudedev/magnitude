...
import type { XmlActFrame } from '../format/types'

const id = () => 'abcdefgh1234'

describe('format handlers', () => {
  it('container open emits container open', () => {
    const ops = containerHandler('actions').open({
      tagName: 'actions',
      attrs: new Map(),
      afterNewline: true,
      stack: [{ type: 'prose', body: '', pendingNewlines: 0 }],
      generateId: id,
    })
  ...
  it('think + lens lifecycle', () => {
    const thinkOps = thinkHandler('lenses').open({
      tagName: 'lenses',
      attrs: new Map(),
      afterNewline: true,
      stack: [{ type: 'prose', body: '', pendingNewlines: 0 }],
    ...
    const lensOps = lensHandler().open({
      tagName: 'lens',
      attrs: new Map([['name', 'task']]),
      afterNewline: true,
      stack: [{ type: 'prose', body: '', pendingNewlines: 0 }, think],
      generateId: id,
    })
  ...
  it('message nested depth handled in open/close', () => {
    ...
    const nested = msg.open({
      ...
      attrs: new Map(),
      afterNewline: true,
      stack: [{ type: 'prose', body: '', pendingNewlines: 0 }, messageFrame],
      generateId: id,
    })
  ...
  it('tool open emits TagOpened', () => {
    const ops = toolHandler('shell', new Set(), undefined).open({
      tagName: 'shell',
  ...
  it('child open emits ChildOpened', () => {
    ...
    const ops = childHandler().open({
      ...
      afterNewline: true,
      stack: [{ type: 'prose', body: '', pendingNewlines: 0 }, parent],
      generateId: id,
    })
  ...
  it('content routing for prose + message + tool', () => {
    const proseOps = xmlActContent({ type: 'prose', body: '', pendingNewlines: 0 }, 'hello')
    expect((proseOps[0] as any).event._tag).toBe('ProseChunk')

    const msgOps = xmlActContent(
      { type: 'message', id: 'm1', dest: 'user', artifactsRaw: null, body: '', depth: 0, pendingNewlines: 0 },
      'hey',
    )
    expect(msgOps.some((op) => op.type === 'emit' && (op as any).event._tag === 'MessageChunk')).toBe(true)

    const toolOps = xmlActContent(
      { type: 'tool-body', tag: 'shell', id: 't1', attrs: new Map(), body: '', children: [], childCounts: new Map(), childTags: new Set(), schema: undefined },
      'ls',
    )
  ...
  it('flush emits unclosed container error', () => {
    const ops = xmlActFlush([{ type: 'prose', body: '', pendingNewlines: 0 }, { type: 'container', tag: 'actions', depth: 0 }])
    expect(ops.some((op) => op.type === 'emit' && (op as any).event._tag === 'ParseError')).toBe(true)
  })
})