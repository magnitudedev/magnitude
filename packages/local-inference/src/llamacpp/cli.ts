...
import { renderFitProfileArguments, type LlamaExecutionProfile } from "./execution-profile"
import { boundedFitDiagnostic, estimateLlamaFitMemory, LlamaFitResult, makeLlamaFitPlan, parseLlamaFitPlacement, type LlamaFitResult as LlamaFitResultType } from "./fit"
...
export const LlamaStartOperation = Schema.Literal("validate-options", "start", "observe-exit")
export type LlamaStartOperation = Schema.Schema.Type<typeof LlamaStartOperation>
export const LlamaStartFailureReason = Schema.Literal("command-failed", "invalid-input")
...
export interface LlamaDeviceSnapshot {
  readonly serverFingerprint: LlamaCppInstallation["executables"]["server"]["fingerprint"]
...
export class LlamaStartError extends Data.TaggedError("LlamaStartError")<{ readonly operation: LlamaStartOperation; readonly reason: LlamaStartFailureReason }> {}
...
export interface LlamaCli {
  readonly installation: LlamaCppInstallation
...
export const makeLlamaCli = (installation: LlamaCppInstallation): Effect.Effect<
  LlamaCli,
  never,
  CommandExecutor.CommandExecutor
> => Effect.gen(function* () {
  const executor = yield* CommandExecutor.CommandExecutor
  const output = (executable: string, operation: LlamaCliError["operation"], args: readonly string[]) => {
    ...
    return runParsedCommand(executable, args, parser, CommandCaptureOptions.Default).pipe(Effect.provideService(CommandExecutor.CommandExecutor, executor), Effect.mapError((error) => error._tag === "LlamaCliError" ? error : cliError(operation, "command-failed")))
  }
  return {
    installation,
    ...
    assessFit: ({ files, profile }) => Effect.gen(function* () {
      const args = ["--model", files.primaryPath, ...files.shardPaths.flatMap((file) => ["--model", file]), ...renderFitProfileArguments(profile), "--fit-print", "on"]
      const rawOutput = yield* output(installation.executables.fitParams.path, "fit", args)
      ...
      const projectorFileBytes = Option.flatMap(files.projectorPath, () => Option.map(
        Option.fromNullable(files.record.files.find(({ role }) => role === "projector")),
        ({ sizeBytes }) => sizeBytes,
      ))
      const memory = estimateLlamaFitMemory(placement.value, projectorFileBytes)
      return LlamaFitResult.Estimated({ plan: makeLlamaFitPlan({ fitExecutableFingerprint: installation.executables.fitParams.fingerprint, profileId: profile.id, fileVersion: files.version, arguments: args, placement: placement.value, memory, rawOutput }) })
    }),
    startRouter: (options) => Effect.gen(function* () {
      ...
      const process = yield* Command.start(Command.make(installation.executables.server.path, ...args)).pipe(Effect.provideService(CommandExecutor.CommandExecutor, executor), Effect.mapError(() => startError("start", "command-failed")))
      const capture = yield* captureProcessOutput(installation.executables.server.path, process, { maxOutputBytes: CommandCaptureOptions.ProcessDefault.maxOutputBytes, redactions: [options.apiKey] })
