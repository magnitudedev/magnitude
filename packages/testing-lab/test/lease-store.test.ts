...
test("PostgreSQL fences stale workers, rolls back transactions and serializes Spark reservations", () => Effect.runPromise(Effect.scoped(Effect.gen(function* () {
  ...
  yield* Effect.gen(function* () {
    ...
    expect((yield* fetch(`/v1/runs/${runId}/progress`, "b".repeat(40))).status).toBe(403)
    const progressResponse = yield* fetch(`/v1/runs/${runId}/progress`, "a".repeat(40))
    expect(progressResponse.status).toBe(200)
    const progress = yield* Effect.promise(() => progressResponse.json())
    expect(progress).toMatchObject({ runId, state: "Finished", stages: [{ kind: "build", state: "Finished" }, { kind: "test", state: "Finished" }] })
