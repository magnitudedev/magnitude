...
for (const mode of ["success", "wrong-version", "corrupt", "cleanup-failure", "cancel", "defect"] as const) test(`artifact worker preserves case results and cleanup for ${mode}`, () => Effect.runPromise(Effect.scoped(Effect.gen(function* () {
  ...
  const json = yield* Schema.encode(Schema.parseJson(Schema.Unknown))({ schemaVersion: 1, kind: "artifacts", release: {
  ...
  yield* Schema.decodeUnknown(Schema.parseJson(ArtifactInput))(json)
  ...
  const program = Effect.gen(function* () {
    ...
    const execution = runArtifactWorker(assignment, { root: join(root, "attempt"), port: 11279, model: "fixture", environment: {} })
    if (mode === "cancel") {
