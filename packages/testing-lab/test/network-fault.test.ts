...
import { controlPlaneExceptions, isolationRules, linuxNetworkFault, NetworkFault, NetworkIsolation, NetworkProbeAddress, networkReachable, windowsIsolationRanges } from "../src/network-fault"
...
test("offline evidence records bounded control and DNS exceptions without allowing model traffic", async () => {
  ...
  const isolation = Schema.decodeUnknownSync(NetworkIsolation)({ _tag: "linux", uid: 1000, rule: `magnitude_lab_${"a".repeat(32)}`, ...exceptions })
  if (isolation._tag !== "linux") throw new Error("Expected Linux isolation")
...
test("Windows ranges cover all IPv4 destinations except explicit infrastructure endpoints", () => {
  const ranges = windowsIsolationRanges(["0.0.0.0", "127.0.0.1", "168.63.129.16", "203.0.113.8", "255.255.255.255"])
  expect(ranges).toEqual([
    "0.0.0.1-127.0.0.0",
    "127.0.0.2-168.63.129.15",
    "168.63.129.17-169.254.169.253",
    "169.254.169.255-203.0.113.7",
    "203.0.113.9-255.255.255.254",
  ])
})
