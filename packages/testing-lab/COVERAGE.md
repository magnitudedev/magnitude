...
| Azure Debian 13 / Fedora 44, x64 Intel, CPU | Existing DEB/RPM graph admitted for all nine suites and all three harnesses. Debian running; Fedora stopped during preparation and its allocation was released. | Inspect retained Fedora failure and finish native qualification. This run reuses artifacts and does not establish another source build. |
...
| Azure Windows 10 / 11, x64 CPU | Interactive desktop launcher and native process/login removal checks exercised on a disposable Windows Server diagnostic; VM removed. | Integrate complete Windows tooling/runtime preparation, confirm client licensing eligibility, then run actual Windows 10/11. Server evidence does not qualify either client. |
