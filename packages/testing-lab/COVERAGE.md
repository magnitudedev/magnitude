...
| Azure Debian 13 / Fedora 44, x64 Intel, CPU | Existing DEB/RPM graph exercised: Debian 65 passed, one CLI check blocked by missing `lsof`. Fedora VM creation rejected because its SCSI image was paired with an NVMe-only VM. Both allocations released. | The corrected `lsof` dependency and Fedora v5 sizes are deployed in runtime v14; qualification is running. This run reuses artifacts and does not establish another source build. |
| Azure Red Hat 10, x64 Intel, CPU | Native headless Wayland desktop and failure cleanup verified on Red Hat 10.2. VM removed. | Prepared recipe is deployed in runtime v14; packaged app qualification is queued/running in the active batch. |
| Azure Ubuntu / Debian / Fedora / Red Hat, x64 AMD and arm64 CPU | Native architecture-specific images and tooling deployed for all twelve Linux CPU targets. | Seven x64 consumers and a separate four-target arm64 source-build batch are admitted. Await final reports; image availability is not app qualification. |
| Azure Windows 10 / 11, x64 CPU | Interactive desktop launcher, native process/login removal, pinned tooling installation, real installer-helper compilation, runtime dependencies, Hermes, native terminal startup and one-shot interactive desktop readiness exercised on Windows Server diagnostics. Runtime, Hermes, Rust, terminal subprocess and workspace writes also passed as the admitted desktop user (session 1). This does not qualify client app behavior. | Integrate the verified preparation stages with allocation, confirm client licensing eligibility, then run actual Windows 10/11. |
...
Active Linux runs: x64 `run-45350833-4305-4a1a-91fe-d918aed37992` (seven targets, reusing the built package graph); arm64 `run-cb657d3f-dd0d-440b-adbc-837c64e300df` (source producer and four consumers). Do not treat admission or progress as passing qualification.
