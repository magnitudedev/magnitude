...
| Azure Ubuntu / Debian / Fedora / Red Hat, x64 AMD and arm64 CPU | Native architecture-specific images and tooling deployed for all twelve Linux CPU targets. | Seven x64 consumers are admitted. The ARM64 producer failed because default GCC cannot compile the engine’s SME variants; all four consumers were blocked. Lab compiler selection is being aligned with the release workflow’s Clang configuration before retry. |
...
Linux qualification runs: x64 `run-45350833-4305-4a1a-91fe-d918aed37992` (seven targets, reusing the built package graph); arm64 `run-cb657d3f-dd0d-440b-adbc-837c64e300df` (producer failed; no consumer qualification). All four ARM evidence objects were verified after cleanup (2,180,069 bytes), and Azure inventory contains no resources for that run. Do not treat admission or progress as passing qualification.
