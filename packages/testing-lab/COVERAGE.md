...
| Namespace macOS 15 / 26, arm64 CPU / Metal | Native macOS probes and provider adapter exist; Mac removal checks implemented. Namespace Metal startup calibration fails despite device discovery. | Automatic runtime preparation and reconciliation passed on fresh macOS 15 and 26 machines. macOS 26 uses the image’s Python to bootstrap pinned CPython 3.12.13 for Hermes. The exact Linux coordinator image authenticated to Namespace in an Azure job, and that diagnostic job was deleted. Coordinator v23 is deployed and healthy; a fresh macOS 15 CPU run uses current unpublished source and includes the startup-required Metal distribution pack. The v18 run was cancelled during slow per-object source delivery, before compilation; cleanup passed and Namespace inventory was empty. Complete packaged CPU qualification; resolve actual Metal startup failure. Production signing/update trust remains unqualified. |
...
evidence; the complete lab suite passes 587 tests (14 skipped), and lab typecheck passes. The new
...
Mac v22 full CPU run `run-5503e1ec-c2bb-4a73-8799-5d1fd12896f4` passed compilation
and Finder layout, then exposed missing Metal distribution metadata during packaging. All five
evidence objects / 1,751,009 bytes were verified; cleanup and Namespace inventory were empty.
The corrected source selection includes the startup-required Metal pack for Apple CPU packages;
CPU execution attestation remains unchanged. Fresh v23 run
`run-b0300452-6167-4608-af0d-42009470b075` is active, not qualified.
