...
BunRuntime.runMain(Effect.gen(function* () {
  ...
  const config = yield* fs.readFileString(input).pipe(Effect.flatMap(Schema.decodeUnknown(Schema.parseJson(UbuntuInitialization))))
  const contents = yield* ubuntuInitialization(config)
