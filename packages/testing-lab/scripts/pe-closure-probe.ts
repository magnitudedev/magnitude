...
import { verifyPeDependencyClosure } from "../src/pe-dependency-closure"
...
const main=Effect.scoped(Effect.gen(function*(){
 ...
 for(const [label,system] of [['x64','System32'],['x86','SysWOW64']] as const){
  ...
  reports.push({label,edges:report.edges.length,files:report.files.length,elapsedMs:Date.now()-start,apiSet:report.edges.find(e=>e.name.startsWith('api-ms-'))})
  if(label==='x64'){
   ...
   reports.push({label:'owned-library',files:closure.files.length,edges:closure.edges.length})
   ...
   reports.push({label:'runtime-search-path',files:runtimeClosure.files.length,edges:runtimeClosure.edges.length})
