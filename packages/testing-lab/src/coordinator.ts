...
export const startCoordinator = (config: typeof CoordinatorConfig.Type) => Effect.gen(function* () {
  ...
  const services = yield* Layer.build(Layer.mergeAll(InputRegistryLive, LeaseStoreLive, WorkStoreLive, runStoreLayer(config.accountBudgetUsd)))
  ...
  yield* server.serve(api.pipe(Effect.provide(Context.add(services, Authenticator, auth))))
