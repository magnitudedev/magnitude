...
export const localTransport = Layer.effect(WorkerTransport, Effect.gen(function* () {
  ...
  const environment = Object.fromEntries(["HOME", "USERPROFILE", "PATH", "TMPDIR", "USER", "LOGNAME", "SystemRoot", "ComSpec", "ProgramFiles", "ProgramFiles(x86)", "ProgramW6432", "WINDIR", "PATHEXT", "HOMEDRIVE", "HOMEPATH", "TEMP", "APPDATA", "LOCALAPPDATA", "DISPLAY", "XAUTHORITY", "WAYLAND_DISPLAY", "XDG_RUNTIME_DIR", "XDG_SESSION_TYPE", "DBUS_SESSION_BUS_ADDRESS"].flatMap(key => process.env[key] ? [[key, process.env[key]!]] : []))
