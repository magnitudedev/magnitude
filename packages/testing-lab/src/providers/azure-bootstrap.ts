...
import { MachineTags } from "../machines"
import { type WorkerBootstrap } from "../outward-runner"
...
export const azureLinuxBootstrap = (config: typeof AzureBootstrapConfig.Type) => Effect.gen(function* () {
  ...
  const request = (method: "GET" | "PUT", id: string, body?: unknown) => Effect.scoped(Effect.gen(function* () {
    const args = ["rest", "--subscription", config.subscription, "--method", method, "--url",
      `https://management.azure.com${id}?api-version=2024-11-01`, "--only-show-errors", "--output", "json"]
    if (body !== undefined) {
    ...
    return yield* checkedCommand(config.executable, args, { timeoutMs: 180_000 }).pipe(Effect.provideService(ProcessExecutor, executor))
