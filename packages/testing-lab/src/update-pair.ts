...
export const prepareUpdatePair = (baseline: Digest, candidate: typeof ReleaseManifestSchema.Type, target: Target, directory: string) => Effect.gen(function* () {
  ...
  if (!isNewerVersion(candidate.version, previous.release.version)) return yield* fail("Update candidate must be newer than the previous installed version")
  const previousInstaller = yield* selectInstaller(previous.release, target)
  ...
  const oldPackage = yield* prepareCandidate(previous.release, target, join(directory, "previous"))
