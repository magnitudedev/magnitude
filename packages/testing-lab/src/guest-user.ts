...
export const admitGuestUser = (provider: typeof Provider.Type, disposable: boolean, identity: typeof GuestUserIdentity.Type,
  environment: Readonly<Record<string, string>>, platform: NodeJS.Platform = process.platform) => Effect.gen(function* () {
  ...
  if (provider !== "azure" && provider !== "namespace") return yield* fail("Native desktop user context requires a disposable cloud allocation")
...
export const qualifyGuestUser = (provider: typeof Provider.Type, disposable: boolean, environment: Readonly<Record<string, string>>) => Effect.gen(function* () {
  ...
  const admitted = yield* admitGuestUser(provider, disposable, identity, environment)
