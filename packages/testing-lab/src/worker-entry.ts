...
export const executeGuestInvocation = (invocation: typeof WorkerInvocation.Type, root: string) => Effect.gen(function* () {
  const environment = Object.fromEntries(["PATH", "HOME", "USERPROFILE", "TMPDIR", "USER", "LOGNAME", "SystemRoot", "ComSpec", "ProgramFiles", "ProgramFiles(x86)", "ProgramW6432", "WINDIR", "PATHEXT", "HOMEDRIVE", "HOMEPATH", "TEMP", "APPDATA", "LOCALAPPDATA", "DISPLAY", "XAUTHORITY", "WAYLAND_DISPLAY", "XDG_RUNTIME_DIR", "XDG_SESSION_TYPE", "DBUS_SESSION_BUS_ADDRESS", "CARGO_HOME", "RUSTUP_HOME", "LAB_EXPECTED_APPLE_TEAM_ID", "LAB_EXPECTED_WINDOWS_PUBLISHER", "LAB_WINDOWS_SIGNTOOL", "LAB_TERMINAL_NODE_EXECUTABLE", "LAB_DEPENDENCIES_EXECUTABLE", "LAB_NAMESPACE_METAL_SHIM", "LAB_NAMESPACE_METAL_FAMILY_MAX", "LAB_NAMESPACE_METAL_MAX_THREADGROUP_MEMORY"].flatMap(key => process.env[key] ? [[key, process.env[key]!]] : []))
