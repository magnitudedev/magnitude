...
import { namespaceMetalEnvironment } from "./namespace-metal-compatibility"
...
export const prepareApplicationContext = (root: string, port: number, inherited: Readonly<Record<string, string>>) => Effect.gen(function* () {
  ...
  const candidateEnvironment = yield* namespaceMetalEnvironment(inherited)
  ...
  if (Option.isSome(user)) {
    ...
    const context = ApplicationContext.make({ mode: "installed-user", profile, port: 10100, harnessHome: user.value.home, environment: candidateEnvironment })
  ...
  const context = ApplicationContext.make({ mode: "isolated", profile, port, harnessHome: join(profile, "harness-home"), environment: {
    ...candidateEnvironment, HOME: home, USERPROFILE: home, APPDATA: join(home, "AppData", "Roaming"), LOCALAPPDATA: join(home, "AppData", "Local"),
