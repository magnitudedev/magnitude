...
export const WorkerResultsLive = Layer.effect(WorkerResults, Effect.gen(function* () {
  ...
  const store = yield* Effect.serviceOption(ArtifactStore)
  return {
    submit: (token, reply) => tickets.withAuthority(token, (invocation, tx) => Effect.gen(function* () {
      ...
      if (assignment.work.kind === "build" && reply.result.cases.every(test => test.outcome.status === "passed") && Option.isNone(reply.result.output)) return yield* new InvalidResult({ message: "Successful producer reply requires package output" })
      const claim = reply.claim
      const rows = yield* tx.query("SELECT digest,bytes FROM lab_worker_objects WHERE run_id=$1 AND work_id=$2 AND fence=$3 AND state='Verified'", [claim.runId, claim.workId, claim.fence])
      ...
      if (Option.isSome(reply.result.output)) {
        if (assignment.work.kind !== "build" || Option.isNone(store)) return yield* new InvalidResult({ message: "Only a producer can publish package output" })
        const packages = yield* outputObjects(reply.result.output.value).pipe(Effect.provideService(ArtifactStore, store.value))
        if (packages.some(item => objects.get(item.digest) !== item.bytes)) return yield* new InvalidResult({ message: "Package graph contains objects not verified for this producer attempt" })
      }
      const json = yield* Schema.encode(Schema.parseJson(WorkerReply))(reply)
      const digest = sha256(json)
      const existing = yield* tx.query("SELECT digest FROM lab_worker_results WHERE run_id=$1 AND work_id=$2 AND fence=$3", [claim.runId, claim.workId, claim.fence])
      if (existing[0]) {
      ...
      yield* tx.query("INSERT INTO lab_worker_results(run_id,work_id,fence,reply,digest) VALUES($1,$2,$3,$4,$5)", [claim.runId, claim.workId, claim.fence, json, digest])
    ...
    read: claim => Effect.gen(function* () {
      const rows = yield* db.query("SELECT reply FROM lab_worker_results WHERE run_id=$1 AND work_id=$2 AND fence=$3", [claim.runId, claim.workId, claim.fence])
