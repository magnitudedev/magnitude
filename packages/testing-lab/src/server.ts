...
import { bearerAuthenticator } from "./api"
...
export const ServerConfig = Schema.Struct({ coordinator: CoordinatorConfig,
  ...
  credentials: Schema.NonEmptyArray(Schema.Struct({ tokenEnvironment: Schema.String.pipe(Schema.pattern(/^[A-Z][A-Z0-9_]*$/)), principal: Principal })),
...
export const configuredCoordinator = Effect.gen(function* () {
  ...
  const services = Layer.mergeAll(database, storage, bearerAuthenticator(credentials), Layer.succeed(MachineProviders, { allocators }),
