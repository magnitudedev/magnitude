...
Status presents a quiet service header with a green readiness check on the right, above a divided model row. Model identity is the primary content, with a proportional logo and smaller, separately styled residency text underneath. Stop aligns to the right of the model row. Hardware recommendation artwork belongs to Discover, not Status. Normal background behavior is explained in user terms; native tray registration terminology stays out of the healthy-state UI. Development/test login-startup restrictions explicitly identify the build as such.
