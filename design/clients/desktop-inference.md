...
Status observes live application memory through the privileged desktop host, independently of service readiness. The total covers the application process and its descendants, including service and inference children. It is a timestamped platform measurement, never a model estimate or system capacity. macOS uses physical footprint, Linux proportional resident memory, and Windows private working set; the UI explains their scope. Failed or unsupported observations show unavailable rather than zero or a retained successful total. Sampling belongs to the observed Status view and pauses while the window is hidden. Alongside the application footprint, Status uses the shared hardware-memory derivation over authoritative ACN hardware and active-model residency. Each physical memory domain distinguishes model weights, allocated KV cache, engine buffers, system and other applications, and free capacity. Allocation and process footprint have different accounting and must not be subtracted from one another. Missing or inconsistent domain evidence remains unavailable rather than becoming a fabricated breakdown.

Discover supplements inference hardware with a read-only native description of the client device. Enclosure identity selects bundled, attributed stock photographs through explicit manufacturer/model matches; it never supplies ranking, memory capacity, or processor capabilities. Unknown or placeholder identity cannot imply a known enclosure. A graphics-card image is explicitly a reference component and cannot represent a laptop GPU or complete PC. Photographs remain available offline, retain their source/license attribution, and do not assert an unobserved finish or internal configuration.
