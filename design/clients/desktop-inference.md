...
privileged host. Detected installations appear first, with installation and Magnitude connection status shown separately. Within installed harnesses, connected entries appear first with a green dot and their configuration paths; disconnected entries use a neutral dot and do not present expected paths as existing configuration. Unverifiable configuration remains an explicit unknown state. Its repair guidance includes the known configuration locations to check, without claiming those paths exist or contain a valid connection. Undetected harnesses appear below with their artwork, Not installed status, and an official installation link; configuration details and connection actions are hidden until installation is detected. Detection refreshes automatically and has no manual Detect action. Saved connection receipts govern restoration and ownership, not the Connected label.
