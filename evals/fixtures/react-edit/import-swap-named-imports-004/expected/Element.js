...
export default function Element({ data, index, style }: Props): React.Node {
  ...
  const { ownerFlatTree, ownerID, inspectedElementID } = useContext(TreeStateContext);
  ...
  const isDescendantOfSelected =
    inspectedElementID !== null && !isSelected && store.isDescendantOf(inspectedElementID, id);
  ...
  if (isSelected) {
    ...
    className = treeFocused ? styles.HighlightedElement : styles.InactiveHighlightedElement;
  }
  ...
  return (
    ...
    >
      ...
      <div className={styles.Wrapper}>
        {ownerID === null && <ExpandCollapseToggle element={element} store={store} />}
        ...
        {nameProp && (
          <Fragment>
            ...
            <span className={styles.KeyValue} title={nameProp} onDoubleClick={handleKeyDoubleClick}>
