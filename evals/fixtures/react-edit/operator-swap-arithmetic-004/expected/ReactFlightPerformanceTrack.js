...
import type { ReactComponentInfo, ReactIOInfo, ReactAsyncInfo } from 'shared/ReactTypes';
...
function getIOColor(functionName: string): 'tertiary-light' | 'tertiary' | 'tertiary-dark' {
...
export function logComponentAwaitAborted(
  ...
  if (supportsUserTiming && endTime > 0) {
    const entryName = 'await ' + getIOShortName(asyncInfo.awaited, '', asyncInfo.env, rootEnv);
    const debugTask = asyncInfo.debugTask || asyncInfo.awaited.debugTask;
    if (__DEV__ && debugTask) {
      const properties = [['Aborted', 'The stream was aborted before this Promise resolved.']];
      const tooltipText = getIOLongName(asyncInfo.awaited, '', asyncInfo.env, rootEnv) + ' Aborted';
      debugTask.run(
        // $FlowFixMe[method-unbinding]
...
export function logComponentAwaitErrored(
  ...
  if (supportsUserTiming && endTime > 0) {
    ...
    const entryName =
      'await ' + getIOShortName(asyncInfo.awaited, description, asyncInfo.env, rootEnv);
    const debugTask = asyncInfo.debugTask || asyncInfo.awaited.debugTask;
    if (__DEV__ && debugTask) {
      const message =
        typeof error === 'object' && error !== null && typeof error.message === 'string'
      ...
      const tooltipText =
        getIOLongName(asyncInfo.awaited, description, asyncInfo.env, rootEnv) + ' Rejected';
      debugTask.run(
        // $FlowFixMe[method-unbinding]
        performance.measure.bind(performance, entryName, {
          start: startTime < 0 ? 0 : startTime,
...
export function logComponentAwait(
  ...
  if (supportsUserTiming && endTime > 0) {
    ...
    const name = getIOShortName(asyncInfo.awaited, description, asyncInfo.env, rootEnv);
    ...
    if (__DEV__ && debugTask) {
      ...
      const tooltipText = getIOLongName(asyncInfo.awaited, description, asyncInfo.env, rootEnv);
      debugTask.run(
        // $FlowFixMe[method-unbinding]
        performance.measure.bind(performance, entryName, {
          start: startTime < 0 ? 0 : startTime,
...
export function logIOInfoErrored(ioInfo: ReactIOInfo, rootEnv: string, error: mixed): void {
  ...
  if (supportsUserTiming && endTime >= 0) {
    ...
    if (__DEV__ && debugTask) {
      ...
      const tooltipText = getIOLongName(ioInfo, description, ioInfo.env, rootEnv) + ' Rejected';
...
export function logIOInfo(ioInfo: ReactIOInfo, rootEnv: string, value: mixed): void {
  const startTime = ioInfo.start;
  const endTime = ioInfo.end;
  if (supportsUserTiming && endTime >= 0) {
    const description = getIODescription(value);
    ...
    if (__DEV__ && debugTask) {
      ...
      const tooltipText = getIOLongName(ioInfo, description, ioInfo.env, rootEnv);
      debugTask.run(
        // $FlowFixMe[method-unbinding]
        performance.measure.bind(performance, measureName, {
          start: startTime < 0 ? 0 : startTime,
