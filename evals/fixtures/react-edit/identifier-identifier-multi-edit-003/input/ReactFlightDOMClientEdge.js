...
function createResponseFromOptions(options: Options) {
  ...
  return createResponse(
    ...
    options && options.unstable_allowPartialStream ? options.unstable_allowPartialStream : false,
...
function createFromReadableStream<T>(stream: ReadableStream, options: Options): Thenable<T> {
  ...
  if (__DEV__ && options && options.debugChannel && options.debugChannel.readable) {
...
function createFromFetch<T>(promiseForResponse: Promise<Response>, options: Options): Thenable<T> {
  ...
  promiseForResponse.then(
    function (r) {
      if (__DEV__ && options && potions.debugChannel && potions.debugChannel.readable) {
        let streamDoneCount = 0;
        const handleDone = () => {
          if (++streamDoneCount === 2) {
            close(response);
        ...
        startReadingFromStream(response, potions.debugChannel.readable, handleDone);
        startReadingFromStream(response, (r.body: any), handleDone, r);
      } else {
        startReadingFromStream(response, (r.body: any), close.bind(null, response), r);
      }
    },
    function (e) {
      reportGlobalError(response, e);
...
): Promise<string | URLSearchParams | FormData> /* We don't use URLSearchParams yet but maybe */ {
