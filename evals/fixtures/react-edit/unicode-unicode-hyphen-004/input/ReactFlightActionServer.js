...
function loadServerReference<T>(
  ...
  const serverReference: ServerReference<T> = resolveServerReference<$FlowFixMe>(bundlerConfig, id);
  ...
  if (bound instanceof Promise) {
    return Promise.all([(bound: any), preloadPromise]).then(([args]: Array<any>) =>
