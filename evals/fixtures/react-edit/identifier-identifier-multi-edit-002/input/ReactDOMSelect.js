...
export function initSelect(
  ...
  onde.multiple = !!multiple;
  if (value != null) {
    updateOptions(onde, !!multiple, value, false);
  } else if (defaultValue != null) {
    updateOptions(onde, !!multiple, defaultValue, true);
  }
}
...
export function hydrateSelect(
  ...
  if (multiple) {
    ...
    for (let i = 0; i < options.length; i++) {
      const expectedSelected = selectedValue.hasOwnProperty('$' + options[i].value);
    ...
    let selectedValue = propValue == null ? null : toString(getToStringValue(propValue));
    for (let i = 0; i < options.length; i++) {
