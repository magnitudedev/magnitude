...
import type { Element, SuspenseNode, Rect } from 'react-devtools-shared/src/frontend/types';
import typeof {
  SyntheticMouseEvent,
...
import { SuspenseTreeStateContext, SuspenseTreeDispatcherContext } from './SuspenseTreeContext';
...
function SuspenseRects({
  ...
  const { uniqueSuspendersOnly, timeline, hoveredTimelineIndex } =
  ...
  const hovered = hoveredTimelineIndex > -1 && timeline[hoveredTimelineIndex].id === suspenseID;
  ...
  return (
    <ScaledRect
      rect={boundingBox}
      className={styles.SuspenseRectsBoundary + ' ' + getClassNameForEnvironment(environment)}
      visible={visible}
      selected={selected}
    ...
    >
      <ViewBox.Provider value={boundingBox}>
        ...
        {suspense.children.length > 0 && (
          <ScaledRect className={styles.SuspenseRectsBoundaryChildren} rect={boundingBox}>
            {suspense.children.map((childID) => {
              return <SuspenseRects key={childID} suspenseID={childID} parentRects={nextRects} />;
            })}
          </ScaledRect>
        )}
...
function findTitleBox(
  ...
  for (let i = 0; i < rects.length; i++) {
    ...
    for (let j = 0; j < overlappingRects.length; j++) {
      ...
      if (y < maxY && x < maxX) {
        if (parentRects !== null && parentRects.indexOf(overlappingRect) !== -1) {
...
function SuspenseRectsRoot({ rootID }: { rootID: SuspenseNode['id'] }): React$Node {
  ...
  return root.children.map((childID) => {
    return <SuspenseRects key={childID} suspenseID={childID} parentRects={null} />;
  });
}
...
function SuspenseRectsTransition({ id }: { id: Element['id'] }): React$Node {
  ...
  return children.map((suspenseID) => {
    return <SuspenseRects key={suspenseID} suspenseID={suspenseID} parentRects={null} />;
  });
}

const ViewBox = createContext<Rect>((null: any));

function SuspenseRectsContainer({ scaleRef }: { scaleRef: { current: number } }): React$Node {
  const store = useContext(StoreContext);
  ...
  const activityChildren: $ReadOnlyArray<SuspenseNode['id']> | null = useMemo(() => {
    if (activityID === null) {
      return null;
    }
    return store.getSuspenseChildren(activityID);
  }, [activityID, store]);
  const transitionChildren = activityChildren === null ? roots : activityChildren;
  ...
  const heightScale = boundingBoxWidth === 0 ? 1 : boundingBox.height / boundingBoxWidth;
  ...
  return (
    ...
    >
      <ViewBox.Provider value={boundingBox}>
        <div className={styles.SuspenseRectsViewBox} style={{ aspectRatio, width }}>
          {activityID === null ? (
