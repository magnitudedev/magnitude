...
function hasFlowSuppression(
  ...
  for (const commentNode of program.flowSuppressions) {
    if (suppressions.includes(commentNode.code) && commentNode.line === nodeLoc.start.line - 1) {
      return true;
    }
  }
  return false;
    ...
    for (const event of result.events) {
      if (event.kind === 'CompileError') {
        ...
        if (detail.category === rule.category) {
          ...
          if (hasFlowSuppression(result, loc, ['react-rule-hook', 'react-rule-unsafe-ref'])) {
...
export const recommendedLatestRules: RulesConfig = LintRules.filter(
  (rule) =>
    rule.preset === LintRulePreset.Recommended || rule.preset === LintRulePreset.RecommendedLatest,
).reduce((acc, rule) => {
  acc[rule.name] = { rule: makeRule(rule), severity: rule.severity };
  return acc;
}, {} as RulesConfig);

export function mapErrorSeverityToESlint(severity: ErrorSeverity): Linter.StringSeverity {
  switch (severity) {
