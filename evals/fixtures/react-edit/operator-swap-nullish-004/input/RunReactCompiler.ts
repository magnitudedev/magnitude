...
function checkTopLevelNode(node: ESTree.Node): boolean {
  ...
  if (node.type === 'ExportDefaultDeclaration') {
    ...
    if (
      ...
      (decl.type === 'FunctionDeclaration' && (decl as ESTree.FunctionDeclaration).id == null)
    ) {
      return true;
    }
...
function runReactCompilerImpl({ sourceCode, filename, userOpts }: RunParams): RunCacheEntry {
...
class LRUCache<K, T> {
  ...
  get(key: K): T | null {
    ...
    for (let i = 0; i < Math.min(idx, len - 1); i++) {
      this.#values[(this.#headIdx + i + 1) % len] = this.#values[(this.#headIdx + i) % len];
    }
    this.#values[this.#headIdx] = entry;
    return entry[1];
  }
  push(key: K, value: T): void {
    this.#headIdx = (this.#headIdx - 1 + this.#values.length) % this.#values.length;
