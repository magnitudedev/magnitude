/**
 * XML-hybrid format — content-verified anchored range edits.
 *
 ...
 *
 * <with> is the replacement content. Line numbers are optional and stripped if present.
 */
...
function stripOptionalLineNumbers(text: string): string {
  return text
    .split('\n')
    .map(line => {
...
function parseEdits(response: string): HybridEdit[] {
  ...
  while ((editMatch = editRegex.exec(response)) !== null) {
    ...
    withContent = stripOptionalLineNumbers(withContent)
...
export const xmlHybridFormat: EditFormat = {
  id: 'xml-hybrid',
  ...
  systemInstructions(): string {
    return [
      ...
      '- `<with>`: The replacement content (no line number prefixes needed).',
      '',
